...
-- Existing profile rows already carry updated_at. The application now uses it as an
-- optimistic concurrency token, so an index keeps conditional writes predictable.
...
-- Alliance and owner membership are created in the same PostgreSQL transaction.
-- Any uniqueness, role, or membership failure rolls back the alliance insert.
