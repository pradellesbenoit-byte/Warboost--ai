// WarBoost V2.3.6 — single source of truth for hero identity.
...
export const HERO_DEFINITIONS = [
  ...
  {name:'Carlie',type:'aircraft',aliases:['Carly','Charlie']},
  ...
  {name:'Swift',type:'missile',aliases:['Tonnerre Swift','Tonnere Swift']},
...
function compactWeaponLabel(v){
  return String(v??"").trim().normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g,"");
}

// These are exact, product-specific weapon labels. They are deliberately not
// fuzzy hero-name matching: an exclusive weapon label is allowed to override a
// bad OCR hero_name only when the complete visible label is recognized.
const EXCLUSIVE_WEAPON_HERO_BY_LABEL=new Map([
  ["tonnerreswift","Swift"],
  ["tonnereswift","Swift"]
]);

export function canonicalExclusiveWeaponHeroName(heroName,weaponName){
  const weaponKey=compactWeaponLabel(weaponName),weaponHero=EXCLUSIVE_WEAPON_HERO_BY_LABEL.get(weaponKey);
  if(weaponHero)return weaponHero;
  const embeddedWeaponHero=EXCLUSIVE_WEAPON_HERO_BY_LABEL.get(compactWeaponLabel(heroName));
  if(embeddedWeaponHero)return embeddedWeaponHero;
  return canonicalHeroName(heroName);
}
