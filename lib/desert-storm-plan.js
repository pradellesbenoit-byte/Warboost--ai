...
export function buildDesertStormPlan(members=[],registeredKeys=[],{nowMs=Date.now(),team='A',battleTime='',substituteKeys=[]}={}){
  ...
  const selected=[];const seen=new Set(),explicitSubstituteKeys=new Set((Array.isArray(substituteKeys)?substituteKeys:[]).map(clean).filter(Boolean));
  ...
  if(!selected.length){for(const m of active){if(m?._registered===true){const k=clean(m?._key||m?.lifecycle_key||m?.key||m?.name);if(k&&!seen.has(k)){seen.add(k);selected.push({...m,_key:k,_explicit_substitute:explicitSubstituteKeys.has(k)})}}}}
  for(const member of selected){
    const keys=[member?._key,member?.canonical_member_key,member?.lifecycle_key,rosterLifecycleKey(member),member?.key,member?.roster_key].map(clean).filter(Boolean);
    member._explicit_substitute=keys.some(key=>explicitSubstituteKeys.has(key));
  }
  ...
  const explicitMode=explicitSubstituteKeys.size>0;
  const presentScored=explicitMode?scored.filter(member=>member._explicit_substitute!==true):scored;
  const replacementScored=explicitMode?scored.filter(member=>member._explicit_substitute===true):[];
  const starters=presentScored.slice(0,DESERT_STORM_RULESET.max_starters);
  const substitutes=explicitMode
    ? replacementScored.slice(0,DESERT_STORM_RULESET.max_substitutes)
    : scored.slice(DESERT_STORM_RULESET.max_starters,DESERT_STORM_RULESET.max_starters+DESERT_STORM_RULESET.max_substitutes);
...
function stripInternal(m){const {_score,_evidence,_key,_explicit_substitute,...rest}=m;return {...rest,plan_score:_score||0,plan_evidence:_evidence||{}}}
