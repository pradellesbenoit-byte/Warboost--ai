...
export function allianceParticipationOverview(members,{nowMs=Date.now(),days=30}={}){
  ...
  const linked=list.filter(m=>m?.warboost_linked===true&&String(m?.player_id||'').trim()).length;
  ...
  const insufficientMembers=Math.max(0,list.length-known);
  ...
  return {days,total_members:list.length,linked,known_members:known,insufficient_members:insufficientMembers,linked_known_members:linkedKnown,linked_without_evidence:linkedWithoutEvidence,total_records:totalRecords,participated_records:participatedRecords,confirmed_absences:confirmedAbsences,excused,not_selected:notSelected,insights};
}


// Per-event alliance rollup. Unknown means the active roster member has no known
// participation status for that event type in the selected window. It is never
// converted to an absence or inactivity signal.
export function allianceParticipationByEvent(members,{nowMs=Date.now(),days=30}={}){
  const list=Array.isArray(members)?members:[];
  const by_event=Object.fromEntries(PLAYER_ACTIVITY_EVENT_TYPES.map(type=>[type,{event_type:type,participated:0,absent_confirmed:0,not_selected:0,excused:0,known_members:0,unknown_members:list.length}]));
  for(const member of list){
    const summary=participationSummaryByType(member?.activity_events,{nowMs,days});
    for(const type of PLAYER_ACTIVITY_EVENT_TYPES){
      const src=summary.by_type?.[type]||{};
      const target=by_event[type];
      for(const key of ['participated','absent_confirmed','not_selected','excused'])target[key]+=Number(src[key]||0);
      if(Number(src.total_known||0)>0)target.known_members+=1;
    }
  }
  for(const type of PLAYER_ACTIVITY_EVENT_TYPES){
    const target=by_event[type];
    target.unknown_members=Math.max(0,list.length-target.known_members);
  }
  return {days,total_members:list.length,by_event};
}
