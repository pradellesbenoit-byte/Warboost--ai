...
import {explicitAllianceRank} from './rank-provenance.js';

const MANAGED_RANKS=['R1','R2','R3','R4'];
const ALL_RANKS=['R1','R2','R3','R4','R5'];
function clean(v,max=120){return String(v??'').trim().slice(0,max)}
export function normalizeAllianceRank(v){
  const raw=clean(v,32).toUpperCase(),compact=raw.replace(/[\s_-]+/g,"");
  if(ALL_RANKS.includes(compact))return compact;
...
export function canonicalRosterMemberKey(member={},context={}){
  const explicit=clean(member?.canonical_member_key,260);
  if(explicit)return explicit;
  const alliance_tag=normalizeAllianceTag(member?.alliance_tag||context.allianceTag),server_id=normalizeServerId(member?.server_id||context.serverId),nickname=normalizeLastWarNickname(member?.name,alliance_tag);
  return nickname&&server_id&&alliance_tag?`canonical:${nickname}|${server_id}|${alliance_tag}`:"";
}
function canonicalIdentityKey(member={},context={}){
  const alliance_tag=normalizeAllianceTag(member?.alliance_tag||context.allianceTag),server_id=normalizeServerId(member?.server_id||context.serverId),nickname=normalizeLastWarNickname(member?.name,alliance_tag);
  return nickname&&server_id&&alliance_tag?`canonical:${nickname}|${server_id}|${alliance_tag}`:canonicalRosterMemberKey(member,context);
}
function rowFreshness(row={}){
  const stamp=Date.parse(String(row?.updated_at||row?.rank_confirmed_at||row?.identity_linked_at||""));
  return Number.isFinite(stamp)?stamp:0;
}
function confirmedRankAt(row={}){const stamp=Date.parse(String(row?.rank_confirmed_at||""));return Number.isFinite(stamp)?stamp:0}
function preferCanonicalRow(current={},candidate={}){
  const currentRank=confirmedRankAt(current),candidateRank=confirmedRankAt(candidate);
  if(candidateRank!==currentRank)return candidateRank>currentRank?candidate:current;
  const currentStamp=rowFreshness(current),candidateStamp=rowFreshness(candidate);
  if(candidateStamp!==currentStamp)return candidateStamp>currentStamp?candidate:current;
  if(Boolean(candidate?.player_id||candidate?.warboost_linked)&&!Boolean(current?.player_id||current?.warboost_linked))return candidate;
  return current;
}
export function dedupeCanonicalRosterRows(rows=[],context={}){
  const out=[],indexes=new Map();
  for(const raw of Array.isArray(rows)?rows:[]){
    const row={...raw},key=canonicalIdentityKey(row,context);
    if(!key||!indexes.has(key)){indexes.set(key,out.length);out.push(row);continue}
    const index=indexes.get(key),current=out[index],preferred=preferCanonicalRow(current,row);
    out[index]={...current,...row,...preferred,canonical_member_key:current?.canonical_member_key||row?.canonical_member_key||canonicalRosterMemberKey(preferred,context)};
  }
  return out;
}
export function resolveCanonicalRosterMember(rows=[],raw={},context={}){
  const roster=Array.isArray(rows)?rows:[],requestedKey=clean(raw?.member_key,260);
  if(requestedKey){
    const keyMatches=roster.map((member,index)=>({member,index})).filter(({member})=>canonicalRosterMemberKey(member,context)===requestedKey);
    if(keyMatches.length===1)return {ok:true,index:keyMatches[0].index,member:keyMatches[0].member,mode:"member_key"};
    if(keyMatches.length>1)return {ok:false,code:"member_identity_ambiguous",mode:"member_key",matches:keyMatches.map(x=>x.member)};
  }
  const expectedRaw=clean(raw?.expected_role||raw?.current_role||raw?.from_role,8).toUpperCase();
  const expectedRole=/^R[1-5]$/.test(expectedRaw)?expectedRaw:"";
  if(!expectedRole)return {ok:false,code:"member_identity_required",mode:"identity_fallback",matches:[]};
  const alliance_tag=normalizeAllianceTag(raw?.alliance_tag||context.allianceTag),server_id=normalizeServerId(raw?.server_id||context.serverId),nickname=normalizeLastWarNickname(raw?.name,alliance_tag);
  const matches=roster.map((member,index)=>({member,index})).filter(({member})=>Boolean(nickname&&server_id&&alliance_tag)&&normalizeLastWarNickname(member?.name,alliance_tag)===nickname&&normalizeServerId(member?.server_id||context.serverId)===server_id&&normalizeAllianceTag(member?.alliance_tag||context.allianceTag)===alliance_tag&&normalizeAllianceRank(member?.role)===expectedRole);
  if(matches.length!==1)return {ok:false,code:matches.length?"member_identity_ambiguous":"member_not_found",mode:"identity_fallback",matches:matches.map(x=>x.member)};
  return {ok:true,index:matches[0].index,member:matches[0].member,mode:"identity_fallback"};
}
...
export function rankManagementKey(member={}){return canonicalRosterMemberKey(member)||rosterLifecycleKey(member)}
export function rankCounts(members=[]){const out={R1:0,R2:0,R3:0,R4:0,R5:0};for(const m of Array.isArray(members)?members:[]){const r=explicitAllianceRank(m?.role);if(r)out[r]++}return out}
...
export function previewAllianceRankChanges(members=[],changes=[],{maxR4=10}={}){
  const context={serverId:members?.[0]?.server_id,allianceTag:members?.[0]?.alliance_tag},rows=dedupeCanonicalRosterRows(members,context),byKey=new Map(rows.map(m=>[rankManagementKey(m),m]).filter(([k])=>k));
  ...
  for(const change of normalized){
    ...
    const from=explicitAllianceRank(member.role),to=change.to_role;
    ...
    if(from===to)continue;
    if(from&&from!=='R5')after[from]=Math.max(0,(after[from]||0)-1);
  ...
  return {ok:errors.length===0,changes:resolved,errors,before,after,max_r4:limit,has_r4_transition:resolved.some(x=>x.from_role==='R4'||x.to_role==='R4'),rows};
}
export function applyAllianceRankChanges(members=[],changes=[],{now=new Date().toISOString(),maxR4=10,source='r5_r4_manual_rank_management'}={}){
  ...
  const next=(Array.isArray(members)?members:[]).map(raw=>{
    ...
    return {...raw,role:change.to_role,membership_history:history,rank_confirmed_at:now,rank_confirmed_source:source,rank_confirmation_status:"confirmed_manual",updated_at:now};
  });
