...
export function protectedR5Key(members=[],{ownerPlayerId=""}={}){
  const rows=(Array.isArray(members)?members:[]).filter(m=>normalizeAllianceRank(m?.role)==="R5");
  if(!rows.length)return "";
  const owner=rows.find(m=>ownerPlayerId&&String(m?.player_id||"")===String(ownerPlayerId));
  return rankManagementKey(owner||rows[0])||"";
}
