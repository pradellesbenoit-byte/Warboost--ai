...
export function resolveCanonicalRosterMember(rows=[],raw={},context={}){
  ...
  const matches=roster.map((member,index)=>({member,index})).filter(({member})=>Boolean(nickname&&server_id&&alliance_tag)&&normalizeLastWarNickname(member?.name,alliance_tag)===nickname&&normalizeServerId(member?.server_id||context.serverId)===server_id&&normalizeAllianceTag(member?.alliance_tag||context.allianceTag)===alliance_tag);
