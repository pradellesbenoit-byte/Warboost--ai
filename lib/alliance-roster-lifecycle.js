...
function mergeMember(oldRaw,incomingRaw,{now,returnKind=null}={}){
  ...
  const rankWinner=confirmedRankAt(old)>confirmedRankAt(incoming)?old:incoming;
  const authoritativeRole=confirmedRankAt(rankWinner)>0?role(rankWinner.role):nextRole;
...
export function mergeRosterLifecycleMetadata(localRows=[],freshRows=[]){
  ...
  const fresh=dedupeRows(freshRows),freshKeys=new Set(fresh.map(rosterLifecycleKey));
  const merged=fresh.map(raw=>{
    ...
    const rankWinner=confirmedRankAt(old)>confirmedRankAt(raw)?old:raw;
...
export function preserveVerifiedR5(localRows=[],freshRows=[]){
  const local=dedupeRows(localRows).map(x=>prepared(x,'active'));
