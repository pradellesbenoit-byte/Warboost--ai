...
export async function createAlliance({tag,name,invite_code,owner_player_id}){const rows=await sb("wb1_alliances",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({tag,name,invite_code,owner_player_id,updated_at:new Date().toISOString()})});return rows?.[0]||null}
...
export async function getOwnedAlliance(playerId){
  const rows=await sb(`wb1_alliances?owner_player_id=eq.${encodeURIComponent(playerId)}&select=id,tag,name,invite_code,owner_player_id,updated_at&order=updated_at.desc&limit=1`);
  return rows?.[0]||null;
}
