...
function normalizeShopOffer(o={}){
  return {item_name:cleanString(o?.item_name,120),quantity:numberOrNull(o?.quantity),price:numberOrNull(o?.price),currency:cleanString(o?.currency,60)||null,limit:cleanString(o?.limit,80)||null,discount_pct:numberOrNull(o?.discount_pct),category:cleanString(o?.category,60)||null,rarity:cleanString(o?.rarity,60)||null};
}
...
export function normalizeState(input={}){
  ...
  const squads=Array.from({length:4},(_,i)=>{const q=input.squads?.[i]||{};return {id:i+1,name:cleanString(q.name||`Squad ${i+1}`,40),power:numberOrNull(q.power),updated_at:q.updated_at||null,heroes:Array.from({length:5},(_,j)=>{const h=q.heroes?.[j]||{};return {name:cleanHeroName(h.name),level:numberOrNull(h.level),stars:numberOrNull(h.stars),power:numberOrNull(h.power),exclusive:cleanString(h.exclusive,30)||null,gear:cleanString(h.gear,120)||null}})}});
  ...
  return {version:"2.4.0",player_id:cleanString(input.player_id,120),updated_at:input.updated_at||now,
