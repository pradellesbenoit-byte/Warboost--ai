...
export function normalizeState(input={}){
  const now=new Date().toISOString(),player=input.player||{},drone=input.drone||{},shop=input.shop||{},alliance=input.alliance||{},vs=input.vs||{},season=input.season||{},sync=input.sync||{};
  const squads=Array.from({length:4},(_,i)=>{const q=input.squads?.[i]||{};return {id:i+1,name:cleanString(q.name||`Squad ${i+1}`,40),power:numberOrNull(q.power),updated_at:q.updated_at||null,heroes:Array.from({length:5},(_,j)=>{const h=q.heroes?.[j]||{};return {name:cleanHeroName(h.name),level:numberOrNull(h.level),stars:numberOrNull(h.stars),power:numberOrNull(h.power),exclusive:cleanString(h.exclusive,30)||null,gear:cleanString(h.gear,120)||null,awakening:normalizeAwakening(h)}})}});
  ...
  return {version:"2.4.4",player_id:cleanString(input.player_id,120),updated_at:input.updated_at||now,
...
export function mergeNewest(current={},incoming={}){
  const cur=normalizeState(current),raw={...cur,...incoming,player:{...cur.player,...incoming.player},alliance:{...cur.alliance,...incoming.alliance},vs:{...cur.vs,...incoming.vs},season:{...cur.season,...incoming.season},technology:{...cur.technology,...incoming.technology},sync:{...cur.sync,...incoming.sync,sources:{...cur.sync?.sources,...incoming.sync?.sources}}};
