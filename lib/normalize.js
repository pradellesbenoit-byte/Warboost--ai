...
export function normalizeState(input={}){
  ...
  return {version:"2.4.6",player_id:cleanString(input.player_id,120),updated_at:input.updated_at||now,
...
export function mergeNewest(current={},incoming={}){
  ...
  raw.squads=cur.squads.map((c,i)=>{
    ...
    const heroes=Array.from({length:5},(_,j)=>{
      ...
      // V2.4.6: never attach hero attributes by slot when identity is missing or changed.
