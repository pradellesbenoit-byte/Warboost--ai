...
export function normalizeState(input={}){
  ...
  const squads=Array.from({length:4},(_,i)=>{const q=input.squads?.[i]||{},squadPower=canonicalPowerMillions(q.power_m??q.power);return {id:i+1,name:cleanString(q.name||`Squad ${i+1}`,40),power:squadPower,last_confirmed_power:canonicalPowerMillions(q.last_confirmed_power),power_sync_status:cleanString(q.power_sync_status,20)||(squadPower!==null?"confirmed":"unknown"),updated_at:q.updated_at||null,needs_rescan:q.needs_rescan===true,composition_changed_at:q.composition_changed_at||null,heroes:Array.from({length:5},(_,j)=>{const h=q.heroes?.[j]||{};return {name:cleanHeroName(h.name),level:numberOrNull(h.level),stars:numberOrNull(h.stars),power:parseHeroPower(h.power),exclusive:cleanString(h.exclusive,30)||null,gear:cleanString(h.gear,120)||null,awakening:normalizeAwakening(h)}})}});
