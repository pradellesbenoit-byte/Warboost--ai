...
function normalizeHeroProfile(h={}){
  ...
  return {hero_name,level:numberOrNull(h?.level),stars:numberOrNull(h?.stars),power:parseHeroPower(h?.power),exclusive:numberOrNull(h?.exclusive),gear:cleanString(h?.gear,120)||null,awakening:normalizeAwakening(h),updated_at:h?.updated_at||null,field_updated_at,field_source};
}
...
function normalizeSquadRecord(q={},i){
  ...
  const squad={id:i+1,name:cleanString(q.name||`Squad ${i+1}`,40),power:squadPower,last_confirmed_power:canonicalPowerMillions(q.last_confirmed_power),power_sync_status:cleanString(q.power_sync_status,20)||(squadPower!==null?"confirmed":"unknown"),updated_at:q.updated_at||null,needs_rescan:q.needs_rescan===true,composition_changed_at:q.composition_changed_at||null,composition_confirmed_at:q.composition_confirmed_at||null,composition_source:cleanString(q.composition_source,40)||null,confirmed_composition:Array.isArray(q.confirmed_composition)?q.confirmed_composition:[],composition_conflict:q.composition_conflict&&typeof q.composition_conflict==="object"?q.composition_conflict:null,heroes:Array.from({length:5},(_,j)=>{const h=q.heroes?.[j]||{};return {name:cleanHeroName(h.name),level:numberOrNull(h.level),stars:numberOrNull(h.stars),power:parseHeroPower(h.power),exclusive:cleanString(h.exclusive,30)||null,gear:cleanString(h.gear,120)||null,awakening:normalizeAwakening(h)}})};
  const shaped=normalizeSquadSlots(squad,i+1,{inferLegacy:q.needs_rescan!==true});
  return {...squad,...shaped,heroes:Array.from({length:5},(_,j)=>{const h=shaped.heroes?.[j]||{};return {name:cleanHeroName(h.name),level:numberOrNull(h.level),stars:numberOrNull(h.stars),power:parseHeroPower(h.power),exclusive:cleanString(h.exclusive,30)||null,gear:cleanString(h.gear,120)||null,awakening:normalizeAwakening(h)}})};
}
export function normalizeState(input={}){
  ...
  const exclusive_weapons=Array.isArray(input.exclusive_weapons)?input.exclusive_weapons.slice(0,24).map(w=>({hero_name:cleanHeroName(w?.hero_name)||null,weapon_name:cleanString(w?.weapon_name,80)||null,level:numberOrNull(w?.level),power:parseHeroPower(w?.power),power_raw:cleanString(w?.power_raw||w?.visible_power||"",80)||null,power_parse_status:cleanString(w?.power_parse_status,40)||null,hero_hp_bonus:numberOrNull(w?.hero_hp_bonus),hero_atk_bonus:numberOrNull(w?.hero_atk_bonus),hero_def_bonus:numberOrNull(w?.hero_def_bonus),all_damage_resistance_pct:numberOrNull(w?.all_damage_resistance_pct),max_skill_level:numberOrNull(w?.max_skill_level),updated_at:w?.updated_at||null})).filter(w=>w.hero_name||w.weapon_name||w.level!==null||w.power!==null||w.power_raw):[];
  const hero_progression=Array.isArray(input.hero_progression)?input.hero_progression.slice(0,40).map(h=>{const directPower=parseHeroPower(h?.power),millionPower=canonicalPowerMillions(h?.power_m);return {hero_name:cleanHeroName(h?.hero_name||h?.name)||null,stars:numberOrNull(h?.stars),power:directPower!==null&&directPower>0?directPower:(millionPower===null?null:millionPower*1_000_000),exclusive:numberOrNull(h?.exclusive),awakening:normalizeAwakening(h),updated_at:h?.updated_at||h?.power_updated_at||null}}).filter(h=>h.hero_name):[];
  ...
  const normalized={version:"2.5.28",player_id:cleanString(input.player_id,120),updated_at:input.updated_at||now,
    player:{name:cleanString(player.name,80),server_id:cleanString(player.server_id,20),hq_level:numberOrNull(player.hq_level),power_m:canonicalPowerMillions(player.power_m),coordinates:player.coordinates??null,role:role(player.role),updated_at:player.updated_at||null},
    ...
    drone:{level:numberOrNull(drone.level),power_m:canonicalPowerMillions(drone.power_m),updated_at:drone.updated_at||null},shop:{store_type:shopCurrent.store_type,currency:shopCurrent.currency,currency_balance:shopCurrent.currency_balance,vip_level:shopCurrent.vip_level,vip_days_remaining:shopCurrent.vip_days_remaining,offers:shopOffers,snapshots:shopSnapshots,updated_at:shop.updated_at||null},squads,
    alliance:{id:cleanString(alliance.id,120)||null,owner_player_id:cleanString(alliance.owner_player_id,120)||null,server_id:cleanString(alliance.server_id||player.server_id,20),tag:cleanString(alliance.tag,16).toUpperCase(),name:cleanString(alliance.name,100),role:role(alliance.role),cloud_role_verified:alliance.cloud_role_verified===true,management_verified:alliance.management_verified===true,invite_code:cleanString(alliance.invite_code,40).toUpperCase(),identity_link_status:cleanString(alliance.identity_link_status,40)||"unknown",members:Array.isArray(alliance.members)?alliance.members.slice(0,200).map(m=>normalizeAllianceMember(m,"active")).filter(x=>x.name):[],roster_review:Array.isArray(alliance.roster_review)?alliance.roster_review.slice(0,200).map(m=>normalizeAllianceMember(m,"review")).filter(x=>x.name):[],former_members:Array.isArray(alliance.former_members)?alliance.former_members.slice(0,300).map(m=>normalizeAllianceMember(m,"left_confirmed")).filter(x=>x.name):[],roster_updated_at:alliance.roster_updated_at||null,roster_snapshot_complete_at:alliance.roster_snapshot_complete_at||null,roster_sync_status:cleanString(alliance.roster_sync_status,20)||"unknown",roster_sync_error:cleanString(alliance.roster_sync_error,300)||null,desert_storm:alliance.desert_storm&&typeof alliance.desert_storm==="object"?{team:cleanString(alliance.desert_storm.team,2).toUpperCase()==="B"?"B":"A",battle_time:cleanString(alliance.desert_storm.battle_time,20),registered_keys:Array.isArray(alliance.desert_storm.registered_keys)?alliance.desert_storm.registered_keys.map(x=>cleanString(x,220)).filter(Boolean):[],plan:alliance.desert_storm.plan&&typeof alliance.desert_storm.plan==="object"?alliance.desert_storm.plan:null,updated_at:alliance.desert_storm.updated_at||null}:{team:"A",battle_time:"",registered_keys:[],plan:null,updated_at:null},canyon:normalizeCanyonState(alliance.canyon),unlinked_accounts:Array.isArray(alliance.unlinked_accounts)?alliance.unlinked_accounts.slice(0,200).map(x=>({name:cleanString(x?.name,80),server_id:cleanString(x?.server_id,20),alliance_tag:cleanString(x?.alliance_tag,16).toUpperCase(),reason:cleanString(x?.reason,40),updated_at:x?.updated_at||null})).filter(x=>x.name):[],updated_at:alliance.updated_at||null},
  ...
  return backfillConfirmedHeroPowers(normalized,{now}).state;
}
...
function mergeExclusiveWeaponRows(current=[],incoming=[]){
  ...
  for(const raw of Array.isArray(incoming)?incoming:[]){
    ...
    const old=map.get(k)||{},next={...old,...raw};
    if(confirmedHeroPower(old?.power)&&!confirmedHeroPower(raw?.power))next.power=old.power;
    map.set(k,next);
  }
  return [...map.values()];
}
