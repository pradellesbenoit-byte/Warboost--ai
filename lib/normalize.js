...
export function normalizeState(input={}){
  ...
  return {version:"2.5.28",player_id:cleanString(input.player_id,120),updated_at:input.updated_at||now,
    ...
    alliance:{id:cleanString(alliance.id,120)||null,tag:cleanString(alliance.tag,16).toUpperCase(),name:cleanString(alliance.name,100),role:role(alliance.role),management_verified:alliance.management_verified===true,invite_code:cleanString(alliance.invite_code,32).toUpperCase(),members:Array.isArray(alliance.members)?alliance.members.slice(0,200).map(m=>({player_id:cleanString(m.player_id,120)||null,name:cleanString(m.name,80),hq_level:numberOrNull(m.hq_level),power_m:numberOrNull(m.power_m),role:role(m.role),management_role:role(m.management_role||"R1"),activity_events:mergeActivityEvents(m.activity_events),delta_m:numberOrNull(m.delta_m),vs_points:numberOrNull(m.vs_points),season_points:numberOrNull(m.season_points),contribution:numberOrNull(m.contribution),last_active_at:m.last_active_at||null,updated_at:m.updated_at||null})):[],updated_at:alliance.updated_at||null},
