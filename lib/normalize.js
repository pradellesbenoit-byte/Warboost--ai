...
function mergeNormalizedExclusiveWeaponRows(rows=[]){
  ...
  for(const raw of Array.isArray(rows)?rows:[]){
    ...
    const oldAt=Date.parse(old.updated_at||"")||0,newAt=Date.parse(row.updated_at||"")||0,winner=newAt>oldAt?row:newAt<oldAt?old:row;
    const merged={...old,...row,...winner};
    for(const field of ["weapon_name","level","power","power_raw","power_parse_status","hero_hp_bonus","hero_atk_bonus","hero_def_bonus","all_damage_resistance_pct","max_skill_level","updated_at"]){
      if(row[field]===null||row[field]===undefined||row[field]==="")merged[field]=old[field];
    }
    if(confirmedHeroPower(old.power)&&!confirmedHeroPower(row.power))merged.power=old.power;
