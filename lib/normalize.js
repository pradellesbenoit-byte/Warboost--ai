...
function normalizeAllianceMember(m={},status="active"){return {player_id:cleanString(m.player_id,120)||null,name:cleanString(m.name,80),server_id:cleanString(m.server_id,20),alliance_tag:cleanString(m.alliance_tag,16).toUpperCase(),warboost_linked:m.warboost_linked===true,identity_basis:cleanString(m.identity_basis,60)||null,identity_linked_at:m.identity_linked_at||null,hq_level:numberOrNull(m.hq_level),power_m:numberOrNull(m.power_m),squad_id:numberOrNull(m.squad_id),squad_power_m:numberOrNull(m.squad_power_m??m.main_squad_power_m),squad_power_updated_at:m.squad_power_updated_at||null,squad_type:normalizeAllianceSquadType(m.squad_type),squad_heroes:normalizeAllianceSquadHeroes(m.squad_heroes),squad_profile_updated_at:m.squad_profile_updated_at||null,drone_level:numberOrNull(m.drone_level),drone_power_m:numberOrNull(m.drone_power_m),role:role(m.role),management_role:role(m.management_role||"R1"),activity_events:mergeActivityEvents(m.activity_events),delta_m:numberOrNull(m.delta_m),vs_points:numberOrNull(m.vs_points),season_points:numberOrNull(m.season_points),contribution:numberOrNull(m.contribution),last_active_at:m.last_active_at||null,membership_status:membershipStatus(m.membership_status,status),joined_at:m.joined_at||null,returned_at:m.returned_at||null,left_at:m.left_at||null,missing_from_snapshot_at:m.missing_from_snapshot_at||null,membership_history:normalizeMembershipHistory(m.membership_history),updated_at:m.updated_at||null}}
...
export function normalizeState(input={}){
  ...
  return {version:"2.5.28",player_id:cleanString(input.player_id,120),updated_at:input.updated_at||now,
    player:{name:cleanString(player.name,80),server_id:cleanString(player.server_id,20),hq_level:numberOrNull(player.hq_level),power_m:numberOrNull(player.power_m),coordinates:player.coordinates??null,role:role(player.role),updated_at:player.updated_at||null},
