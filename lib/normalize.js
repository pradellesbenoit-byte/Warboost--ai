...
export function normalizeState(input={}){
  ...
  const squads=Array.from({length:4},(_,i)=>{const q=input.squads?.[i]||{};return {id:i+1,name:cleanString(q.name||`Squad ${i+1}`,40),power:canonicalPowerMillions(q.power_m??q.power),updated_at:q.updated_at||null,needs_rescan:q.needs_rescan===true,composition_changed_at:q.composition_changed_at||null,heroes:Array.from({length:5},(_,j)=>{const h=q.heroes?.[j]||{};return {name:cleanHeroName(h.name),level:numberOrNull(h.level),stars:numberOrNull(h.stars),power:numberOrNull(h.power),exclusive:cleanString(h.exclusive,30)||null,gear:cleanString(h.gear,120)||null,awakening:normalizeAwakening(h)}})}});
