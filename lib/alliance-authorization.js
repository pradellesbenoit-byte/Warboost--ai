...
function role(v){const value=clean(v,8).toUpperCase();return /^R[1-5]$/.test(value)?value:"R1"}
...
// grants the role: owner_player_id, wb1_alliance_members.role, or a uniquely linked
// canonical roster row are the only authority.
export function canonicalAllianceAuthorization({playerId,membership,alliance,roster=[],identity={}}={}){
  ...
  const membershipManager=membershipInAlliance&&isManagerRole(membershipRole);
  const linkedCanonicalManager=Boolean(membershipInAlliance&&linkedSelf&&isManagerRole(linkedCanonicalRole));
  const allowed=Boolean(owner||(membershipManager)||(linkedCanonicalManager));
  ...
  else if(membershipManager)reason="membership_role_confirmed";
