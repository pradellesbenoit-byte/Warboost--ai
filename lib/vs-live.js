...
const LASTWAR_SERVER_SHIFT_MS=2*60*60*1000;
function dateValue(v){const d=v instanceof Date?v:new Date(v);return Number.isNaN(d.getTime())?null:d}
function serverDateKey(v){const d=dateValue(v);if(!d)return null;const x=new Date(d.getTime()-LASTWAR_SERVER_SHIFT_MS);return `${x.getUTCFullYear()}-${String(x.getUTCMonth()+1).padStart(2,'0')}-${String(x.getUTCDate()).padStart(2,'0')}`}
export function vsSnapshotFreshness(v={},opts={}){
  const now=dateValue(opts?.now||new Date()),updated=dateValue(v?.updated_at);
  if(!now)return {status:'unknown',current:false,same_server_day:false,age_seconds:null,updated_at:v?.updated_at||null};
  if(!updated)return {status:'unknown',current:false,same_server_day:false,age_seconds:null,updated_at:null};
  const age=(now.getTime()-updated.getTime())/1000,same=serverDateKey(now)===serverDateKey(updated),future=age < -300;
  return {status:same&&!future?'current':'stale',current:same&&!future,same_server_day:same,age_seconds:Math.max(0,Math.round(age)),updated_at:updated.toISOString(),server_day:serverDateKey(updated),current_server_day:serverDateKey(now)};
}
 ...
 */
export function vsDecisionEngine(v={},opts={}){
  const freshness=opts&&Object.prototype.hasOwnProperty.call(opts,'now')?vsSnapshotFreshness(v,{now:opts.now}):{status:'unchecked',current:true,same_server_day:true,age_seconds:null};
  const sit=vsSituation(v),rawKnown=sit.known,stale=rawKnown&&!freshness.current,trend=stale?null:vsTrend(v),rawRemain=n(v.time_remaining_seconds),ended=!stale&&rawRemain!==null&&rawRemain<=0,remain=!stale&&rawRemain!==null&&rawRemain>0?rawRemain:null;
  const base=DECISION_BASE[sit.status]||DECISION_BASE.unknown;
  const out={
    known:stale?false:sit.known,
    score_known:rawKnown,
    stale,
    freshness,
    ended,
    situation:stale?'stale':sit.status,
    decision:stale?'scan':(ended?'ended':base.decision),
    urgency:stale?'unknown':(ended?'low':base.urgency),
    rescan_minutes:stale?0:(ended?0:base.rescan_minutes),
    risk:stale?'stale_snapshot':(ended?'event_ended':'no_trend'),
    trend:(ended||stale)?null:trend,
    eta_seconds:null,
    eta_kind:null,
    time_remaining_seconds:ended?0:remain,
    gap:sit.gap,
