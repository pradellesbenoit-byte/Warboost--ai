...
export function vsDecisionEngine(v={}){
  const sit=vsSituation(v),trend=vsTrend(v),rawRemain=n(v.time_remaining_seconds),ended=rawRemain!==null&&rawRemain<=0,remain=rawRemain!==null&&rawRemain>0?rawRemain:null;
  const base=DECISION_BASE[sit.status]||DECISION_BASE.unknown;
  const out={
    known:sit.known,
    ended,
    situation:sit.status,
    decision:ended?'ended':base.decision,
    urgency:ended?'low':base.urgency,
    rescan_minutes:ended?0:base.rescan_minutes,
    risk:ended?'event_ended':'no_trend',
    trend:ended?null:trend,
    eta_seconds:null,
    eta_kind:null,
