...
export function mergeConfirmedExclusiveWeaponPowers(input,{weapons=[],updatedAt=null}={}){
  ...
  for(const match of powers.values())mergeProfileRecord(profiles,match.name,{power:match.power},{updatedAt:now,source:"exclusive_weapon"});
  out.hero_profiles=profileArray(profiles);
  const changed=updated.length>0||powers.size>0;
  if(changed)out.updated_at=now;
  return {state:out,changed,updated_heroes:[...new Set([...updated,...[...powers.values()].map(x=>x.name)])]};
}
