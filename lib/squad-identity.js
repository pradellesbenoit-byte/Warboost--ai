...
function buildProfileMap(state,{includeSquads=true}={}){
  ...
  for(const w of Array.isArray(state?.exclusive_weapons)?state.exclusive_weapons:[]){const name=canonicalExclusiveWeaponHeroName(w?.hero_name,w?.weapon_name);if(!name)continue;const at=w?.field_updated_at?.power||w?.power_updated_at||w?.updated_at||null;mergeProfileRecord(map,name,{power:w?.power,exclusive:w?.level},{updatedAt:at,source:"exclusive_weapon"});}
  return map;
}
...
export function backfillConfirmedHeroPowers(input,{now=null}={}){
  ...
  for(const raw of Array.isArray(out.exclusive_weapons)?out.exclusive_weapons:[])add(canonicalExclusiveWeaponHeroName(raw?.hero_name,raw?.weapon_name),raw?.power,sourceTime(raw,null),"exclusive_weapon");
  ...
  for(const candidate of candidates.values())profileChanged=mergeProfileRecord(profileMap,candidate.name,{power:candidate.power},{updatedAt:candidate.updated_at,source:candidate.source})||profileChanged;
...
export function mergeConfirmedExclusiveWeaponPowers(input,{weapons=[],updatedAt=null}={}){
  ...
  for(const raw of Array.isArray(weapons)?weapons:[]){
    const name=canonicalExclusiveWeaponHeroName(raw?.hero_name,raw?.weapon_name),power=confirmedHeroPower(raw?.power);
    if(!name||power===null)continue;
    const key=heroKey(name),candidate={name,power,updated_at:raw?.updated_at||now},previous=powers.get(key);
    if(!previous||timestamp(candidate.updated_at)>=timestamp(previous.updated_at))powers.set(key,candidate);
  }
  ...
  for(const squad of out.squads){
    for(const hero of squad.heroes){
      ...
      if(!match)continue;
      const slotAt=timestamp(hero?.field_updated_at?.power||hero?.power_updated_at||squad?.updated_at);
      if(confirmedHeroPower(hero.power)!==match.power&&(!slotAt||timestamp(match.updated_at)>=slotAt)){hero.power=match.power;updated.push(match.name);}
    }
  }
  const profiles=buildProfileMap(out,{includeSquads:true});
  for(const match of powers.values())mergeProfileRecord(profiles,match.name,{power:match.power},{updatedAt:match.updated_at,source:"exclusive_weapon"});
