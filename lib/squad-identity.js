...
export function backfillConfirmedHeroPowers(input,{now=null}={}){
  const out=ensureShape(input),candidates=new Map(),add=(name,value,updatedAt,source)=>{
    const key=heroKey(name),power=confirmedHeroPower(value);if(!key||power===null)return;
  ...
  for(const raw of Array.isArray(out.hero_profiles)?out.hero_profiles:[])add(raw?.hero_name||raw?.name,raw?.power,raw?.field_updated_at?.power||raw?.updated_at,raw?.field_source?.power||"confirmed_scan");
  for(const raw of Array.isArray(out.exclusive_weapons)?out.exclusive_weapons:[])add(raw?.hero_name,raw?.power,raw?.updated_at,"exclusive_weapon");
  if(!candidates.size)return {state:out,changed:false,updated_heroes:[]};
  const profileMap=buildProfileMap(out,{includeSquads:true}),updated=[];
  for(const candidate of candidates.values())mergeProfileRecord(profileMap,candidate.name,{power:candidate.power},{updatedAt:candidate.updated_at||now,source:candidate.source});
  ...
  return {state:out,changed:updated.length>0,updated_heroes:[...new Set(updated)]};
}
