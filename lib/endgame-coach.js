...
export function deriveEndgameCoachHomeState(report,{privateVisible=false}={}){
  if(!privateVisible)return {visible:false,state:"private",active:false,showProfileScan:false};
  if(report?.eligibility?.eligible===true&&report?.eligibility?.status==="eligible"){
    return {visible:true,state:"eligible",active:true,showProfileScan:false};
  }
  if(report?.eligibility?.status==="unknown"){
    return {visible:true,state:"unknown",active:false,showProfileScan:true};
  }
  return {visible:true,state:"locked",active:false,showProfileScan:false};
}
