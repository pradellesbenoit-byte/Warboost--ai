...
async function db(path,options={}){if(!dbConfigured())throw Object.assign(new Error("Commercial database is not configured."),{status:503,code:"COMMERCIAL_DATABASE_NOT_CONFIGURED"});return parse(await fetchWithTimeout(`${sbUrl()}/rest/v1/${path}`,{...options,headers:adminHeaders(options.headers||{})},6000,{code:"COMMERCIAL_DATABASE_TIMEOUT",message:"Commercial database timed out"}))}
...
async function providerRequest(path,{method="GET",params=null}={}){
  ...
  const r=await fetchWithTimeout(`https://api.stripe.com/v1/${path}`,options,10000,{code:"STRIPE_TIMEOUT",message:"Stripe request timed out"}),body=await r.json().catch(()=>({}));
