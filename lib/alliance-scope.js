...
// A complete roster snapshot is an explicit R5/R4 action. It may shrink the active
// canonical roster, but only to the names supplied in that complete snapshot. Existing
// participation evidence is retained for exact members. Missing rows are not labelled
// inactive here; the client keeps them in a separate review/former-member lifecycle.
export function replaceCanonicalRosterFromCompleteSnapshot(existingRows,incomingRows,{serverId,allianceTag}={}){
  ...
  for(const row of incoming){
    ...
    out.push({...old,...row,activity_events:mergeActivityEvents(old?.activity_events,row.activity_events),updated_at:row.updated_at||old?.updated_at||null});
  }
  return out.slice(0,200);
}
