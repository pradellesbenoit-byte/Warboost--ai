...
export function mergeRosterMembers(existing,imported){
  ...
  for(const incoming of Array.isArray(imported)?imported:[]){
    ...
    const old=rows[idx];
    rows[idx]={...old,name:incoming.name||old.name,role:incoming.role||old.role,hq_level:incoming.hq_level??old.hq_level??null,power_m:incoming.power_m??old.power_m??null,server_id:incoming.server_id||old.server_id||"",alliance_tag:incoming.alliance_tag||old.alliance_tag||"",source:old.player_id?(old.source||"cloud"):"manual_import",imported_at:incoming.imported_at||old.imported_at||null,updated_at:incoming.updated_at||old.updated_at||null,player_id:old.player_id||null,warboost_linked:old.warboost_linked===true,identity_basis:old.identity_basis||null,identity_linked_at:old.identity_linked_at||null};
  }
  return rows;
}
