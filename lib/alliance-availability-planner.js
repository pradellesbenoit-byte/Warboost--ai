...
const state=(v,event)=>{
  ...
  const aliases={participated:'present',confirmed_present:'present',yes:'present',absent_confirmed:'absent',no_show:'absent',replacement:'substitute',remplacant:'substitute',remplaçant:'substitute'};
  const normalized=aliases[s]||s;
  return AVAILABILITY_STATES.includes(normalized)&&(!['vs','season'].includes(event)||normalized!=='substitute')?normalized:'unknown';
};
...
export function normalizeAvailabilityRecord(raw={},defaults={}){
  ...
  const status=state(raw.status||raw.availability||raw.presence||raw.participation_status,event);
...
export function countAvailabilitySlots(records=[],{event_type=null,includeUnknown=true}={}){
  ...
  for(const row of normalizeAvailabilityRecords(records,{event_type})){
    ...
    if(!out[slot])out[slot]={slot,present:0,substitute:0,absent:0,uncertain:0,unknown:0,to_confirm:0,total:0};
...
export function recommendBestSlot(records=[],options={}){
  ...
  rows.sort((a,b)=>b.present-a.present||b.substitute-a.substitute||b.uncertain-a.uncertain||a.to_confirm-b.to_confirm||a.slot.localeCompare(b.slot));
  return rows[0]||{slot:null,present:0,substitute:0,absent:0,uncertain:0,unknown:0,to_confirm:0,total:0};
}
...
export function planAvailabilityRoster(members=[],records=[],{event_type=null,max_starters=20,max_substitutes=10,slot=null}={}){
  ...
  const confirmation=[],eligible=[],explicitSubstitutes=[],excluded=[];
  for(const member of people){
    ...
    else if(row.status==='substitute')explicitSubstitutes.push(item);
    else if(row.status==='absent')excluded.push(item);
    else confirmation.push(item);
  }
  const score=(a,b)=>evidenceScore(b)-evidenceScore(a)||clean(a.name).localeCompare(clean(b.name));
  eligible.sort(score);explicitSubstitutes.sort(score);
  const starters=eligible.slice(0,Math.max(0,max_starters));
  // Legacy data only had "present", so preserve its old overflow behavior when
  // no explicit replacement status exists. Once a replacement is selected,
  // never silently turn an extra present player into a replacement.
  const substitutes=explicitSubstitutes.length
    ? explicitSubstitutes.slice(0,Math.max(0,max_substitutes))
    : eligible.slice(starters.length,starters.length+Math.max(0,max_substitutes));
...
export function availabilityCapacityRoster(members=[],records=[],options={}){
  const roster=planAvailabilityRoster(members,records,options);
  return {
    participants:roster.starters,
    substitutes:roster.substitutes,
    confirmation:roster.confirmation,
    excluded:roster.excluded,
    present_count:roster.starters.length+roster.substitutes.length,
    substitute_count:roster.substitutes.length
  };
}
