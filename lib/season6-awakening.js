...
export function awakeningReadiness(hero,weapons=[],season={}){
  const name=heroName(hero),eligibleType=S6_AWAKENING_HEROES[name]||null,a=awakeningBlock(hero),stars=num(hero?.stars),ex=heroExLevel(hero,weapons),inSeason6=Number(season?.number)===6&&seasonIsActive(season);
  ...
  const hardReady=checks.season6&&checks.eligible&&checks.stars.ok&&checks.exclusive.ok&&(!checks.named_shards.known||checks.named_shards.ok)&&(!checks.trial.known||checks.trial.ok);
...
export function awakeningDecisionScore({hero,weapons=[],season={},mainType=null,formationBonus=0,importance=1}={}){
  const r=awakeningReadiness(hero,weapons,season);if(!r.checks.eligible||!r.checks.season6)return null;
  let score=70;
