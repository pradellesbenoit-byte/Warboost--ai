...
function mergePowerRows(base=[],incoming=[],keyOf){
  ...
  for(const raw of Array.isArray(incoming)?incoming:[]){
    ...
    const old=map.get(key)||{},merged={...old,...clone(raw)};
    if(powerConfirmed(old?.power)&&!powerConfirmed(raw?.power))merged.power=old.power;
    map.set(key,merged);
  }
  return [...map.values()];
}
...
export function hydrateCloudState(remote,defaults,userId=''){
  ...
  out.exclusive_weapons=mergePowerRows(base.exclusive_weapons,src.exclusive_weapons,x=>String(x?.hero_name||x?.weapon_name||"").trim().toLowerCase());
  out.hero_profiles=mergePowerRows(base.hero_profiles,src.hero_profiles,x=>String(x?.hero_name||x?.name||"").trim().toLowerCase());
