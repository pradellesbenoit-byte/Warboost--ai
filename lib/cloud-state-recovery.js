import {canonicalHeroName,canonicalExclusiveWeaponHeroName} from "./heroes.js";
import {normalizeSquadSlots,confirmedCompositionForSquad,backfillConfirmedHeroPowers} from "./squad-identity.js";
import {mergeCanyonState} from "./canyon-storm-plan.js";
import {normalizeUnlinkedAccounts} from "./alliance-identity.js";
...
function desertStormSelectionShape(value={}){
  const registered=stringKeys(value.registered_keys??value.registeredKeys);
  const substitutes=stringKeys(value.substitute_keys??value.substituteKeys).filter(key=>registered.includes(key)).sort();
  const substituteSet=new Set(substitutes);
  return {
    participants:registered.filter(key=>!substituteSet.has(key)).sort(),
    substitutes,
    team:String(value.team||"A").toUpperCase()==="B"?"B":"A",
    battle_time:String(value.battle_time??value.battleTime??"").trim()
  };
}
export function desertStormSelectionSignature(value={}){
  return JSON.stringify(desertStormSelectionShape(objectValue(value)));
}
function validDesertStormPlan(plan){
...
function powerConfirmed(value){const n=typeof value==="number"?value:Number(String(value??"").replace(",","."));return Number.isFinite(n)&&n>0}
function mergePowerRows(base=[],incoming=[],keyOf){
  const map=new Map();
  for(const raw of Array.isArray(base)?base:[]){const key=keyOf(raw);if(key)map.set(key,clone(raw));}
  for(const raw of Array.isArray(incoming)?incoming:[]){
    const key=keyOf(raw);if(!key)continue;
    const old=map.get(key)||{},incomingRow=clone(raw),oldAt=timestamp(old?.updated_at),incomingAt=timestamp(incomingRow?.updated_at),merged=incomingAt>=oldAt?{...old,...incomingRow}:{...incomingRow,...old};
    if(powerConfirmed(old?.power)&&(!powerConfirmed(incomingRow?.power)||oldAt>incomingAt))merged.power=old.power;
    map.set(key,merged);
  }
  return [...map.values()];
}
function canonicalExclusiveRow(row={}){
  return {...clone(row),hero_name:canonicalExclusiveWeaponHeroName(row?.hero_name,row?.weapon_name)||null};
}
function canonicalProfileRow(row={}){
  return {...clone(row),hero_name:canonicalHeroName(row?.hero_name||row?.name)||null};
}
...
export function mergeDesertStormState(base={},incoming={}){
  ...
  const mergedKeys=(key)=>remoteWins?
    stringKeys(hasOwn(remote,key)?remote[key]:older[key]):
    remoteAt===0&&baseAt===0?
      stringKeys([...stringKeys(older[key]),...stringKeys(remote[key])]):
      stringKeys(older[key]);
  const registered_keys=mergedKeys("registered_keys"),substitute_keys=mergedKeys("substitute_keys");
  const selection_initialized=remoteWins?
    (hasOwn(remote,"selection_initialized")?remote.selection_initialized===true:older.selection_initialized===true):
    remoteAt===0&&baseAt===0?
      (older.selection_initialized===true||remote.selection_initialized===true):
      older.selection_initialized===true;
  const merged={...older,...remote,...winner,registered_keys,substitute_keys,selection_initialized,updated_at:winner.updated_at||older.updated_at||remote.updated_at||null};
  merged.team=String(merged.team||"A").toUpperCase()==="B"?"B":"A";
  merged.battle_time=String(merged.battle_time??"").trim();
  const signature=desertStormSelectionSignature(merged);
  const candidates=[older,remote].flatMap((source,index)=>{
    const plan=objectValue(source.plan);
    if(!validDesertStormPlan(plan))return [];
    const planSignature=String(plan.selection_signature||"")||desertStormSelectionSignature(source);
    if(planSignature!==signature)return [];
    return [{plan,at:timestamp(plan.generated_at)||timestamp(source.updated_at),remote:index===1}];
  }).sort((a,b)=>a.at-b.at||(a.remote?1:-1));
  const selectedPlan=candidates[candidates.length-1];
  merged.plan=selectedPlan?{...selectedPlan.plan,selection_signature:signature}:null;
  return merged;
}
...
export function hydrateCloudState(remote,defaults,userId=''){
  ...
  const previousPlayerName=out.player?.name;
  // Pending association rows are derived, not profile state. Never resurrect
  // the browser's older queue when a cloud profile omits this field.
  const rawUnlinked=hasOwn(objectValue(src.alliance),'unlinked_accounts')?chooseArray(objectValue(src.alliance),'unlinked_accounts'):[],currentId=String(userId||src.player_id||''),cachedPlayerName=String(base.player_id||'')===currentId?objectValue(base.player).name:'';
  out.alliance.unlinked_accounts=normalizeUnlinkedAccounts(rawUnlinked,out.alliance.members,{serverId:out.alliance.server_id||out.player?.server_id,allianceTag:out.alliance.tag,currentPlayerId:userId||out.player_id,currentPlayerName:previousPlayerName,currentPlayerAliases:[cachedPlayerName],identityLinkStatus:out.alliance.identity_link_status});
  const canonicalSelf=out.alliance.members.find(member=>member?.warboost_linked===true&&String(member?.player_id||'').trim()===String(userId||out.player_id||'').trim()&&String(member?.name||'').trim());
  if(canonicalSelf?.name)out.player={...out.player,name:canonicalSelf.name};
  ...
  out.alliance.canyon=mergeCanyonState(objectValue(base.alliance).canyon,objectValue(src.alliance).canyon);
  ...
  for(const key of ['activity_events','hero_progression','progression_snapshots'])out[key]=chooseArray(src,key,base[key]);
  out.exclusive_weapons=mergePowerRows(arrayValue(base.exclusive_weapons).map(canonicalExclusiveRow),arrayValue(src.exclusive_weapons).map(canonicalExclusiveRow),x=>String(x?.hero_name||x?.weapon_name||"").trim().toLowerCase());
  out.hero_profiles=mergePowerRows(arrayValue(base.hero_profiles).map(canonicalProfileRow),arrayValue(src.hero_profiles).map(canonicalProfileRow),x=>String(x?.hero_name||x?.name||"").trim().toLowerCase());
  const restored=backfillConfirmedHeroPowers(out);
  restored.state.player_id=String(userId||src.player_id||base.player_id||'');
