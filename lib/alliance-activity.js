import {activityEventEvidence} from "./activity-events.js";
...
export function normalizeAllianceRole(value){
  ...
  if(/^(?:ROLE|RANK|RANG)R?[1-5]$/.test(compact))return compact.match(/[1-5]$/)?.[0]?`R${compact.match(/[1-5]$/)[0]}`:"R1";
...
export function classifyAllianceMember(member={},nowMs=Date.now()){
  ...
  const eventEvidence=activityEventEvidence(member.activity_events,nowMs);
  const dataSignals=(freshnessH!==null?1:0)+(lastActiveH!==null?1:0)+statSignals+(explicitStatus?1:0)+(eventEvidence.count>0?1:0);

  // Player-confirmed event participation is WarBoost evidence, not official Last War telemetry.
  if(eventEvidence.latest_hours!==null&&eventEvidence.latest_hours<=72){
    return {key:"active",confidence:"high",freshnessH,lastActiveH,positive,explicitZero,eventEvidence,reason:"recent_event_confirmation"};
  }
  if(eventEvidence.latest_hours!==null&&eventEvidence.latest_hours<=168){
    return {key:"active",confidence:"medium",freshnessH,lastActiveH,positive,explicitZero,eventEvidence,reason:"recent_event_confirmation"};
  }
...
export function summarizeAllianceActivity(members=[],nowMs=Date.now()){
  ...
  const counts={active:0,refresh:0,insufficient:0,inactive:0,unknown:0};
  for(const row of rows){
    if(row.activity.key==="active")counts.active++;
    else if(row.activity.key==="inactive")counts.inactive++;
    else if(row.activity.key==="unknown"){counts.unknown++;counts.refresh++;counts.insufficient++;}
    else {counts.refresh++;counts.insufficient++;}
  }
