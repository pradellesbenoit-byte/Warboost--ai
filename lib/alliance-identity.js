...
export function normalizeUnlinkedAccounts(accounts=[],members=[],{serverId="",allianceTag="",currentPlayerId="",currentPlayerName="",currentPlayerAliases=[],identityLinkStatus=""}={}){
  ...
  const currentNameKeys=new Set(currentLinked?[currentPlayerName,...(Array.isArray(currentPlayerAliases)?currentPlayerAliases:[])].map(name=>normalizeLastWarNickname(name,currentLinked.alliance_tag||allianceTag)).filter(Boolean):[]);
  return (Array.isArray(accounts)?accounts:[]).map(raw=>sanitizeUnlinkedAccount(raw)).filter(account=>{
    ...
    // already confirmed the current account, its previous local profile name is
    // sufficient to remove that self entry, but only within the exact scope.
    if(!accountId&&currentLinked&&currentNameKeys.has(normalizeLastWarNickname(account.name,account.alliance_tag||allianceTag))&&identityScopeMatches(account,currentLinked,fallback))return false;
    const key=normalizeLastWarNickname(account.name,account.alliance_tag||allianceTag);
    if(!key)return true;
    // Pending rows are derived. An exact nickname + server + alliance match to
    // any roster row already marked linked is stale even if an old row omitted
    // player_id or carried an obsolete player_id.
    const exactLinkedMatch=roster.some(member=>member?.warboost_linked===true&&identityScopeMatches(account,member,fallback)&&normalizeLastWarNickname(member.name,member.alliance_tag||allianceTag)===key);
    return !exactLinkedMatch;
  });
}

export function rebuildCanonicalPendingAccounts(source={}, {currentPlayerId=source?.player_id,currentPlayerAliases=[]}={}){
  const alliance=source?.alliance||{};
  return normalizeUnlinkedAccounts(
    Array.isArray(alliance.unlinked_accounts)?alliance.unlinked_accounts:[],
    Array.isArray(alliance.members)?alliance.members:[],
    {
      serverId:alliance.server_id||source?.player?.server_id||"",
      allianceTag:alliance.tag||"",
      currentPlayerId:currentPlayerId||"",
      currentPlayerName:source?.player?.name||"",
      currentPlayerAliases,
      identityLinkStatus:alliance.identity_link_status||""
    }
  );
}
