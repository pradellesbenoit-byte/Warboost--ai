...
// A linked canonical roster row wins over any older local/cloud pending entry, but
// scope must still match so an account from another server or alliance is retained.
export function normalizeUnlinkedAccounts(accounts=[],members=[],{serverId="",allianceTag="",currentPlayerId="",currentPlayerName="",identityLinkStatus=""}={}){
  ...
  const currentLinked=currentId?roster.find(member=>establishedRosterLink(member)&&String(member.player_id).trim()===currentId&&identityScopeMatches({server_id:serverId,alliance_tag:allianceTag},member,fallback)):null;
  ...
  return (Array.isArray(accounts)?accounts:[]).map(raw=>sanitizeUnlinkedAccount(raw)).filter(account=>{
    ...
    const linkedById=accountId&&roster.some(member=>establishedRosterLink(member)&&String(member.player_id).trim()===accountId&&identityScopeMatches(account,member,fallback));
