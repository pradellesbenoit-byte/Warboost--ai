...
import {backfillRosterIdentityContext,dedupeRosterAccountLinks,identityCandidates,lastWarIdentity,linkCurrentPlayerIdentityIntoRoster,sanitizeUnlinkedAccount} from './alliance-identity.js';
import {normalizeRosterRemovalTombstones} from './alliance-roster-lifecycle.js';
import {canonicalRosterMemberKey} from './alliance-rank-management.js';
...
function deactivateLink(m){return {...m,player_id:null,warboost_linked:false,identity_basis:null}}
...
export function mergeCloudRosterWithIdentity(existingRows,cloudRows,{serverId,allianceTag,removal_tombstones=[]}={}){
  const existing=dedupeRosterAccountLinks(backfillRosterIdentityContext(existingRows,{serverId,allianceTag}));
  const tombstones=normalizeRosterRemovalTombstones(removal_tombstones),blocked=new Set(tombstones.filter(x=>!x.rejoined_at).map(x=>x.key));
  const cloud=dedupeCloudAccounts(backfillRosterIdentityContext(cloudRows,{serverId,allianceTag})).filter(m=>!blocked.has(canonicalRosterMemberKey(m,{serverId,allianceTag})));
  if(!existing.length){
  ...
  for(const m of cloud){
    ...
    let idx=-1;
    const candidates=cloudIdentityReady(m)?identityCandidates(out,{name:m.name,server_id:m.server_id,alliance_tag:m.alliance_tag}).filter(x=>!claimed.has(x.index)):[];
    const previousIdx=playerId?out.findIndex((x,i)=>!claimed.has(i)&&String(x?.player_id||'').trim()===playerId&&establishedLink(x)):-1;
    const exactOwned=candidates.filter(x=>String(x.m?.player_id||'').trim()===playerId&&establishedLink(x.m));
    if(exactOwned.length===1)idx=exactOwned[0].index;
    else if(candidates.length===1)idx=candidates[0].index;
    else if(candidates.length>1){unlinked.push(sanitizeUnlinkedAccount({...m,reason:'ambiguous_roster_match'}));continue}
    // A cloud account may have an old nickname while the canonical roster
    // already contains its current exact identity. Move the active link to
    // that exact row; never let the stale row count as a second association.
    if(idx>=0&&previousIdx>=0&&previousIdx!==idx)out[previousIdx]=deactivateLink(out[previousIdx]);
    // If this exact WarBoost account ID already owns an established canonical
    // row, an older cloud nickname is not a second pending association. Keep
    // the existing link and roster data; only an exact identity candidate may
    // move it to another row.
    if(idx<0&&previousIdx>=0){claimed.add(previousIdx);continue}
