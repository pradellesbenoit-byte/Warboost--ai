...
export function createWarBoostSupabaseAuthClient({url,key,storage=defaultStorage(),fetchImpl=globalThis.fetch,refreshSkewSeconds=60}={}){
  ...
  scheduleRefresh();
  return {auth:{getSession,onAuthStateChange,signInWithPassword,signUp,verifyOtp,resend,signOut,refreshSession},diagnostics:{transport:'direct-supabase-auth-api',external_cdn:false,storage_key:storageKey}};
}
