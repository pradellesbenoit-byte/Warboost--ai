...
async function markAccepted(row,user){
  ...
  return await inviteRest(`wb1_beta_invites?id=eq.${encodeURIComponent(row.id)}`,{method:"PATCH",headers:{Prefer:"return=minimal"},body:JSON.stringify(payload)},1800);
}
...
export async function betaAccessForUserAsync(user,{trace}={}){
  ...
  let lookup;
  try{lookup=await findInvite(email);traceStage(trace,"BETA_INVITE",started,lookup?.error?"degraded":"ok",lookup?.error||null)}
  ...
  if(allowed&&row?.status==="pending"){
    ...
    try{const accepted=await markAccepted(row,user);traceStage(trace,"BETA_ACCEPT",acceptStarted,accepted?.error?"degraded":"ok",accepted?.error||null)}
