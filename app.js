...
const STORE_KEY=PUBLISHER_DEMO_MODE?"warboost_publisher_demo_state_v2521":"warboost_v1_core_state", CLIENT_KEY=PUBLISHER_DEMO_MODE?"warboost_publisher_demo_client_v2521":"warboost_v1_client_id", LANG_KEY="warboost_v12_language";
const BACKUP_KEY=PUBLISHER_DEMO_MODE?"warboost_publisher_demo_last_good_v2521":"warboost_last_good_state", ACCOUNT_STATE_PREFIX=PUBLISHER_DEMO_MODE?"warboost_publisher_demo_account:":"warboost_account_state:", VOICE_ENABLED_KEY="warboost_voice_enabled", VOICE_ID_KEY="warboost_voice_id";
...
function betaAccessMessage(){if(PUBLISHER_DEMO_MODE)return "Publisher sandbox · anonymized sample data · no account required";if(!cloudSession?.user)return t("beta_signin_required");if(betaState.enforced&&!betaState.allowed)return t("beta_invite_required");if(!betaState.enforced)return t("beta_allowlist_setup");return t("beta_invited")}
...
function renderAuth(){
  if(PUBLISHER_DEMO_MODE){$("#authLoggedOut")?.classList.add("hidden");$("#authLoggedIn")?.classList.add("hidden");$("#publisherSandboxAuth")?.classList.remove("hidden");if($("#authPill"))$("#authPill").textContent="Publisher sandbox";renderBeta();renderPro();return}
...
function render(){
  ...
  const shareInvite=$("#shareInviteBtn"),cloudAlliance=Boolean(a.id||a.invite_code),canShareAllianceInvite=["R4","R5"].includes(normalizedRole(a.role||p.role));if(shareInvite)shareInvite.disabled=cloudAlliance&&!canShareAllianceInvite;
  $("#rosterFresh").textContent=a.updated_at?updatedLabel(a.updated_at):t("sync_needed");renderMembers();
  $("#vsWeekTitle").textContent=t("vs_week",{week:currentVsWeek()});$("#vsDayPill").textContent=currentVsDay()===0?t("vs_prep_day"):t("day_n",{day:currentVsDay()});$("#vsUs").textContent=v.our_alliance||a.tag||t("your_alliance");$("#vsThem").textContent=v.opponent||t("unknown_opponent");$("#vsUsScore").textContent=v.our_score??"—";$("#vsThemScore").textContent=v.their_score??"—";renderVsTimeline();
