...
async function pushServerState({keepalive=false}={}){if(!cloudSession?.access_token||!betaAccessAllowed()||!betaConsentAccepted())return {skipped:true};try{const r=await fetch("/api/state",{method:"POST",headers:authHeaders({"content-type":"application/json"}),body:JSON.stringify({state}),keepalive:Boolean(keepalive)}),j=await r.json().catch(()=>({}));if(!r.ok){markCloudPending(j?.error||`state_http_${r.status}`);return {ok:false,error:j?.error||r.status}}cloudDirty=false;clearTimeout(cloudRetryTimer);cloudRetryTimer=null;state.sync={...state.sync,status:"ok",last_sync:j?.updated_at||new Date().toISOString(),last_error:null,pending_cloud_save:false};safeLocalSet(STORE_KEY,JSON.stringify(state));rememberAccountState(cloudSession.user.id,state);renderProvider();return {ok:true}}catch(e){markCloudPending(e?.name||"offline");return {ok:false,error:e?.name||"offline"}}}
async function pullServerState(loginSeed=null){if(!cloudSession?.access_token)return {skipped:true};try{const r=await fetch("/api/state",{cache:"no-store",headers:authHeaders()}),j=await r.json().catch(()=>({}));if(!r.ok){if(j?.error==="database_schema_missing"){state.sync.last_error=t("cloud_schema_missing");state.sync.status="offline";renderProvider()}return {ok:false,error:j?.error||"state_error"}}if(!j?.state)return {ok:true,cloud_empty:true};const localBefore=hasMeaningfulCore(loginSeed)?loginSeed:safeClone(state),localTs=Date.parse(state?.updated_at||"")||0,cloudTs=Date.parse(j.updated_at||j.state?.updated_at||"")||0,preferLocal=Boolean(localTs&&cloudTs&&localTs>cloudTs);suppressPush=true;let merged=mergeStateProtected(state,j.state,{preferBase:preferLocal});if(hasMeaningfulCore(localBefore)&&!hasMeaningfulCore(merged))merged=mergeStateProtected(merged,localBefore,{preferBase:false});const localRecovered=recoverLocalHeroHistory(merged);state=repairLegacySquadIdentity(localRecovered.state).state;state.player_id=cloudSession.user.id;state.updated_at=preferLocal?(state.updated_at||new Date().toISOString()):(j.state?.updated_at||j.updated_at||state.updated_at);safeLocalSet(STORE_KEY,JSON.stringify(state));rememberLastGoodState(state,"cloud-pull");rememberAccountState(cloudSession.user.id,state);render();suppressPush=false;return {ok:true,cloud_empty:false}}catch{return {ok:false,error:"offline"}}}
