...
const RELEASE_LABEL="HF8.6.18"; // Fast Login Restore · lightweight authenticated profile restore before heavy history/roster reconciliation · includes HF8.6.17
...
async function ensureAuthenticatedSessionApplied(data){
  ...
  if(betaConsentAccepted()&&!cloudProfileVerified){
    ...
    let recovered=await pullServerState(seed,{fastRestore:true});
...
async function pushServerState({keepalive=false}={}){
  ...
  try{
    ...
    const r=keepalive?await fetch("/api/state",requestInit):await fetchSessionCritical("/api/state",requestInit,12000),j=await r.json().catch(()=>({}));
...
async function pullServerState(loginSeed=null,{fastRestore=false}={}){
  ...
  try{
    const stateUrl=fastRestore?"/api/state?restore=1":"/api/state",stateTimeout=fastRestore?6500:12000;
    const r=await fetchSessionCritical(stateUrl,{cache:"no-store",headers:authHeaders()},stateTimeout),j=await r.json().catch(()=>({}));
    if(!r.ok){
