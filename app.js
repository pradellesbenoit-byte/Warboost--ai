...
import {parseRosterImport,mergeRosterMembers,rosterNameKey} from "./lib/roster-import.js";
...
function initialState(){return {version:APP_VERSION,player_id:clientId(),updated_at:null,player:{name:"",server_id:"",hq_level:null,power_m:null,coordinates:null,role:"R1"},player_context:{objective:"auto",account_age_days:null,server_profile:"auto",updated_at:null},activity_events:[],exclusive_weapons:[],hero_progression:[],hero_profiles:[],drone:{level:null,power_m:null,updated_at:null},shop:{store_type:"",currency:"",currency_balance:null,vip_level:null,vip_days_remaining:null,offers:[],snapshots:[],updated_at:null},squads:[1,2,3,4].map(emptySquad),alliance:{id:null,tag:"",name:"",role:"R1",management_verified:false,invite_code:"",members:[],unlinked_accounts:[],identity_link_status:"unknown",updated_at:null},vs:{week:null,day:null,our_alliance:"",opponent:"",our_score:null,their_score:null,updated_at:null},season:{name:"",number:null,day:null,total_days:null,profession:"",progress_pct:null,resistance:null,focus:null,lifecycle:"unknown",lifecycle_source:null,ended_at:null,measured_hybrid_synergy:false,awakening_swap:null,updated_at:null},technology:{type_mastery_pct:null,hero_tech_pct:null,siege_to_seize_pct:null,defensive_fortification_pct:null,tactical_weapon_pct:null,updated_at:null},sync:{provider:"warboost-local",provider_kind:"local",access_status:"pending",capabilities:[],status:"local",last_sync:null,last_error:null,auto_ready:true,last_scan:null,official_last_sync:null,public_last_sync:null,sources:{official:false,public:false,scan:false,alliance:false}}}}
...
function reconcileCurrentPlayerAllianceIdentity({touch=false}={}){
  ...
  state.alliance={...state.alliance,members:result.members,identity_link_status:result.status};
  return result;
}
...
function render(){
  ...
  renderPlayerOnboarding();renderSquads();renderExclusiveWeapons();renderPlayerActivity();
  $("#aTag").textContent=a.tag||"—";$("#aCount").textContent=String(a.members?.length||0);$("#aRole").textContent=p.role||"R1";$("#inviteCode").textContent=state.alliance.invite_code||"—";const inviteNote=$("#inviteNote");if(inviteNote)inviteNote.textContent=t("invite_note",{alliance:a.tag||t("alliance")});
  const shareInvite=$("#shareInviteBtn"),cloudAlliance=Boolean(a.id||a.invite_code),canShareAllianceInvite=a.management_verified===true&&["R4","R5"].includes(normalizedRole(a.role));if(shareInvite)shareInvite.disabled=cloudAlliance&&!canShareAllianceInvite;
...
function renderAllianceIdentityLinks(members){
  ...
  const summary=rosterLinkSummary(members),pending=Array.isArray(state.alliance?.unlinked_accounts)?state.alliance.unlinked_accounts:[];
  if(box){
    ...
    const currentText=currentLinked?t("identity_current_linked"):(["no_match","ambiguous","missing_nickname","missing_server","missing_alliance","context_conflict","ambiguous_rename"].includes(currentStatus)?t("identity_current_pending"):"");
  ...
  if(pendingDetails)pendingDetails.classList.toggle("hidden",!pending.length);
  if(pendingBox)pendingBox.innerHTML=pending.length?pending.map(x=>`<div class="unlinkedAccountRow"><div><b>⚪ ${esc(x.name||t("player"))}</b><small>${esc(t("server"))} ${esc(x.server_id||"—")} · ${esc(t("alliance"))} ${esc(x.alliance_tag||state.alliance?.tag||"—")}</small></div><span class="pill">${esc(t("identity_unlinked_short"))}</span></div>`).join(""):"";
}
function renderAllianceActivity(){
  ...
  const eventBox=$("#allianceEventSummary");if(eventBox){
    ...
    eventBox.innerHTML=PLAYER_ACTIVITY_EVENT_TYPES.map(type=>{const x=totals[type];return `<span class="eventCountChip eventStatusChip"><b>${esc(activityEventLabel(type))}</b><small>✅ ${x.participated} · ❌ ${x.absent_confirmed} · 🔵 ${x.not_selected} · 🟠 ${x.excused}</small></span>`}).join("")
  }
...
$("#warPlanBtn").addEventListener("click",async()=>{if(!requirePro())return;if(!hasDeclaredAllianceCommandRole()){$("#warPlanText").textContent=managerOnlyMessage();return}const j=await requestAdvice("alliance");$("#warPlanText").textContent=structuredAdviceText("alliance",j);renderAllianceStructured(j)});$("#vsPlanBtn").addEventListener("click",async()=>{if(!requirePro())return;const j=await requestAdvice("vs");$("#vsPlanText").textContent=structuredAdviceText("vs",j)});$("#seasonLifecycleSelect")?.addEventListener("change",()=>{const value=$("#seasonLifecycleSelect").value||"unknown",now=new Date().toISOString();state.season=repairSeasonState({...state.season,lifecycle:value,lifecycle_source:"manual",ended_at:(value==="ended"||value==="interseason")?(state.season.ended_at||now):null,updated_at:now});saveState();$("#seasonAdviceText").textContent=t("season_empty")});
...
$("#rosterImportBtn")?.addEventListener("click",()=>{const status=$("#rosterImportStatus");if(!hasDeclaredAllianceCommandRole()){if(status){status.className="notice warn";status.textContent=managerOnlyMessage()}return}const imported=parseRosterImport($("#rosterImportText")?.value||"").map(row=>({...row,server_id:state.player?.server_id||row.server_id||"",alliance_tag:state.alliance?.tag||row.alliance_tag||""}));if(!imported.length){if(status){status.className="notice warn";status.textContent=t("import_error")}return}const before=(state.alliance.members||[]).length;state.alliance.members=mergeRosterMembers(state.alliance.members,imported);reconcileCurrentPlayerAllianceIdentity({touch:true});state.alliance.updated_at=new Date().toISOString();state.sync.sources={...state.sync.sources,alliance:true};saveState();const added=Math.max(0,state.alliance.members.length-before);if(status){status.className="notice";status.textContent=`${imported.length} ${t("import_done")} · +${added}`};if($("#rosterImportText"))$("#rosterImportText").value=""});
...
async function openProAction(){proMessage(t("beta_payment_disabled"),true)}
