...
import {normalizeAvailabilityRecord,mergeAvailabilityRecords,countAvailabilitySlots,recommendBestSlot} from "./lib/alliance-availability-planner.js";
...
function ensureCanyonState(){
  ...
  const generic=(alliance.event_availability||[]).filter(row=>row?.event_type==="canyon_storm").map(row=>({...row,member_key:row.member_key||row.canonical_member_key||row.lifecycle_key,canonical_member_key:row.canonical_member_key||null}));
