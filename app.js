...
function loadState(){try{const raw=localStorage.getItem(STORE_KEY);const parsed=raw?JSON.parse(raw):null;if(parsed&&hasMeaningfulCore(parsed))rememberLastGoodState(parsed,"pre-v2.5.15-load");const base=parsed?mergeState(initialState(),parsed):initialState();const migrated=migrateLegacyLocalState(base),repaired=repairLegacySquadIdentity(migrated.state),recovered=recoverLocalHeroHistory(repaired.state),finalRepair=repairLegacySquadIdentity(recovered.state);let next=finalRepair.state;const backup=readLastGoodState();if(!hasMeaningfulCore(next)&&hasMeaningfulCore(backup))next=mergeStateProtected(next,backup,{preferBase:false});next.version=APP_VERSION;if(migrated.changed||repaired.changed||recovered.changed||finalRepair.changed||!raw)localStorage.setItem(STORE_KEY,JSON.stringify(next));rememberLastGoodState(next,"post-v2.5.15-load");return next}catch{const backup=readLastGoodState();return hasMeaningfulCore(backup)?mergeState(initialState(),backup):initialState()}}
...
function render(){
  ...
  // V2.5.15 privacy boundary: saved local/cloud data is preserved in state but is never rendered
...
function renderProPriority(analysis){
  ...
  if(metaSources){const mi=analysis.meta_intelligence||{},rows=Array.isArray(mi.evidence)?mi.evidence:[];const kindLabel=k=>t(k==="official"?"meta_source_official":k==="guide"?"meta_source_guide":"meta_source_community");metaSources.innerHTML=`<div class="metaHeader"><b>${esc(t("meta_updated"))}: ${esc(mi.knowledge_date||"—")}</b><small>${esc(t("meta_source_count"))}: ${esc(mi.source_count??rows.length)} · ${esc(t("context_confidence"))}: ${esc(mi.confidence??0)}%</small><small>${esc(t("meta_secondary_policy"))}</small></div>${rows.map(x=>`<div class="metaSourceRow"><span>${esc(kindLabel(x.kind))}</span><div><b>${esc(x.title||x.topic||t("meta_source_community"))}</b><small>${esc(x.date||"—")} · ${esc(x.topic||"")}</small></div></div>`).join("")}` }
  ...
  if(shopList){const rows=(Array.isArray(shop.recommendations)?shop.recommendations:[]).slice(0,3);shopList.innerHTML=rows.length?rows.map(x=>`<article class="shopCard ${esc(x.verdict_key||"")}"><div class="shopHead"><span class="shopRank">${esc(x.rank||"•")}</span><div><b>${esc(native?(x.item||""):structuredShopItemLabel(x))}</b><small>${native?`${esc(x.store||"")}${x.price_label?` · ${esc(x.price_label)}`:""}`:(["scan","official"].includes(String(x.source||""))&&x.price_label?esc(x.price_label):"")}</small>${x.relevance_score!=null||x.score!=null?`<div class="shopMetrics"><span class="shopScore">${esc(t("shop_relevance"))} ${esc(String(Math.round(Number(x.relevance_score??x.score))))}/100</span>${x.evidence_confidence!=null?`<span class="shopEvidence">${esc(t("shop_data_confidence"))} ${esc(String(Math.round(Number(x.evidence_confidence))))}%</span>`:""}</div>`:""}</div><span class="shopVerdict">${esc(native?(x.verdict||""):shopVerdictLabel(x))}</span></div><details class="decisionDetails"><summary>${esc(native?ui.details:"ℹ️")}</summary><p>${esc(native?(x.reason||""):t("condition_neutral"))}</p><span class="shopAvailability">${esc(t("shop_availability"))}: ${esc(x.availability_status==="official_current"?t("shop_availability_official"):x.availability_status==="observed_scan"?t("shop_availability_observed"):t("shop_availability_unverified"))}</span>${native&&x.target?`<strong>${esc(x.target)}</strong>`:""}</details></article>`).join(""):`<div class="notice">${esc(t("shop_no_recommendations"))}</div>`}
...
async function refreshPro(){if(!cloudSession?.access_token){proState={active:false,status:"free",configured:false,plan:null,beta:true};renderPro();return}try{const r=await fetch("/api/pro",{cache:"no-store",headers:authHeaders()});const j=await r.json().catch(()=>({}));if(r.ok)proState={active:Boolean(j.active),status:j.status||"free",configured:Boolean(j.configured),plan:j.plan||null,beta:Boolean(j.beta),payments_enabled:Boolean(j.payments_enabled)};else proState={active:false,status:"free",configured:false,plan:null,beta:true}}catch{proState={active:false,status:"free",configured:false,plan:null,beta:true}}renderPro()}
function requirePro(){if(!requireBetaAccess()||!requireBetaConsent())return false;if(proState.active)return true;openDrawer("account");setTimeout(()=>$("#proSection")?.scrollIntoView({behavior:"smooth",block:"center"}),160);proMessage(cloudSession?.user?t("beta_invite_required"):t("connect_pro"));return false}
