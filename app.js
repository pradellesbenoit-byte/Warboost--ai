...
function renderDesertStormPicker(){
  ...
  const members=activeMembers.map(m=>({...m,_key:desertStormMemberKeys(m)[0]})).filter(m=>m._key),capacity=desertStormAvailabilityCapacity(activeMembers),selectionAccess=desertStormSelectionAccess(),selected=new Set(ds.registered_keys),q=rosterNameKey(desertStormSearchTerm);
...
function renderCanyonPlanner(){
  ...
  if(adjudicator){adjudicator.innerHTML=`<option value="">À confirmer</option>${available.map(m=>{const key=canyonMemberKeys(m)[0];return `<option value="${esc(key)}"${key===canyon.adjudicator_key?" selected":""}>${esc(m.name)}</option>`}).join("")}`;adjudicator.value=canyon.adjudicator_key||""}
   const summary=$("#canyonSummary"),participantCount=Math.min(available.length,CANYON_STORM_RULESET.max_starters),substituteCount=Math.min(Math.max(0,available.length-CANYON_STORM_RULESET.max_starters),CANYON_STORM_RULESET.max_substitutes),best=recommendBestSlot(canyon.availability,{event_type:"canyon_storm"}),priority=canyon.status==="completed"?"Terminé":canyon.status==="battle"?"Laboratoire / objectifs":"Disponibilités";
