...
$("#collapseAllSquadsBtn")?.addEventListener("click",e=>{e.preventDefault();squadAccordionOpenIds.clear();document.querySelectorAll("#squadList .squadToggle").forEach(input=>{input.checked=false;input.setAttribute("aria-expanded","false");input.closest(".squad")?.classList.add("closed");input.closest(".squad")?.classList.remove("open")})});
