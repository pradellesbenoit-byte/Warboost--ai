...
function togglePlayerActivityEvent(type){
  ...
  const ownId=String(state.player_id||""),ownName=String(state.player?.name||"").trim().toLowerCase(),self=(state.alliance.members||[]).find(m=>(m?.player_id&&String(m.player_id)===ownId)||(!m?.player_id&&ownName&&String(m?.name||"").trim().toLowerCase()===ownName));
  if(self)self.activity_events=mergeActivityEvents(self.activity_events,[event]);saveState();render();
}
...
function renderMemberRow(m){
  ...
  return `<div class="member compactMember"><div><b>${icon} ${esc(m.name||t("player"))}</b><small>${t("hq")} ${m.hq_level??"—"} · ${fmtPower(m.power_m)} · ${role}</small><span class="activityLine">${esc(label)} · ${esc(reason)}</span><span class="activityLine participationLine">${esc(participationLine)}</span></div><div class="memberRight">${deltaText?`<div class="delta">${esc(deltaText)}</div>`:""}${roleControl}</div></div>`;
}
