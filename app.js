...
function mergeStateProtected(base,incoming,{preferBase=false}={}){
  ...
  out.alliance=safeFields(base.alliance,incoming.alliance,preferBase);out.alliance.desert_storm=mergeDesertStormState(base.alliance?.desert_storm,incoming.alliance?.desert_storm);out.alliance.canyon=mergeCanyonState(base.alliance?.canyon,incoming.alliance?.canyon);out.alliance.members=mergeAllianceMembersProtected(base.alliance?.members,incoming.alliance?.members,preferBase);
...
// UI-only state: accordion visibility is never part of player data or local/cloud persistence.
let squadAccordionOpenIds=new Set();
...
function render(){
  ...
  safeRenderStep("ALLIANCE_MEMBERS",renderMembers);safeRenderStep("DESERT_STORM",renderDesertStormPlanner);safeRenderStep("CANYON",renderCanyonPlanner);
...
function renderSquads(){
  ...
  state.squads.forEach((sq,i)=>{
    ...
    const swapMenu=swapTargets.length?`<div class="squadSwapMenu hidden" data-squad-swap-menu="${id}"><span>${esc(t("squad_swap_with"))}</span>${swapTargets.map(x=>`<button type="button" class="squadSwapTarget" data-squad-swap-target="${x.id}">${esc(t("squad"))} ${x.id}</button>`).join("")}</div>`:"";
      const isOpen=squadAccordionOpenIds.has(id);
      shell.innerHTML=`<div class="squad ${isOpen?"open":"closed"}" data-squad-id="${id}"><button class="squadHead" type="button" data-squad-toggle="${id}" aria-controls="squadBody${id}" aria-expanded="${isOpen}"><span class="squadNo">${id}</span><span class="squadName"><b>${esc(name)}</b><small>${esc(freshness)}</small></span><span class="squadPower">${esc(fmtConfirmedSquadPower(sq))}</span><span class="chev" aria-hidden="true"></span></button><div id="squadBody${id}" class="squadBody"${isOpen?"":" hidden"}>${Array.from({length:5},(_,j)=>{const h=heroes[j]||emptyHero(j+1),hn=isGenericHeroName(h.name)?`${t("hero")} ${j+1} · ${t("hero_unconfirmed")}`:h.name;const detail=heroDetailLine(h,hn);return `<div class="heroRow"${!isGenericHeroName(h.name)?` data-hero="${esc(canonicalStoredHeroName(h.name))}`:""}><div class="heroAvatar">${j+1}</div><div class="heroInfo"><b>${esc(hn)}</b><small>${esc(detail.main)}</small>${detail.stats?`<span class="heroWeaponStats">${esc(detail.stats)}</span>`:""}</div><div class="heroPwr">${esc(fmtConfirmedHeroPower(heroPowerForDisplay(sq,h)))}</div></div>`}).join("")}${needsHeroConfirm?inlineHeroConfirmationHtml(sq,id):""}</div></div>${swapButton}${swapMenu}`;
    box.appendChild(shell);
     const toggle=shell.querySelector("[data-squad-toggle]"),squad=shell.querySelector(".squad"),body=shell.querySelector(".squadBody");
     toggle?.addEventListener("click",()=>{
       const open=!squad?.classList.contains("open");
       if(open)squadAccordionOpenIds.add(id);else squadAccordionOpenIds.delete(id);
       squad?.classList.toggle("open",open);squad?.classList.toggle("closed",!open);
       toggle?.setAttribute("aria-expanded",String(open));if(body)body.hidden=!open;
     });
...
function renderCanyonPlanner(){
  ...
  const summary=$("#canyonSummary"),plan=canyon.plan,best=recommendBestSlot(canyon.availability,{event_type:"canyon_storm"}),priority=canyon.status==="completed"?"Terminé":canyon.status==="battle"?"Laboratoire / objectifs":"Disponibilités";
...
$("#collapseAllSquadsBtn")?.addEventListener("click",e=>{e.preventDefault();squadAccordionOpenIds.clear();document.querySelectorAll("#squadList [data-squad-toggle]").forEach(toggle=>{toggle.setAttribute("aria-expanded","false");const squad=toggle.closest(".squad");squad?.classList.add("closed");squad?.classList.remove("open");squad?.querySelector(".squadBody")?.setAttribute("hidden","")})});
