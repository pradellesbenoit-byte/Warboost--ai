...
import {PUBLISHER_DEMO_MODE,buildPublisherDemoState,buildPublisherScanState} from "./lib/publisher-demo.js";

const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
const APP_VERSION="2.5.22";
const STORE_KEY=PUBLISHER_DEMO_MODE?"warboost_publisher_demo_state_v2522":"warboost_v1_core_state", CLIENT_KEY=PUBLISHER_DEMO_MODE?"warboost_publisher_demo_client_v2522":"warboost_v1_client_id", LANG_KEY="warboost_v12_language";
const BACKUP_KEY=PUBLISHER_DEMO_MODE?"warboost_publisher_demo_last_good_v2522":"warboost_last_good_state", ACCOUNT_STATE_PREFIX=PUBLISHER_DEMO_MODE?"warboost_publisher_demo_account:":"warboost_account_state:", VOICE_ENABLED_KEY="warboost_voice_enabled", VOICE_ID_KEY="warboost_voice_id";
...
let state=PUBLISHER_DEMO_MODE?buildPublisherDemoState():loadState(),serverNow=new Date(),pushTimer=null,suppressPush=false,cloud=null,cloudSession=null,cloudInit={status:"starting",configured:false,transport:"direct-supabase-auth-api",error:null},proState={active:false,status:"free",configured:false,plan:null,beta:false},betaState={release:true,enforced:false,configured:false,allowed:false,access_status:"sign-in-required",consent_version:BETA_CONSENT_VERSION,payments_enabled:false,pro_included:true},scanImageData=null;
...
function betaConsentAccepted(){if(PUBLISHER_DEMO_MODE)return true;const key=betaConsentStorageKey();return Boolean(key&&localStorage.getItem(key)==="1")}
...
function betaAccessAllowed(){if(PUBLISHER_DEMO_MODE)return true;return Boolean(cloudSession?.user)&&(!betaState.enforced||betaState.allowed)}
function betaPrivateDataVisible(){if(PUBLISHER_DEMO_MODE)return true;return Boolean(cloudSession?.user)&&betaAccessAllowed()&&betaConsentAccepted()}
function betaAccessMessage(){if(PUBLISHER_DEMO_MODE)return t("publisher_local_only_note");if(!cloudSession?.user)return t("beta_signin_required");if(betaState.enforced&&!betaState.allowed)return t("beta_invite_required");if(!betaState.enforced)return t("beta_allowlist_setup");return t("beta_invited")}
function requireBetaAccess(){if(PUBLISHER_DEMO_MODE)return true;if(betaAccessAllowed())return true;openDrawer("account");setTimeout(()=>$("#betaAccessSection")?.scrollIntoView({behavior:"smooth",block:"center"}),120);return false}
function requireBetaConsent(){if(PUBLISHER_DEMO_MODE)return true;if(betaConsentAccepted())return true;openDrawer("account");setTimeout(()=>$("#betaAccessSection")?.scrollIntoView({behavior:"smooth",block:"center"}),120);const el=$("#betaAccessStatus");if(el){el.className="notice warn";el.textContent=t("beta_consent_required")}return false}
...
function updatedLabel(iso){if(PUBLISHER_DEMO_MODE)return t("publisher_demo_data");if(!iso)return t("not_synced");const d=Math.max(0,Date.now()-new Date(iso).getTime());return d<60e3?t("updated_now"):t("updated_ago",{ago:fmtAgo(iso)})}
...
function applyPublisherCopy(){if(!PUBLISHER_DEMO_MODE)return;
  const demo=t("publisher_demo_data"),set=(sel,text)=>{const el=$(sel);if(el)el.textContent=text};
  set("#betaHeaderBadge","PUBLISHER FINAL");
  set("#scanPlayerBtn",`📷 ${t("scan_title")} · ${demo}`);set("#syncPlayerBtn",`🌐 ${t("refresh")} · ${demo}`);
  set("#playerSyncInfo",t("publisher_local_only_note"));
  set("#shareInviteBtn",`${t("share")} · ${demo}`);set("#inviteNote",t("publisher_invite_note"));
  const wp=$("#warPlanBtn");if(wp)wp.innerHTML=`💎 <span>${esc(t("refresh"))} · ${esc(demo)}</span>`;
  set("#rosterImportHelp",t("publisher_import_note"));set("#rosterImportBtn",`${t("import_roster")} · ${demo}`);set("#activityDemoPill",`${t("ai_estimate")} · ${demo}`);
  const vp=$("#vsPlanBtn");if(vp)vp.innerHTML=`💎 <span>${esc(t("refresh"))} · ${esc(demo)}</span>`;set("#vsSampleNote",demo);
  const sp=$("#seasonAdviceBtn");if(sp)sp.innerHTML=`💎 <span>${esc(t("refresh"))} · ${esc(demo)}</span>`;
  set("#scanDrawerTitle",`${t("scan_title")} · ${demo}`);set("#scanIntro",t("publisher_scan_privacy"));set("#scanPrivacy",t("publisher_scan_privacy"));
  set("#openScanBtn",`${t("scan_title")} · ${demo}`);set("#syncAllBtn",`${t("refresh")} · ${demo}`);set("#publisherProviderNote",t("publisher_local_only_note"));set("#publisherResetBtn","↺ Publisher Demo");
} 
function applyLanguage(){lang=resolveLanguage(languageChoice);locale=localeFor(lang);t=translator(lang);document.documentElement.lang=lang;document.documentElement.dir=dirFor(lang);$$('[data-i18n]').forEach(el=>{el.textContent=t(el.dataset.i18n)});$$('[data-i18n-aria]').forEach(el=>el.setAttribute('aria-label',t(el.dataset.i18nAria)));$$('[data-i18n-placeholder]').forEach(el=>el.setAttribute('placeholder',t(el.dataset.i18nPlaceholder)));const sel=$("#languageSelect");if(sel){sel.innerHTML=LANGUAGES.map(([v,label])=>`<option value="${v}">${label}</option>`).join("");sel.value=languageChoice}renderScanTypeOptions();applyPublisherCopy();renderClock();render();renderAuth();renderBeta();renderPro();renderVoiceSettings();applyPublisherCopy();$("#proPriorityPanel")?.classList.add("hidden");$("#playerSyncInfo")?.classList.remove("hidden")}
function saveState(){state=repairLegacySquadIdentity(state).state;state.updated_at=new Date().toISOString();state.version=APP_VERSION;localStorage.setItem(STORE_KEY,JSON.stringify(state));rememberLastGoodState(state,"save");if(cloudSession?.user?.id&&String(state.player_id||"")===String(cloudSession.user.id))rememberAccountState(cloudSession.user.id,state);render();if(!PUBLISHER_DEMO_MODE&&!suppressPush)scheduleServerSave()}
function scheduleServerSave(){clearTimeout(pushTimer);pushTimer=setTimeout(pushServerState,700)}

async function initCloudAuth(){
  if(PUBLISHER_DEMO_MODE){cloud=null;cloudSession=null;cloudInit={status:"publisher-demo",configured:false,transport:"publisher-sandbox",error:null};proState={active:true,status:"publisher-demo",configured:false,plan:null,beta:false,payments_enabled:false};betaState={release:false,enforced:false,configured:false,allowed:true,access_status:"publisher-demo",consent_version:BETA_CONSENT_VERSION,payments_enabled:false,pro_included:true};renderAuth();renderBeta();renderPro();return}
...
async function applySession(session){
  ...
  if(cloudSession?.user?.id){
    ...
    proState={active:false,status:"free",configured:false,plan:null,beta:true};
...
function renderAuth(){
  if(PUBLISHER_DEMO_MODE){$("#authLoggedOut")?.classList.add("hidden");$("#authLoggedIn")?.classList.add("hidden");$("#publisherSandboxAuth")?.classList.remove("hidden");if($("#authPill"))$("#authPill").textContent=`${t("local")} · ${t("publisher_demo_data")}`;renderBeta();renderPro();return}
...
function render(){
  ...
  $("#vsMeta").textContent=`${t("week")} ${currentVsWeek()}`;
  $("#pName").textContent=p.name||"—";$("#pHq").textContent=p.hq_level?`${t("hq")} ${p.hq_level}`:"—";$("#pPower").textContent=fmtPower(p.power_m);$("#pDrone").textContent=d.level?`${t("level")}${d.level}${d.power_m?` · ${fmtPower(d.power_m)}`:""}`:"—";
  renderSquads();renderExclusiveWeapons();
  $("#aTag").textContent=a.tag||"—";$("#aCount").textContent=String(a.members?.length||0);$("#aRole").textContent=a.role||p.role||"R1";$("#inviteCode").textContent=state.alliance.invite_code||"—";const inviteNote=$("#inviteNote");if(inviteNote)inviteNote.textContent=t("invite_note",{alliance:a.tag||t("alliance")});
  const shareInvite=$("#shareInviteBtn"),cloudAlliance=Boolean(a.id||a.invite_code),canShareAllianceInvite=["R4","R5"].includes(normalizedRole(a.role||p.role));if(shareInvite)shareInvite.disabled=PUBLISHER_DEMO_MODE?false:(cloudAlliance&&!canShareAllianceInvite);
  $("#rosterFresh").textContent=PUBLISHER_DEMO_MODE?t("publisher_demo_data"):(a.updated_at?updatedLabel(a.updated_at):t("sync_needed"));renderMembers();
  $("#vsWeekTitle").textContent=t("vs_week",{week:currentVsWeek()});$("#vsDayPill").textContent=(currentVsDay()===0?t("vs_prep_day"):t("day_n",{day:currentVsDay()}))+(PUBLISHER_DEMO_MODE?` · ${t("publisher_demo_data")}`:"");$("#vsUs").textContent=v.our_alliance||a.tag||t("your_alliance");$("#vsThem").textContent=v.opponent||t("unknown_opponent");$("#vsUsScore").textContent=v.our_score??"—";$("#vsThemScore").textContent=v.their_score??"—";renderVsTimeline();

  const seasonLife=seasonLifecycle(s),seasonActive=seasonIsActive(s),pct=activeSeasonProgress(s),baseSeasonName=PUBLISHER_DEMO_MODE?t("season"):s.name||(s.number?`S${s.number}`:"—"),seasonHistorical=(seasonLife==="ended"||seasonLife==="interseason"),professionValue=PUBLISHER_DEMO_MODE&&String(s.profession||"").toLowerCase()==="engineer"?t("publisher_profession_engineer"):(s.profession||"—");
  ...
  $("#sName").textContent=seasonHistorical?`${baseSeasonName} · ${t("season_ended_short")}`:(PUBLISHER_DEMO_MODE?`${baseSeasonName} · ${t("publisher_demo_data")}`:baseSeasonName);$("#sDay").textContent=seasonActive?(s.day||"—"):"—";$("#sProfession").textContent=professionValue;
  ...
  const lifeStatus=PUBLISHER_DEMO_MODE?`${t("publisher_demo_data")} · ${t("season")}: ${s.day||"—"}/${s.total_days||"—"} · ${s.resistance??"—"}`:seasonLife==="interseason"?t("season_interseason_note",{name:baseSeasonName,profession:professionValue}):seasonLife==="ended"?t("season_ended_note",{name:baseSeasonName,profession:professionValue}):seasonLife==="unknown"?t("season_unknown_note"):s.updated_at?`${t("last_update",{ago:fmtAgo(s.updated_at)})} · ${s.resistance??"—"}`:t("season_wait");$("#seasonStatus").textContent=lifeStatus;
  renderAdvice();renderAccountFields();renderProvider();
}
...
function planModeLabel(row){const mode=String(row?.mode||"focus"),action=String(row?.action_key||"");if(action==="secondary_priority")return t("plan7_secondary");if(mode==="checkpoint")return t("plan7_checkpoint");if(mode==="scan")return PUBLISHER_DEMO_MODE?`${t("refresh")} · ${t("publisher_demo_data")}`:t("plan7_scan");if(mode==="hold")return t("plan7_hold");if(mode==="shop")return t("plan7_shop_resources");if(mode==="timing")return t("plan7_timing");if(mode==="review")return t("plan7_review");return t("plan7_focus")}
...
function renderProPriority(analysis){
  ...
  if(shopSummary)shopSummary.textContent=PUBLISHER_DEMO_MODE?t("publisher_shop_note"):(native?(shop.summary||t("shop_summary_default")):t("shop_summary_default"));
  ...
  if(shopCatalogPill){shopCatalogPill.textContent=PUBLISHER_DEMO_MODE?t("publisher_demo_data"):(native?(shop.catalog_label||"—"):t("adaptive"));shopCatalogPill.className=`pill ${shop.catalog_status==="official"?"catalogOfficialPill":"catalogPartialPill"}`}
...
function renderAllianceActivity(){
  ...
  const note=$("#activityNote");if(note)note.textContent=PUBLISHER_DEMO_MODE?`${t("publisher_demo_data")} · ${t("activity_reliability_note")}`:(members.length?t("activity_reliability_note"):t("activity_no_data"));
  return summary;
}
function renderMemberRow(m){
  const a=classifyAllianceMember(m),icon=activityIcon(a),label=activityLabel(a),reason=activityReason(a),delta=Number(m?.delta_m),canManage=!PUBLISHER_DEMO_MODE&&normalizedRole(state?.alliance?.role)==="R5"&&Boolean(m?.player_id);
...
function renderMembers(){
  ...
  const groups=roles.map(role=>{
    ...
    return `<details class="roleRosterGroup" data-roster-role="${role}"${open}><summary><span class="roleRosterTitle"><b>${role}</b><small>${rows.length} ${esc(rows.length===1?t("publisher_member_singular"):t("members_short"))}</small></span><span class="roleRosterChevron" aria-hidden="true">⌄</span></summary><div class="roleRosterBody">${body}</div></details>`;
  }).join("");
...
function structuredAdviceText(scope,j){
  ...
  if(scope==="vs"){const d=Number(j.day),prep=d===0,key=Number.isInteger(d)&&d>=1&&d<=6?`vs_focus_${d}`:null,gap=Number(j.score_gap),focus=prep?t("vs_prep_focus"):key?t(key):t("plan7_hold"),head=prep?t("vs_prep_day"):(Number.isInteger(d)?`${t("vs")} · #${d}`:t("vs")),hold=prep?t("vs_prep_hold_rule"):t("vs_hold_rule");return `${head}\n${t("plan7_focus")}: ${focus}${!prep&&Number.isFinite(gap)?`\nΔ ${Math.round(gap*100)/100}`:""}\n${hold}`}
  if(scope==="season"){if(j.lifecycle==="interseason"||j.lifecycle==="ended")return [j.lifecycle==="interseason"?t("season_interseason"):t("season_ended_short"),j.last_known_profession?`${t("season_last_profession")}: ${j.last_known_profession}`:null,t("season_wait_next"),t("season_no_old_advice")].filter(Boolean).join("\n");if(j.lifecycle==="unknown")return [t("season_unknown"),t("season_confirm_state")].join("\n");const parts=[t("season_structured_priority")];if(j.progress_pct!=null)parts.push(`${t("season_progress")}: ${j.progress_pct}%`);else parts.push(`${t("season_progress")}: ${t("season_unknown")}`);if(j.profession)parts.push(`${t("profession")}: ${j.profession}`);if(j.resistance!=null)parts.push(`🛡️ ${j.resistance}`,t("season_resistance_priority"));if(j.day&&j.total_days&&Number(j.day)/Number(j.total_days)>=.8)parts.push(t("season_late_priority"));return parts.join("\n")}
  return j.advice||"";
}
...
function renderProvider(){const s=state.sync||{},sources=s.sources||{},official=Boolean(sources.official),pill=$("#providerPill"),box=$("#providerStatus"),reveal=betaPrivateDataVisible();if(!pill||!box)return;if(PUBLISHER_DEMO_MODE){pill.textContent=t("official_pending");pill.className="pill";if($("#publicSourceState"))$("#publicSourceState").textContent=t("official_pending");if($("#scanSourceState"))$("#scanSourceState").textContent=`${t("ready")} · ${t("publisher_demo_data")}`;if($("#allianceCloudState"))$("#allianceCloudState").textContent=`${t("local")} · ${t("publisher_demo_data")}`;box.className="notice warn";box.textContent=t("publisher_local_only_note");if($("#openScanBtn"))$("#openScanBtn").disabled=false;if($("#syncAllBtn"))$("#syncAllBtn").disabled=false;return}if(!reveal){pill.textContent=t("official_pending");pill.className="pill";$("#publicSourceState").textContent=t("official_pending");$("#scanSourceState").textContent=t("ready");$("#allianceCloudState").textContent=t("not_connected");box.className="notice warn";box.textContent=t("beta_signin_required");if($("#openScanBtn"))$("#openScanBtn").disabled=true;if($("#syncAllBtn"))$("#syncAllBtn").disabled=true;return}if($("#openScanBtn"))$("#openScanBtn").disabled=false;if($("#syncAllBtn"))$("#syncAllBtn").disabled=false;pill.textContent=official?t("official_connected"):t("official_pending");pill.className=`pill ${official?"activePill":""}`;$("#publicSourceState").textContent=official?t("official_connected"):t("official_pending");$("#scanSourceState").textContent=(sources.scan||s.last_scan)?t("available"):t("ready");$("#allianceCloudState").textContent=(sources.alliance||cloudSession)?t("available"):t("not_connected");if(s.last_error&&s.status==="offline"){box.className="notice warn";box.textContent=s.last_error}else if(official){box.className="notice";box.textContent=s.official_last_sync?t("last_update",{ago:fmtAgo(s.official_last_sync)}):t("official_connected_note")}else{box.className="notice warn";box.textContent=t("hybrid_no_public")}}
...
$$('[data-open]').forEach(b=>b.addEventListener("click",()=>{if(!requireBetaAccess())return;openDrawer(b.dataset.open)}));$$('[data-close]').forEach(b=>b.addEventListener("click",closeDrawers));$("#backdrop").addEventListener("click",closeDrawers);$("#accountBtn").addEventListener("click",()=>openDrawer("account"));$("#adviceAction").addEventListener("click",()=>{if(state.player.name&&!requireBetaAccess())return;openDrawer(state.player.name?"player":"account")});$("#languageSelect").addEventListener("change",e=>{languageChoice=e.target.value;localStorage.setItem(LANG_KEY,languageChoice);applyLanguage()});
...
$("#analyzeScanBtn").addEventListener("click",async()=>{if(!scanImageData){$("#scanStatus").className="notice warn";$("#scanStatus").textContent=t("scan_wait");return}if(!requireBetaAccess()||!requireBetaConsent())return;if(!PUBLISHER_DEMO_MODE&&!cloudSession?.access_token){openDrawer("account");authMessage(t("connect_pro"));return}const btn=$("#analyzeScanBtn");btn.disabled=true;btn.textContent=t("scan_processing");$("#scanStatus").className="notice";$("#scanStatus").textContent=t("scan_processing");if(PUBLISHER_DEMO_MODE){const now=new Date().toISOString(),sample=buildPublisherScanState($("#scanType").value,now);state=repairLegacySquadIdentity(mergeState(state,sample)).state;state.sync.last_scan=now;state.sync.sources={...state.sync.sources,scan:true};saveState();$("#scanStatus").className="notice";$("#scanStatus").textContent=t("publisher_scan_done");btn.disabled=false;btn.textContent=t("analyze");return}try{const r=await fetch("/api/scan",{method:"POST",headers:authHeaders({"content-type":"application/json"}),body:JSON.stringify({scan_type:$("#scanType").value,locale:lang,image_data_url:scanImageData,current_state:state,publisher_demo:PUBLISHER_DEMO_MODE})});const j=await r.json().catch(()=>({}));if(r.ok&&j.state){if(PUBLISHER_DEMO_MODE){state=repairLegacySquadIdentity(mergeState(state,j.state)).state;state.sync.last_scan=j.scanned_at||new Date().toISOString();state.sync.sources={...state.sync.sources,scan:true};saveState();$("#scanStatus").className="notice";$("#scanStatus").textContent="Publisher scan simulation loaded. The uploaded image was not processed or sent to an external vision provider.";closeHeroConfirmation(true);return}const scanType=$("#scanType").value;const sm=String(scanType||"").match(/^squad([1-4])$/i);let suggestedNames=[],scanSlots=[];if(sm){const idx=Number(sm[1])-1,incomingSq=j.state.squads?.[idx];if(incomingSq?.heroes){scanSlots=Array.from({length:5},(_,i)=>({...((incomingSq.heroes?.[i]&&typeof incomingSq.heroes[i]==="object")?incomingSq.heroes[i]:{})}));suggestedNames=scanSlots.map(h=>String(h?.name||"").trim());delete incomingSq.heroes}}state=repairLegacySquadIdentity(mergeState(state,j.state)).state;if(sm){const staged=state.squads?.[Number(sm[1])-1];if(staged){staged.needs_rescan=true;staged.composition_changed_at=j.scanned_at||new Date().toISOString()}}state.sync.last_scan=j.scanned_at||new Date().toISOString();state.sync.sources={...state.sync.sources,scan:true};saveState();$("#proPriorityPanel")?.classList.add("hidden");$("#playerSyncInfo")?.classList.remove("hidden");if(sm){const count=suggestedNames.filter(Boolean).length;$("#scanStatus").className="notice warn";$("#scanStatus").textContent=count>0?t("hero_auto_recognized",{count}):t("hero_confirm_needed");startHeroConfirmation(Number(sm[1]),suggestedNames,scanSlots)}else{$("#scanStatus").className="notice";$("#scanStatus").textContent=PUBLISHER_DEMO_MODE?"Publisher scan simulation loaded. The uploaded image was not sent to an external vision provider; sample data only.":t("scan_saved");closeHeroConfirmation(true)}}else{$("#scanStatus").className="notice warn";$("#scanStatus").textContent=j.code==="SCAN_NOT_CONFIGURED"?t("scan_unconfigured"):(j.message||t("scan_error"))}}catch{$("#scanStatus").className="notice warn";$("#scanStatus").textContent=t("scan_error")}finally{btn.disabled=false;btn.textContent=t("analyze")}});

async function requestAdvice(scope){if(scope==="vs"){state.vs.week=currentVsWeek();state.vs.day=currentVsDay()}const adviceState=PUBLISHER_DEMO_MODE?safeClone(state):state;if(PUBLISHER_DEMO_MODE&&adviceState.season){adviceState.season.name=t("season");if(String(adviceState.season.profession||"").toLowerCase()==="engineer")adviceState.season.profession=t("publisher_profession_engineer")}try{const r=await fetch("/api/advice",{method:"POST",headers:authHeaders({"content-type":"application/json"}),body:JSON.stringify({scope,state:adviceState,locale:lang,publisher_demo:PUBLISHER_DEMO_MODE})});const j=await r.json().catch(()=>({}));if(r.ok)return j}catch{}return null}
...
$("#rosterImportBtn")?.addEventListener("click",()=>{const status=$("#rosterImportStatus");if(!isAllianceManager()){if(status){status.className="notice warn";status.textContent=managerOnlyMessage()}return}const imported=parseRosterImport($("#rosterImportText")?.value||"");if(!imported.length){if(status){status.className="notice warn";status.textContent=t("import_error")}return}const before=(state.alliance.members||[]).length;state.alliance.members=mergeRosterMembers(state.alliance.members,imported);state.alliance.updated_at=new Date().toISOString();state.sync.sources={...state.sync.sources,alliance:true};saveState();const added=Math.max(0,state.alliance.members.length-before);if(status){status.className="notice";status.textContent=PUBLISHER_DEMO_MODE?t("publisher_import_done"):`${imported.length} ${t("import_done")} · +${added}`};if($("#rosterImportText"))$("#rosterImportText").value=""});

$("#shareInviteBtn").addEventListener("click",async()=>{const btn=$("#shareInviteBtn");if(PUBLISHER_DEMO_MODE){inviteMessage(t("publisher_invite_status"),true);return}if(!requireBetaAccess()||!requireBetaConsent())return;if(!cloudSession?.access_token){inviteMessage(t("alliance_invite_connect"));openDrawer("account");return}const original=btn?.textContent;if(btn){btn.disabled=true;btn.textContent="…"}try{const rr=await fetch("/api/invite",{method:"POST",headers:authHeaders({"content-type":"application/json"}),body:JSON.stringify({tag:state.alliance.tag||"WB",name:state.alliance.name||state.alliance.tag||"WarBoost"})}),jj=await rr.json().catch(()=>({}));if(!rr.ok||!jj.invite_code)throw Object.assign(new Error(jj.message||jj.error||"invite_failed"),{code:jj.error||"invite_failed"});const code=jj.invite_code;state.alliance.id=jj.alliance?.id||state.alliance.id;state.alliance.tag=jj.alliance?.tag||state.alliance.tag;state.alliance.name=jj.alliance?.name||state.alliance.name;state.alliance.invite_code=code;state.alliance.role=jj.role||state.alliance.role;state.vs.our_alliance=state.alliance.tag;state.sync.sources={...state.sync.sources,alliance:true};saveState();inviteMessage(t("alliance_invite_ready"),true);const url=`${location.origin}${location.pathname}?join=${encodeURIComponent(code)}`,text=`WarBoost · ${code}`;try{if(navigator.share)await navigator.share({title:`WarBoost · ${t("alliance")}`,text,url});else{await navigator.clipboard.writeText(`${text}\n${url}`);if(btn){btn.textContent=t("copy");setTimeout(()=>{btn.textContent=t("share")},1400)}}}catch{}}catch(e){inviteMessage(t("alliance_invite_failed"))}finally{if(btn){btn.disabled=false;if(btn.textContent==="…")btn.textContent=original||t("share")}}});
async function joinPendingAlliance(){const code=String(state.alliance.invite_code||"").trim();if(!code||!state.player.name||!cloudSession?.access_token||!betaAccessAllowed()||!betaConsentAccepted())return false;try{const r=await fetch("/api/join",{method:"POST",headers:authHeaders({"content-type":"application/json"}),body:JSON.stringify({invite_code:code})}),j=await r.json().catch(()=>({}));if(!r.ok){inviteMessage(t(j?.error==="alliance_owner_switch_blocked"?"alliance_owner_switch_blocked":j?.error==="invite_not_found"?"alliance_invite_not_found":"alliance_join_failed"));return false}if(j.alliance){state.alliance.id=j.alliance.id||state.alliance.id;state.alliance.tag=j.alliance.tag||state.alliance.tag;state.alliance.name=j.alliance.name||state.alliance.name;state.alliance.role=j.membership?.role||state.alliance.role||"R1";state.vs.our_alliance=state.alliance.tag;state.sync.sources={...state.sync.sources,alliance:true};saveState();inviteMessage(t(j.already_member?"alliance_already_joined":"alliance_joined"),true);return true}}catch{inviteMessage(t("alliance_join_failed"))}return false}
function handleJoinLink(){if(PUBLISHER_DEMO_MODE)return;const code=new URLSearchParams(location.search).get("join");if(!code)return;state.alliance.invite_code=code.toUpperCase();localStorage.setItem(STORE_KEY,JSON.stringify(state));setTimeout(()=>openDrawer("account"),500)}
...
function renderPro(){if(PUBLISHER_DEMO_MODE){const pill=$("#proPill"),btn=$("#proActionBtn"),title=$("#proTitle"),price=$("#proPrice"),section=$("#proSection");section?.classList.add("betaIncluded");if(pill){pill.textContent="PRO";pill.classList.add("active")}if(title)title.textContent=`WarBoost PRO · Publisher Demo`;if(price)price.textContent=t("publisher_no_payment_note");if(btn){btn.textContent="Publisher Demo";btn.disabled=true}return}const pill=$("#proPill"),btn=$("#proActionBtn"),title=$("#proTitle"),price=$("#proPrice"),section=$("#proSection");if(!pill||!btn)return;const logged=Boolean(cloudSession?.user),beta=proState.beta!==false;section?.classList.toggle("betaIncluded",beta);if(beta){pill.textContent=proState.active?t("beta_pro_short"):t("beta_badge");pill.classList.toggle("active",proState.active);title.textContent=proState.active?t("beta_pro_included"):t("beta_pro_title");if(!logged){price.textContent=t("beta_pro_signin");btn.textContent=t("login");btn.disabled=false;return}if(betaState.enforced&&!betaState.allowed){price.textContent=t("beta_invite_required");btn.textContent=t("beta_invite_short");btn.disabled=true;return}price.textContent=t("beta_pro_free");btn.textContent=t("beta_pro_included");btn.disabled=true;return}pill.textContent=proState.active?"PRO":t("free");pill.classList.toggle("active",proState.active);title.textContent=proState.active?t("pro_active"):t("upgrade");if(!logged){price.textContent=t("connect_for_pro");btn.textContent=t("login");btn.disabled=false;return}if(!proState.configured){price.textContent=t("pro_config");btn.textContent=t("pro_soon");btn.disabled=true;return}const formatted=formatProPrice(proState.plan);price.textContent=proState.active?t("subscription_active",{price:formatted}):formatted;btn.textContent=proState.active?t("manage_subscription"):t("go_pro");btn.disabled=false}
async function refreshPro(){if(PUBLISHER_DEMO_MODE){proState={active:true,status:"publisher-demo",configured:false,plan:null,beta:false,payments_enabled:false};renderPro();return}if(!cloudSession?.access_token){proState={active:false,status:"free",configured:false,plan:null,beta:true};renderPro();return}try{const r=await fetch("/api/pro",{cache:"no-store",headers:authHeaders()});const j=await r.json().catch(()=>({}));if(r.ok)proState={active:Boolean(j.active),status:j.status||"free",configured:Boolean(j.configured),plan:j.plan||null,beta:Boolean(j.beta),payments_enabled:Boolean(j.payments_enabled)};else proState={active:false,status:"free",configured:false,plan:null,beta:true}}catch{proState={active:false,status:"free",configured:false,plan:null,beta:true}}renderPro()}
function requirePro(){if(PUBLISHER_DEMO_MODE)return true;if(!requireBetaAccess()||!requireBetaConsent())return false;if(proState.active)return true;openDrawer("account");setTimeout(()=>$("#proSection")?.scrollIntoView({behavior:"smooth",block:"center"}),160);proMessage(cloudSession?.user?t("beta_invite_required"):t("connect_pro"));return false}
async function openProAction(){if(PUBLISHER_DEMO_MODE){proMessage(t("publisher_no_payment_note"),true);return}if(!cloudSession?.access_token){proMessage(t("connect_for_pro"));$("#authEmail")?.focus();return}if(proState.beta!==false){proMessage(t("beta_payment_disabled"),true);return}const btn=$("#proActionBtn");if(!proState.configured){proMessage(t("pro_config"));return}btn.disabled=true;const original=btn.textContent;btn.textContent="…";try{const action=proState.active?"portal":"checkout",r=await fetch(`/api/pro?action=${action}`,{method:"POST",headers:authHeaders({"content-type":"application/json"}),body:JSON.stringify({action})}),j=await r.json().catch(()=>({}));if(!r.ok||!j.url)throw new Error(j.message||"Stripe");location.href=j.url}catch(e){proMessage(e.message||"Stripe");btn.disabled=false;btn.textContent=original}}
...
$("#publisherResetBtn")?.addEventListener("click",()=>{if(!PUBLISHER_DEMO_MODE)return;window.__WARBOOST_PUBLISHER_DEMO__?.reset?.();const st=$("#publisherResetStatus");if(st){st.className="notice";st.classList.remove("hidden");st.textContent=t("publisher_reset_done")}applyPublisherCopy()});
if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("/sw.js").catch(()=>{}));
window.__WARBOOST_PUBLISHER_DEMO__={enabled:PUBLISHER_DEMO_MODE,version:APP_VERSION,reset(){if(!PUBLISHER_DEMO_MODE)return;localStorage.removeItem(STORE_KEY);localStorage.removeItem(BACKUP_KEY);state=buildPublisherDemoState();localStorage.setItem(STORE_KEY,JSON.stringify(state));render();renderAuth();renderBeta();renderPro();applyPublisherCopy();}};
