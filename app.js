...
import {applyRosterImportLifecycle,confirmRosterDeparture,restoreRosterReviewMember,rosterLifecycleKey} from "./lib/alliance-roster-lifecycle.js";
...
function renderMemberRow(m){
  ...
  const linked=m?.warboost_linked===true,linkLine=`${linked?"🟢":"⚪"} ${t(linked?"identity_linked_short":"identity_unlinked_short")}`,historyLine=membershipHistoryLine(m,2);
...
function renderVsDecision(v,engine){
  ...
  if(use)use.textContent=engine.ended?t("vs_use_ended"):vsUseNowText(v);if(keep)keep.textContent=engine.ended?t("vs_keep_after_end"):t("vs_hold_rule");if(rescan)rescan.textContent=engine.ended?t("vs_no_rescan_ended"):(engine.rescan_minutes>0?t("vs_rescan_minutes",{minutes:engine.rescan_minutes}):t("vs_scan_now"));if(risk)risk.textContent=vsRiskText(engine);
}
...
$("#rosterImportBtn")?.addEventListener("click",()=>{
  ...
  const result=applyRosterImportLifecycle({members:state.alliance.members,review:state.alliance.roster_review,former:state.alliance.former_members},imported,{complete,now});
  state.alliance.members=result.members;state.alliance.roster_review=result.review;state.alliance.former_members=result.former;if(complete)state.alliance.roster_snapshot_complete_at=now;
  reconcileCurrentPlayerAllianceIdentity({touch:true});state.alliance.updated_at=now;state.sync.sources={...state.sync.sources,alliance:true};saveState();
  if(status){status.className="notice";status.textContent=complete?t("roster_import_complete_result",{active:result.summary.active_count,review:result.summary.review_count,added:result.summary.added,returned:result.summary.returned}):t("roster_import_partial_result",{count:result.summary.imported,added:result.summary.added,returned:result.summary.returned})}
