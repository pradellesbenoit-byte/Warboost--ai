...
function saveState(){state=repairLegacySquadIdentity(state).state;state.updated_at=new Date().toISOString();state.version=APP_VERSION;localStorage.setItem(STORE_KEY,JSON.stringify(state));render();if(!suppressPush)scheduleServerSave()}
...
function renderProPriority(analysis){
  ...
  if(summary){
    ...
    summary.textContent=`${analysis.summary||""}${composition}${analysis.candidates_evaluated?` · ${t("options_compared_count",{count:analysis.candidates_evaluated})}`:""}${mi?.source_count?` · ${ui.sources} ${mi.source_count} · ${ui.meta} ${mi.confidence}%`:""}`;
  }
  ...
  if(list){
    ...
    list.innerHTML=items.length?items.map((x,i)=>{const hero=proHeroAttr(x),heroVisual=proHeroVisual(hero,i===0),progress=proProgressLabel(x),marginal=Number(x.marginal_value_score),certainty=adaptiveCertaintyLabel(x.certainty),condition=adaptiveConditionLabel(x.condition_key),calculated=adaptiveDateLabel(x.calculated_at||analysis.generated_at);return `<article class="priorityCard compactDecision${heroVisual?"":" noHeroDecision"}"${hero?` data-hero="${esc(hero)}"`:""}><span class="priorityRank">${esc(x.rank||"•")}</span>${heroVisual}<div class="priorityMain"><div class="decisionHead"><div class="decisionTitle"><b>${esc(x.title||"")}${x.target?` · ${esc(x.target)}`:""}</b>${progress?`<span class="priorityProgress">${esc(progress)}</span>`:""}</div><span class="decisionMetric">${esc(ui.impact)} : ${esc(x.impact_label||"—")} · ${esc(t("resource_efficiency"))} : ${esc(x.resource_efficiency_label||x.roi_label||"—")}</span></div>${Number.isFinite(marginal)?`<small>📈 ${esc(t("marginal_return"))} ${marginal}/100 · ${esc(t("certainty_label"))}: ${esc(certainty)}</small>`:""}<strong>${esc(x.action||"")}</strong><details class="decisionDetails"><summary>${esc(ui.why)}</summary><p>${esc(x.reason||"")}</p>${x.comparison_note?`<small>⚖️ ${esc(x.comparison_note)}</small>`:""}${progress?`<small>🎯 ${esc(progress)}</small>`:""}${x.progress_needed_levels>0?`<small>📈 ${esc(String(x.progress_needed_levels))} ${esc(t("levels_to_breakpoint"))}</small>`:""}${x.timing_window?.label?`<small>⏱️ ${esc(x.timing_window.label)}</small>`:""}${x.condition_key?`<small>🔀 ${esc(t("conditional_recommendation"))}: ${esc(condition)}</small>`:""}${x.data_freshness?.label?`<small>🕒 ${esc(x.data_freshness.label)}</small>`:""}<small>📅 ${esc(t("recommendation_date"))}: ${esc(calculated)}</small><small>🆓 ${esc(x.buy_free||"")}</small><small>💎 ${esc(x.buy_paid||"")}</small></details></div></article>`}).join(""):`<div class="notice">${esc(analysis.summary||t("player_sync_note"))}</div>`;
  }
  if(avoid){
