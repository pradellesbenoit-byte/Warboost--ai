...
import {hasMeaningfulCoreState,hydrateCloudState,mergeDesertStormState,canUseKeepaliveBody,betaStateAfterVerifiedStateRead} from "./lib/cloud-state-recovery.js";
...
function mergeStateProtected(base,incoming,{preferBase=false}={}){
  ...
  out.alliance=safeFields(base.alliance,incoming.alliance,preferBase);out.alliance.desert_storm=mergeDesertStormState(base.alliance?.desert_storm,incoming.alliance?.desert_storm);out.alliance.members=mergeAllianceMembersProtected(base.alliance?.members,incoming.alliance?.members,preferBase);
  ...
  out.squads=Array.from({length:4},(_,i)=>{const b=base.squads?.[i]||emptySquad(i+1),n=incoming.squads?.[i];if(!n)return b;const sq=safeFields(b,n,preferBase),incomingPower=n.power_m??n.power,basePending=b.power_sync_status==="pending";sq.id=i+1;sq.name=`Squad ${i+1}`;sq.power=preferBase||basePending?b.power:(heroPowerIsConfirmed(incomingPower)?canonicalPowerMillions(incomingPower):b.power);sq.last_confirmed_power=canonicalPowerMillions(n.last_confirmed_power??b.last_confirmed_power);sq.power_sync_status=basePending?"pending":(n.power_sync_status==="pending"?"pending":(heroPowerIsConfirmed(incomingPower)?(n.power_sync_status||"confirmed"):b.power_sync_status||"unknown"));sq.needs_rescan=preferBase?b.needs_rescan===true:n.needs_rescan===true;sq.composition_changed_at=(preferBase?b.composition_changed_at:null)||n.composition_changed_at||b.composition_changed_at||null;sq.heroes=Array.from({length:5},(_,j)=>mergeHeroSlotIdentitySafe(b.heroes?.[j],n.heroes?.[j],{preferBase}));return sq});
  ...
  try{
    ...
    if(!r.ok){
      if(r.status===409&&j?.error==="profile_write_conflict"){
        ...
        if(remote){try{state=mergeStateProtected(remote,state,{preferBase:false})}catch{state=remote}state.player_id=cloudSession.user.id}
...
function heroPowerForDisplay(squad,hero){
  ...
  return profileAt>=slotAt?profilePower:slotPower;
}
...
function performSquadSwap(fromId,toId){
  ...
  try{
    ...
    expandedSquads.add(from);expandedSquads.add(to);
    requestAnimationFrame(()=>{document.querySelector(`#squadList .squad[data-squad-id="${from}"]`)?.classList.add("open");document.querySelector(`#squadList .squad[data-squad-id="${to}"]`)?.classList.add("open");document.querySelector(`#squadList .squad[data-squad-id="${from}"] .squadHead`)?.setAttribute("aria-expanded","true");document.querySelector(`#squadList .squad[data-squad-id="${to}"] .squadHead`)?.setAttribute("aria-expanded","true")});
...
function renderSquads(){
  ...
  state.squads.forEach((sq,i)=>{
    ...
    const swapMenu=swapTargets.length?`<div class="squadSwapMenu hidden" data-squad-swap-menu="${id}"><span>${esc(t("squad_swap_with"))}</span>${swapTargets.map(x=>`<button type="button" class="squadSwapTarget" data-squad-swap-target="${x.id}">${esc(t("squad"))} ${x.id}</button>`).join("")}</div>`:"";
      const isExpanded=expandedSquads.has(id);
     ...
     el.querySelector(".squadHead")?.addEventListener("click",()=>{const next=!expandedSquads.has(id);if(next)expandedSquads.add(id);else expandedSquads.delete(id);el.classList.toggle("open",next);el.querySelector(".squadHead")?.setAttribute("aria-expanded",String(next))});
...
$("#saveProfileBtn").addEventListener("click",async()=>{
  const status=$("#profileSaveStatus"),btn=$("#saveProfileBtn"),name=$("#fName").value.trim(),server=$("#fServer").value.trim(),hq=Number($("#fHq").value),requestedRole=$("#fRole").value.trim().toUpperCase()||"R1";
...
async function syncAll(){if(!requireBetaAccess()||!requireBetaConsent())return;const btns=[$("#syncAllBtn"),$("#syncPlayerBtn")].filter(Boolean);btns.forEach(b=>{b.disabled=true;b.textContent=t("syncing")});try{if(!hasMeaningfulCore(state)&&cloudSession?.access_token){const recovered=await reconcileAuthenticatedRuntime("manual-sync",{force:true});if(!recovered?.ok||!hasMeaningfulCore(state)){const note=$("#playerSyncInfo");if(note){note.className="notice warn";note.classList.remove("hidden");note.textContent=t("offline_keep")}scheduleCloudPullRetry();return}}const {response:r,json:j}=await fetchJsonBounded("/api/sync",{method:"POST",headers:authHeaders({"content-type":"application/json"}),body:JSON.stringify({state,locale:lang,base_updated_at:cloudRevision})},20000);if(r.ok&&j.state){cloudRevision=j.updated_at||cloudRevision;const remote=hydrateCloudState(j.state,initialState(),cloudSession?.user?.id||state.player_id);let merged;try{merged=mergeStateProtected(state,remote,{preferBase:false})}catch{merged=remote}try{state=repairLegacySquadIdentity(merged).state}catch{state=remote}state.sync={...state.sync,status:"ok",provider:j.provider||state.sync.provider||"warboost-local",provider_kind:j.provider_kind||state.sync.provider_kind||"local",capabilities:j.capabilities||state.sync.capabilities||[],last_sync:j.synced_at||new Date().toISOString(),last_error:null,sources:{...state.sync.sources,...j.sources}};saveState();$("#playerSyncInfo").textContent=t("safe_sync_done")}else if(r.status===409&&j?.error==="profile_write_conflict"){const remote=j?.state?hydrateCloudState(j.state,initialState(),cloudSession?.user?.id||state.player_id):null;if(remote){try{state=mergeStateProtected(remote,state,{preferBase:false})}catch{state=remote}}cloudRevision=j?.updated_at||null;state.sync.last_error="profile_write_conflict";state.sync.status="waiting";saveState();scheduleCloudRetry(250)}else{state.sync.last_error=j.message||j.error||t("hybrid_no_public");state.sync.status="waiting";saveState()}}catch{state.sync.last_error=t("offline_keep");state.sync.status="offline";saveState()}finally{btns.forEach((b,i)=>{b.disabled=false;b.textContent=i===0?t("public_refresh"):t("public_button")})}}
...
$("#collapseAllSquadsBtn")?.addEventListener("click",()=>{expandedSquads.clear();document.querySelectorAll("#squadList .squad").forEach(el=>{el.classList.remove("open");el.querySelector(".squadHead")?.setAttribute("aria-expanded","false")})});
