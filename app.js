...
const RELEASE_LABEL="HF8.6.19"; // Public Beta Full Reliability · bounded requests + precise bootstrap tracing + safe onboarding · includes HF8.6.18
...
function betaPrivateDataVisible(){const userId=String(cloudSession?.user?.id||"");const trustedLocal=Boolean(userId&&hasMeaningfulCore(readAccountState(userId)));return Boolean(cloudSession?.user)&&betaAccessAllowed()&&betaConsentAccepted()&&(cloudProfileVerified||trustedLocal)}
...
window.addEventListener("online",()=>{if(!cloudProfileVerified&&cloudSession?.access_token&&betaConsentAccepted())pullServerState(readAccountState(cloudSession.user?.id),{fastRestore:true});else if(cloudDirty)pushServerState()});
document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible"){if(!cloudProfileVerified&&cloudSession?.access_token&&betaConsentAccepted())pullServerState(readAccountState(cloudSession.user?.id),{fastRestore:true});else if(cloudDirty)pushServerState()}else if(document.visibilityState==="hidden"&&cloudDirty)pushServerState({keepalive:true})});
