...
import {ACTIVITY_EVENT_TYPES,activityEventId,mergeActivityEvents,confirmedActivityEvents,eventCountsByType} from "./lib/activity-events.js";
...
function renderPlayerActivity(){
  ...
  grid.innerHTML=ACTIVITY_EVENT_TYPES.map(type=>{const active=confirmedToday.has(type);return `<button type="button" class="activityEventBtn${active?" confirmed":""}" data-activity-event="${esc(type)}"><span>${active?"✅":"○"}</span><b>${esc(activityEventLabel(type))}</b><small>${esc(t(active?"activity_remove":"activity_confirm"))}</small></button>`}).join("");
...
function togglePlayerActivityEvent(type){
  if(!betaPrivateDataVisible()||!ACTIVITY_EVENT_TYPES.includes(type))return;const date=lastWarDateKey(),id=activityEventId(type,date),events=mergeActivityEvents(state.activity_events),current=events.find(x=>x.id===id),now=new Date().toISOString(),confirmed=current?.confirmed===true,event={event_type:type,event_date:date,confirmed:!confirmed,confirmed_at:!confirmed?now:null,updated_at:now,source:"player_self_report"};
  ...
  if(self)self.activity_events=mergeActivityEvents(self.activity_events,[event]);saveState();
}
