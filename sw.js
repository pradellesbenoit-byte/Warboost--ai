...
const CACHE="warboost-v2-5-32-hf8-6-32-qg35-home-visibility-r1";
...
self.addEventListener("fetch",e=>{const u=new URL(e.request.url);if(e.request.method!=="GET"||u.pathname.startsWith("/api/"))return;e.respondWith(fetch(e.request).then(r=>{const copy=r.clone();caches.open(CACHE).then(c=>c.put(e.request,copy));return r}).catch(()=>caches.open(CACHE).then(async c=>{const cached=await c.match(e.request);if(cached)return cached;if(e.request.mode==="navigate")return(await c.match("/")||await c.match("/index.html"))||new Response("Offline",{status:503});return new Response("",{status:504})})))});
