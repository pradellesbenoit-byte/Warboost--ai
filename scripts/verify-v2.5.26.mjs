...
import {providerConfig,fetchLastWarState} from '../lib/provider.js';
...
function callAdvice(scope,state,locale='fr-FR'){
  if(scope==='player'){
    ...
    analysis.engine='warboost-ai-core-v2.5.26';
  ...
  if(scope==='alliance'){
    ...
    return {status:200,body:{ok:true,engine:'warboost-alliance-ai-v2.5.26',...a}};
  }
...
// V2.5.2 reliability must survive V2.5.26: Squad 1 is the selected main squad.
{
  ...
  assert.match(body.analysis.engine,/2\.5\.26/);
...
// R5/R4 alliance AI: structured immediate groups + Plan B; R1 is blocked.
{
  ...
  assert.equal(status,200);
  assert.equal(body.ok,true);
  assert.match(body.engine,/2\.5\.26/);
...
// V2.5.26 regression: stale/unknown members must NEVER be assigned to tactical groups.
{
...
// V2.5.26 regression: mixed roster assigns ONLY confirmed-active members.
{
...
// VS and Season structured endpoints still work.
{
  ...
  assert.equal(vs.status,200);assert.equal(vs.body.ok,true);assert.equal(vs.body.day,3);assert.match(vs.body.engine,/2\.5\.26/);
  assert.ok(Array.isArray(vs.body.priorities)&&vs.body.priorities.length>=2);
  const season=callAdvice('season',baseState,'fr-FR');
  assert.equal(season.status,200);assert.equal(season.body.ok,true);assert.equal(season.body.day,20);assert.equal(season.body.total_days,35);assert.match(season.body.engine,/2\.5\.26/);
  assert.ok(Array.isArray(season.body.priorities)&&season.body.priorities.length>=1);
  log('VS and Season advice remain operational on the V2.5.26 engine');
}
...
// Legacy/partial state normalization and merge must preserve known player/squad data.
{
  ...
  assert.equal(normalized.version,'2.5.26');
  ...
  log('Legacy/partial state normalization keeps known player and hero data through V2.5.26');
}
...
// 22 requested language families + separate US English variant = 23 explicit choices, plus Auto.
{
  ...
  log('All 23 explicit language choices resolve V2.5.26 structured Player/Alliance/VS/Season/voice labels');
}

// V2.5.26 Safe Launch beta: invitation allowlist, free PRO, consent and payment lock are enforced by design.
{
  ...
  assert.equal(access.allowed,false);assert.equal(access.access_status,'invite-setup-required');
  ...
  assert.match(html,/id="betaHeaderBadge"/);assert.match(html,/id="betaAccessSection"/);assert.match(html,/id="betaConsent"/);assert.match(html,/id="supportBtn"/);assert.match(html,/id="supportDrawer"/);
  ...
  assert.match(pro,/SAFE_LAUNCH_PAYMENT_DISABLED/);assert.match(pro,/payments_enabled:false/);assert.match(pro,/beta_configured:beta\.configured/);assert.match(pro,/allowed:beta\.allowed/);assert.match(app,/fetch\("\/api\/pro"/);
  for(const flag of ['public_beta_invite_badge','beta_database_invitation_registry','beta_admin_invite_manager','beta_bulk_invites','beta_invite_revocation','beta_legacy_env_allowlist_fallback','beta_access_enforced','beta_pro_free_for_invited_testers','beta_payments_disabled','beta_consent_required_before_cloud_ai_writes','beta_consent_revocable_on_device','beta_consent_account_scoped','support_ticketing','support_private_attachments','support_admin_allowlist','support_player_admin_identity_separated','support_password_never_collected','beta_feedback_no_full_user_agent','beta_existing_player_data_preserved','beta_status_reuses_pro_endpoint']) assert.match(health,new RegExp(flag+':true'));
  const inviteMigration=read('supabase/migration_v2_5_26_beta_invites.sql');assert.match(inviteMigration,/wb1_beta_invites/);assert.doesNotMatch(inviteMigration,/\bdrop\s+table\b|\btruncate\b|\bdelete\s+from\b/i);
  assert.equal(fs.readdirSync(path.join(root,'api')).filter(x=>x.endsWith('.js')).length,12,'V2.5.26 must stay within the Vercel Hobby 12-function deployment limit');
  log('Safe Launch invite access, consent, free PRO, feedback privacy and payment lock safeguards are present');
}

// V2.5.26 browser auth: no third-party CDN dependency, compatible Supabase session storage,
...
// Static application safeguards.
{
  ...
  assert.match(app,/APP_VERSION\s*=\s*["']2\.5\.26["']/);
  ...
  assert.equal(manifest.name.includes('V2.5.26'),true);
  assert.match(sw,/warboost-v2-5-26-safe-launch/);
  ...
  assert.equal(pkg.version,'2.5.26');
...
async function withMockSupabase(routes,fn){
  const old={url:process.env.SUPABASE_URL,key:process.env.SUPABASE_SERVICE_ROLE_KEY,anon:process.env.SUPABASE_ANON_KEY,beta:process.env.WARBOOST_BETA_EMAILS,fetch:globalThis.fetch};
  ...
  if(!process.env.WARBOOST_BETA_EMAILS)process.env.WARBOOST_BETA_EMAILS='test-invited@example.com';
  const calls=[];
  globalThis.fetch=async(url,options={})=>{calls.push({url:String(url),options});for(const route of routes){if(route.match(String(url),options,calls)){let body=route.body;if(String(url).endsWith('/auth/v1/user')&&body&&typeof body==='object'&&!Array.isArray(body)&&!body.email)body={...body,email:'test-invited@example.com'};return jsonResponse(body,route.status||200)}}return jsonResponse({message:'unmatched mock route',url:String(url)},500)};
  try{return await fn(calls)}finally{globalThis.fetch=old.fetch;for(const [k,v] of [['SUPABASE_URL',old.url],['SUPABASE_SERVICE_ROLE_KEY',old.key],['SUPABASE_ANON_KEY',old.anon],['WARBOOST_BETA_EMAILS',old.beta]]){if(v===undefined)delete process.env[k];else process.env[k]=v}}
}
...
// Private-beta server gate: invitation and per-account consent are enforced server-side; payments stay disabled.
{
  ...
  await withMockSupabase([
    ...
    const pay=await callAsync(proHandler,{action:'checkout'});assert.equal(pay.status,403);assert.equal(pay.body.error,'SAFE_LAUNCH_PAYMENT_DISABLED');
  });
  if(oldEmails===undefined)delete process.env.WARBOOST_BETA_EMAILS;else process.env.WARBOOST_BETA_EMAILS=oldEmails;
...
// V2.5.26: relevance, data confidence and availability are independent dimensions.
{
...
// V2.5.26: recent user scan can raise evidence confidence but is still an observation, not official live availability.
{
...
// V2.5.26 Season lifecycle: unknown is not 0%, active keeps real 0%, ended/interseason disables S6 advice.
{
...
// V2.5.26 structured labels must exist in every explicit language.
{
  ...
  log('All 23 explicit languages include V2.5.26 alliance and Sunday-prep labels');
}


// V2.5.26 source guards: no missing progress coercion, lifecycle UI and health flags are present.
{
...
// V2.5.26 Season UI: historical labels are dynamic and localized; lifecycle engine remains V2.5.10-compatible.
{
  ...
  for(const [code] of explicit){
    ...
    assert.notEqual(label,'season_last_profession_short',`${code} missing V2.5.26 historical profession label`);
...
// V2.5.26 private-beta privacy boundary: local/cloud state is preserved but never rendered signed out,
...
// V2.5.26 UI cleanup: no internal season lifecycle key, home tile shows inter-season state,
...
// V2.5.26 regression: EX explanations must match the real ranking and cover all five main heroes.
{
...
// V2.5.26 regression: an unknown EX remains visible as missing; it is never silently invented.
{
  const st=structuredClone(baseState);
...
// V2.5.26 regression: source evidence must be filtered to the active diagnostic domains.
{
  const st=structuredClone(baseState);
  st.player_context={objective:'vs',server_profile:'auto'};
  st.squads[0]={id:1,power:42.58,updated_at:now,needs_rescan:false,heroes:[
...
// V2.5.26 UI: expose all-hero EX comparison and dated/source-typed meta evidence without claiming live meta.
{
...
// V2.5.26 labels are present in all explicit languages, including the safer VS/server/alliance copy.
{
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const k of keys)assert.notEqual(tr(k),k,`${code} missing V2.5.26 ${k}`);assert.match(tr('tagline'),/V2\.5\.26/)}
  ...
  log('All 23 explicit languages include the V2.5.26 explainability and safer-context labels');
}


// V2.5.26 regression: Boutique IA consumes the exact current Diagnostic PRO EX ranking.
{
  const st=structuredClone(baseState);
  st.player_context={objective:'vs',server_profile:'auto'};
  st.vs={day:0,week:35,opponent:null,updated_at:now};
...
// V2.5.26 beta regression: historical real-money references are quarantined from current paid-offer rankings.
{
...
// V2.5.26 regression: a scanned paid offer remains guarded until price, contents AND cost/gain are all current/verified.
{
...
// V2.5.26 regression: gear-shop advice identifies a known target or explicitly says the target is unconfirmed.
{
...
// V2.5.26 UI/health/multilingual contract: beta players can distinguish historical paid references from current offers.
{
  ...
  assert.equal(pkg.version,'2.5.26');assert.equal(pkg.name,'warboost-v2-safe-launch-invite-manager');assert.equal(pkg.scripts.verify,'node scripts/verify-v2.5.26.mjs && node scripts/verify-support-v2.5.24.mjs && node scripts/verify-auth-recovery-v2.5.26.mjs && node scripts/verify-beta-invites-v2.5.26.mjs && node scripts/verify-invite-admin-api-v2.5.26.mjs');
  ...
  assert.match(css,/\.shopHistoricalPaidCard/);assert.match(css,/\.shopHistoryGuard/);assert.match(sw,/warboost-v2-5-26-safe-launch/);
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const key of keys)assert.notEqual(tr(key),key,`${code} missing V2.5.26 ${key}`);assert.match(tr('tagline'),/V2\.5\.26/)}
...
// V2.5.26 HF1: unconfirmed beta accounts must recover in-place instead of showing raw English errors.
{
  ...
  assert.match(sw,/warboost-v2-5-26-safe-launch/,'Service worker cache must be bumped so beta players receive the V2.5.26 onboarding build');
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){
    ...
    for(const key of keys)assert.notEqual(tr(key),key,`${code} missing V2.5.26 HF1 ${key}`);
  }
...
// V2.5.26 private-beta onboarding: no publisher-demo label is rendered to players,
// incomplete player profiles get a direct first-scan path, and no existing data is cleared.
{
  const app=read('app.js'),html=read('index.html'),health=read('api/health.js'),sw=read('sw.js');
  assert.doesNotMatch(html,/data-i18n=["']publisher_demo["']/,'Private beta home must not render the Publisher Demo eyebrow');
  assert.match(html,/data-i18n=["']beta_badge["'][^>]*>PRIVATE BETA|data-i18n=["']beta_badge["']/,'Private beta eyebrow must use beta copy');
  assert.match(html,/id=["']playerOnboarding["']/);
  assert.match(html,/id=["']playerOnboardingScanBtn["']/);
  assert.match(app,/function\s+playerNeedsOnboarding\s*\(/);
  assert.match(app,/function\s+renderPlayerOnboarding\s*\(/);
  assert.match(app,/playerOnboardingScanBtn/);
  assert.match(app,/playerNeedsOnboarding\(\).*openDrawer\("scan"\)/s,'Home CTA should open Scan for incomplete beta profiles');
  assert.match(app,/Number\(p\.power_m\)>0\?fmtPower\(p\.power_m\):t\("to_fill"\)/,'Missing power should be action-oriented, not a bare dash');
...
console.log('\nWarBoost V2.5.26 verification: PASS');


// V2.5.26 Safe Launch: external game access is hard-disabled even if legacy/official env vars exist.
{
  const old={...process.env};
  process.env.WARBOOST_LASTWAR_OFFICIAL_URL='https://example.invalid/official';
  process.env.WARBOOST_LASTWAR_PROVIDER_URL='https://example.invalid/provider';
  process.env.WARBOOST_PUBLIC_LASTWAR_URL='https://example.invalid/public';
  process.env.WARBOOST_ALLOW_LEGACY_PROVIDER='true';
  const cfg=providerConfig();
  assert.equal(cfg.safe_launch_lock,true);
  assert.equal(cfg.official,false);assert.equal(cfg.approved,false);assert.equal(cfg.legacy,false);
  let err=null;try{await fetchLastWarState({})}catch(e){err=e}
  assert.ok(err);assert.equal(err.code,'PROVIDER_NOT_CONNECTED');assert.equal(err.safe_launch_lock,true);
  for(const k of ['WARBOOST_LASTWAR_OFFICIAL_URL','WARBOOST_LASTWAR_PROVIDER_URL','WARBOOST_PUBLIC_LASTWAR_URL','WARBOOST_ALLOW_LEGACY_PROVIDER']){
    if(old[k]===undefined)delete process.env[k]; else process.env[k]=old[k];
  }
  log('Safe Launch ignores external game-provider environment variables and hard-locks game access');
}

// Payment implementation must not ship in Safe Launch.
{
  const pro=read('api/pro.js'),ingest=read('api/ingest.js'),sync=read('api/sync.js');
  assert.doesNotMatch(pro,/stripe\.com|STRIPE_SECRET_KEY|checkout\/sessions|billing_portal/i);
  assert.match(pro,/SAFE_LAUNCH_PAYMENT_DISABLED/);
  assert.match(ingest,/SAFE_LAUNCH_INGEST_DISABLED/);
  assert.equal(fs.existsSync(path.join(root,'api','cron-sync.js')),false,'Disabled external cron endpoint is removed to keep the Vercel Hobby function budget for /api/support');
  assert.doesNotMatch(sync,/fetchLastWarState|WARBOOST_LASTWAR_|WARBOOST_PUBLIC_LASTWAR_/);
  log('Payment and external ingest/provider paths are hard-disabled; unused cron endpoint is removed');
}
...
// Independent branding and player-facing transparency.
{
  ...
  assert.match(html,/WarBoost V2\.5\.26/);
...
// Game update is informational only; no unverified hero/meta mutation is asserted.
{
  ...
  log('Latest reviewed public game update is informational only and Season 7 rumors stay inactive');
}

// Safe Launch consent revision must not erase previous player state.
{
  const app=read('app.js'),beta=read('lib/beta-access.js');
  assert.match(app,/2026-09-05-safe-launch-v2/);assert.match(beta,/2026-09-05-safe-launch-v2/);
  assert.doesNotMatch(app,/localStorage\.clear\s*\(/);
  assert.match(app,/STORE_KEY="warboost_v1_core_state"/);
  assert.match(app,/BACKUP_KEY="warboost_last_good_state"/);
  log('Safe Launch re-consent is versioned while existing local player data keys remain intact');
}

// All explicit languages expose the Safe Launch critical labels.
{
  const critical=['beta_badge','beta_release_label','beta_access_title','safe_external_disabled','safe_sync_note','independent_title','independent_disclaimer','safe_no_access','legal_notice','privacy_notice','game_update_title','game_update_note','game_update_impact','season7_guard'];
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){
    const t=translator(code);for(const key of critical)assert.notEqual(t(key),key,`${code} missing Safe Launch ${key}`);
  }
  log('All 23 explicit languages resolve Safe Launch critical labels');
}



// V2.5.26 Support joueurs: private ticketing, optional screenshots, history, replies and admin console.
{
  const html=read('index.html'),app=read('app.js'),support=read('api/support.js'),admin=read('support-admin.js'),adminHtml=read('support-admin.html'),health=read('api/health.js'),migration=read('supabase/migration_v2_5_24_support.sql'),privacy=read('privacy.html');
  for(const id of ['supportBtn','supportDrawer','supportCategory','supportSubject','supportDescription','supportAttachment','supportDiagnostics','supportConsent','supportSubmitBtn','supportTickets','supportRefreshBtn'])assert.match(html,new RegExp(`id=["']${id}["']`));
  assert.match(app,/async function submitSupportTicket\(/);assert.match(app,/async function refreshSupportTickets\(/);assert.match(app,/async function replySupportTicket\(/);assert.match(app,/async function supportImageData\(/);
  assert.match(app,/image\/jpeg/);assert.match(app,/2\.7\*1024\*1024/);assert.doesNotMatch(app,/password.*support/i);
  assert.match(support,/WARBOOST_SUPPORT_ADMINS/);assert.match(support,/warboost-support/);assert.match(support,/ATTACHMENT_TOO_LARGE/);assert.match(support,/author_kind/);assert.match(support,/waiting_player/);assert.match(support,/signedAttachment/);
  assert.match(adminHtml,/noindex,nofollow/);assert.match(admin,/\/api\/support\?admin=1/);assert.match(admin,/action:"status"/);assert.match(admin,/action:"reply"/);
  assert.match(migration,/create table if not exists public\.wb1_support_tickets/i);assert.match(migration,/create table if not exists public\.wb1_support_messages/i);assert.match(migration,/warboost-support/);assert.match(migration,/revoke all privileges.*authenticated/i);assert.doesNotMatch(migration,/\bdrop\s+table\b|\btruncate\b|\bdelete\s+from\b/i);
  assert.match(privacy,/Service client et tickets/);assert.match(privacy,/2 Mo/);assert.match(health,/support_ticketing:true/);assert.match(health,/support_private_attachments:true/);assert.match(health,/support_admin_allowlist:true/);
  const keys=['support_button','support_title','support_new_ticket','support_send','support_history','support_refresh','support_reply','support_status_received','support_status_in_progress','support_status_waiting_player','support_status_resolved','support_cat_login','support_cat_scan','support_cat_data','support_cat_ai','support_cat_alliance','support_cat_bug','support_cat_suggestion','support_cat_other'];
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const key of keys)assert.notEqual(tr(key),key,`${code} missing V2.5.26 support ${key}`)}
  assert.equal(fs.readdirSync(path.join(root,'api')).filter(x=>x.endsWith('.js')).length,12,'Support must keep total serverless functions at 12');
  log('Support ticketing, private optional screenshots, player history/replies and admin console are present without exceeding 12 serverless functions');
}


// V2.5.26 password recovery preview redirect must use the stable Vercel branch URL, not an ephemeral deployment URL.
{
  const cfg=read('api/cloud-config.js'),app=read('app.js');
  assert.match(cfg,/VERCEL_BRANCH_URL/);
  assert.match(cfg,/recovery_redirect_url:recoveryRedirect/);
  assert.match(app,/cloudRecoveryRedirect\|\|new URL\("\/reset-password\.html",location\.origin\)/);
  log('Password recovery uses the stable Vercel Preview branch URL with a current-origin fallback');
}

console.log('\nWarBoost V2.5.26 Safe Launch verification: PASS');
