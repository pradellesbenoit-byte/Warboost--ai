...
function callAdvice(scope,state,locale='fr-FR'){
  if(scope==='player'){
    ...
    analysis.engine='warboost-ai-core-v2.5.14';
  ...
  if(scope==='alliance'){
    ...
    return {status:200,body:{ok:true,engine:'warboost-alliance-ai-v2.5.14',...a}};
  }
...
// V2.5.2 reliability must survive V2.5.14: Squad 1 is the selected main squad.
{
  ...
  assert.match(body.analysis.engine,/2\.5\.14/);
...
// R5/R4 alliance AI: structured immediate groups + Plan B; R1 is blocked.
{
  ...
  assert.equal(status,200);
  assert.equal(body.ok,true);
  assert.match(body.engine,/2\.5\.14/);
...
// V2.5.14 regression: stale/unknown members must NEVER be assigned to tactical groups.
{
...
// V2.5.14 regression: mixed roster assigns ONLY confirmed-active members.
{
...
// VS and Season structured endpoints still work.
{
  ...
  assert.equal(vs.status,200);assert.equal(vs.body.ok,true);assert.equal(vs.body.day,3);assert.match(vs.body.engine,/2\.5\.14/);
  assert.ok(Array.isArray(vs.body.priorities)&&vs.body.priorities.length>=2);
  const season=callAdvice('season',baseState,'fr-FR');
  assert.equal(season.status,200);assert.equal(season.body.ok,true);assert.equal(season.body.day,20);assert.equal(season.body.total_days,35);assert.match(season.body.engine,/2\.5\.14/);
  assert.ok(Array.isArray(season.body.priorities)&&season.body.priorities.length>=1);
  log('VS and Season advice remain operational on the V2.5.14 engine');
}
...
// Legacy/partial state normalization and merge must preserve known player/squad data.
{
  ...
  assert.equal(normalized.version,'2.5.14');
  ...
  log('Legacy/partial state normalization keeps known player and hero data through V2.5.14');
}
...
// 22 requested language families + separate US English variant = 23 explicit choices, plus Auto.
{
  ...
  log('All 23 explicit language choices resolve V2.5.14 structured Player/Alliance/VS/Season/voice labels');
}

// V2.5.14 private beta: invitation allowlist, free PRO, consent and payment lock are enforced by design.
{
  ...
  assert.equal(fs.readdirSync(path.join(root,'api')).filter(x=>x.endsWith('.js')).length,12,'V2.5.14 must stay within the Vercel Hobby 12-function deployment limit');
  const readme=read('README.md'),guide=read('UPLOAD_GUIDE_V2_5_14.txt');assert.match(readme,/WARBOOST_BETA_EMAILS/);assert.match(guide,/beta\.access_enforced = true/);
  log('Private beta access, consent, free PRO, feedback privacy and payment lock safeguards are present');
}

// V2.5.14 browser auth: no third-party CDN dependency, compatible Supabase session storage,
...
// Static application safeguards.
{
  ...
  assert.match(app,/APP_VERSION\s*=\s*["']2\.5\.14["']/);
  ...
  assert.equal(manifest.name.includes('V2.5.14'),true);
  assert.match(sw,/warboost-v2-5-14-cloud-auth-reliability/);
  ...
  assert.equal(pkg.version,'2.5.14');
...
// V2.5.14: relevance, data confidence and availability are independent dimensions.
{
...
// V2.5.14: recent user scan can raise evidence confidence but is still an observation, not official live availability.
{
...
// V2.5.14 Season lifecycle: unknown is not 0%, active keeps real 0%, ended/interseason disables S6 advice.
{
...
// V2.5.14 structured labels must exist in every explicit language.
{
  ...
  log('All 23 explicit languages include V2.5.14 alliance and Sunday-prep labels');
}


// V2.5.14 source guards: no missing progress coercion, lifecycle UI and health flags are present.
{
...
// V2.5.14 Season UI: historical labels are dynamic and localized; lifecycle engine remains V2.5.10-compatible.
{
  ...
  for(const [code] of explicit){
    ...
    assert.notEqual(label,'season_last_profession_short',`${code} missing V2.5.14 historical profession label`);
...
// V2.5.14 private-beta privacy boundary: local/cloud state is preserved but never rendered signed out,
...
// V2.5.14 UI cleanup: no internal season lifecycle key, home tile shows inter-season state,
...
console.log('\nWarBoost V2.5.14 verification: PASS');
