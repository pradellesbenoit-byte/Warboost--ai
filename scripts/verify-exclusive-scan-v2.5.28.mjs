...
assert.match(app,/const collapsedSquads=readCollapsedSquads\(\)/);
assert.match(app,/collapsedSquads\.add\(id\)/);
assert.match(app,/body\.hidden=!expanded/);
assert.doesNotMatch(app,/squads\[id-1\]\?\.classList\.add\(['"]open['"]\)/);
assert.doesNotMatch(app,/expandedSquads\.add\(from\)/);
