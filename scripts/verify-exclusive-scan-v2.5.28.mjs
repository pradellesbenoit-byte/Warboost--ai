...
assert.match(app,/class="squadToggle"/);
assert.match(app,/type="checkbox"/);
assert.match(app,/class="squadHead"/);
assert.doesNotMatch(app,/squadHead"\)\?\.addEventListener\("click"/);
assert.match(app,/querySelectorAll\("#squadList \.squadToggle"\)/);
assert.doesNotMatch(app,/<details[^>]+class="squad"/);
assert.match(css,/\.squadCaptureStepGood/);
assert.match(css,/\.squadSectionToolbar/);
assert.match(css,/\.squadToggle:not\(:checked\)\s+~\s+\.squadBody[\s\S]*display:none !important/);
assert.match(css,/\.squadToggle:checked\s+~\s+\.squadBody[\s\S]*display:block !important/);
assert.match(css,/\.squadToggle:checked\s+~\s+\.squadHead \.chev::before[\s\S]*content:"⌃"/);
