...
assert.match(app,/data-squad-toggle/);
assert.match(app,/body\.hidden=!open/);
assert.match(app,/querySelectorAll\("#squadList \[data-squad-toggle\]"\)/);
assert.doesNotMatch(app,/<details[^>]+class="squad"/);
assert.match(css,/\.squadCaptureStepGood/);
assert.match(css,/\.squadSectionToolbar/);
assert.match(css,/\.squadBody\[hidden\][\s\S]*display:none !important/);
assert.match(css,/\.squad\.open \.squadBody:not\(\[hidden\]\)[\s\S]*display:block/);
