...
// V2.5.2 reliability must survive V2.5.3: Squad 1 is the selected main squad.
{
  ...
  assert.match(body.analysis.engine,/2\.5\.3/);
...
// R5/R4 alliance AI: structured immediate groups + Plan B; R1 is blocked.
{
  ...
  assert.equal(status,200);
  assert.equal(body.ok,true);
  assert.match(body.engine,/2\.5\.3/);
...
// VS and Season structured endpoints still work.
{
  ...
  assert.equal(vs.status,200);assert.equal(vs.body.ok,true);assert.equal(vs.body.day,3);assert.match(vs.body.engine,/2\.5\.3/);
  assert.ok(Array.isArray(vs.body.priorities)&&vs.body.priorities.length>=2);
  const season=callAdvice('season',baseState,'fr-FR');
  assert.equal(season.status,200);assert.equal(season.body.ok,true);assert.equal(season.body.day,20);assert.equal(season.body.total_days,35);assert.match(season.body.engine,/2\.5\.3/);
  assert.ok(Array.isArray(season.body.priorities)&&season.body.priorities.length>=1);
  log('VS and Season advice remain operational on the V2.5.3 engine');
}
...
// Legacy/partial state normalization and merge must preserve known player/squad data.
{
  ...
  assert.equal(normalized.version,'2.5.3');
  ...
  log('Legacy/partial state normalization keeps known player and hero data through V2.5.3');
}
...
// 22 requested language families + separate US English variant = 23 explicit choices, plus Auto.
{
  ...
  log('All 23 explicit language choices resolve V2.5.3 structured Player/Alliance/VS/Season/voice labels');
}
...
{
  const app=read('app.js'),html=read('index.html'),pkg=JSON.parse(read('package.json')),sync=read('api/sync.js'),roleApi=read('api/alliance-role.js'),health=read('api/health.js'),migration=read('supabase/migration_v2_5_3.sql'),manifest=JSON.parse(read('manifest.webmanifest')),sw=read('sw.js');
  assert.match(app,/APP_VERSION\s*=\s*["']2\.5\.3["']/);
  ...
  assert.doesNotMatch(app,/warboost_v253_/);
  ...
  assert.match(health,/pending-approval/);
  assert.match(health,/unauthorized_source_default\s*:\s*false/);
  assert.equal(manifest.name.includes('V2.5.3'),true);
  assert.match(sw,/warboost-v2-5-3-audit-completion/);
...
console.log('\nWarBoost V2.5.3 verification: PASS');
