...
// V2.5.2 reliability must survive V2.5.9: Squad 1 is the selected main squad.
{
  ...
  assert.match(body.analysis.engine,/2\.5\.9/);
...
// R5/R4 alliance AI: structured immediate groups + Plan B; R1 is blocked.
{
  ...
  assert.equal(status,200);
  assert.equal(body.ok,true);
  assert.match(body.engine,/2\.5\.9/);
...
// V2.5.9 regression: stale/unknown members must NEVER be assigned to tactical groups.
{
...
// V2.5.9 regression: mixed roster assigns ONLY confirmed-active members.
{
...
// VS and Season structured endpoints still work.
{
  ...
  assert.equal(vs.status,200);assert.equal(vs.body.ok,true);assert.equal(vs.body.day,3);assert.match(vs.body.engine,/2\.5\.9/);
  assert.ok(Array.isArray(vs.body.priorities)&&vs.body.priorities.length>=2);
  const season=callAdvice('season',baseState,'fr-FR');
  assert.equal(season.status,200);assert.equal(season.body.ok,true);assert.equal(season.body.day,20);assert.equal(season.body.total_days,35);assert.match(season.body.engine,/2\.5\.9/);
  assert.ok(Array.isArray(season.body.priorities)&&season.body.priorities.length>=1);
  log('VS and Season advice remain operational on the V2.5.9 engine');
}
...
// Legacy/partial state normalization and merge must preserve known player/squad data.
{
  ...
  assert.equal(normalized.version,'2.5.9');
  ...
  log('Legacy/partial state normalization keeps known player and hero data through V2.5.9');
}
...
// 22 requested language families + separate US English variant = 23 explicit choices, plus Auto.
{
  ...
  log('All 23 explicit language choices resolve V2.5.9 structured Player/Alliance/VS/Season/voice labels');
}
...
// Static application safeguards.
{
  ...
  assert.match(app,/APP_VERSION\s*=\s*["']2\.5\.9["']/);
  ...
  assert.equal(manifest.name.includes('V2.5.9'),true);
  assert.match(sw,/warboost-v2-5-9-shop-evidence-confidence-reliability/);
  ...
  assert.equal(pkg.version,'2.5.9');
...
async function callAsync(handler,body={}){
  ...
  const req={method:'POST',headers:{authorization:'Bearer user-token'},body};
...
// V2.5.9: relevance, data confidence and availability are independent dimensions.
{
...
// V2.5.9: recent user scan can raise evidence confidence but is still an observation, not official live availability.
{
...
// Season: missing data stays low-confidence and S6 never invents an exact power projection.
{
  const empty=structuredClone(baseState);empty.season={};const e=callAdvice('season',empty,'fr-FR');assert.equal(e.body.data_quality,'low');assert.equal(e.body.day,undefined);
  const s6=callAdvice('season',baseState,'fr-FR');assert.equal(s6.body.day,20);assert.equal(s6.body.total_days,35);assert.equal(s6.body.season6_awakening.exact_power_projection,false);
  log('Season advice preserves unknowns and forbids invented exact S6 power projections');
}

// V2.5.9 structured labels must exist in every explicit language.
{
  ...
  log('All 23 explicit languages include V2.5.9 alliance and Sunday-prep labels');
}

console.log('\nWarBoost V2.5.9 verification: PASS');
