...
function callAdvice(scope,state,locale='fr-FR'){
  if(scope==='player'){
    ...
    analysis.engine='warboost-ai-core-v2.5.20';
  ...
  if(scope==='alliance'){
    ...
    return {status:200,body:{ok:true,engine:'warboost-alliance-ai-v2.5.20',...a}};
  }
...
// V2.5.2 reliability must survive V2.5.20: Squad 1 is the selected main squad.
{
  ...
  assert.match(body.analysis.engine,/2\.5\.20/);
...
// R5/R4 alliance AI: structured immediate groups + Plan B; R1 is blocked.
{
  ...
  assert.equal(status,200);
  assert.equal(body.ok,true);
  assert.match(body.engine,/2\.5\.20/);
...
// V2.5.20 regression: stale/unknown members must NEVER be assigned to tactical groups.
{
...
// V2.5.20 regression: mixed roster assigns ONLY confirmed-active members.
{
...
// VS and Season structured endpoints still work.
{
  ...
  assert.equal(vs.status,200);assert.equal(vs.body.ok,true);assert.equal(vs.body.day,3);assert.match(vs.body.engine,/2\.5\.20/);
  assert.ok(Array.isArray(vs.body.priorities)&&vs.body.priorities.length>=2);
  const season=callAdvice('season',baseState,'fr-FR');
  assert.equal(season.status,200);assert.equal(season.body.ok,true);assert.equal(season.body.day,20);assert.equal(season.body.total_days,35);assert.match(season.body.engine,/2\.5\.20/);
  assert.ok(Array.isArray(season.body.priorities)&&season.body.priorities.length>=1);
  log('VS and Season advice remain operational on the V2.5.20 engine');
}
...
// Legacy/partial state normalization and merge must preserve known player/squad data.
{
  ...
  assert.equal(normalized.version,'2.5.20');
  ...
  log('Legacy/partial state normalization keeps known player and hero data through V2.5.20');
}
...
// 22 requested language families + separate US English variant = 23 explicit choices, plus Auto.
{
  ...
  log('All 23 explicit language choices resolve V2.5.20 structured Player/Alliance/VS/Season/voice labels');
}

// V2.5.20 private beta: invitation allowlist, free PRO, consent and payment lock are enforced by design.
{
  ...
  assert.equal(fs.readdirSync(path.join(root,'api')).filter(x=>x.endsWith('.js')).length,12,'V2.5.20 must stay within the Vercel Hobby 12-function deployment limit');
...
// V2.5.20 browser auth: no third-party CDN dependency, compatible Supabase session storage,
...
// Static application safeguards.
{
  ...
  assert.match(app,/APP_VERSION\s*=\s*["']2\.5\.20["']/);
  ...
  assert.equal(manifest.name.includes('V2.5.20'),true);
  assert.match(sw,/warboost-v2-5-20-private-beta-onboarding/);
  ...
  assert.equal(pkg.version,'2.5.20');
...
// V2.5.20: relevance, data confidence and availability are independent dimensions.
{
...
// V2.5.20: recent user scan can raise evidence confidence but is still an observation, not official live availability.
{
...
// V2.5.20 Season lifecycle: unknown is not 0%, active keeps real 0%, ended/interseason disables S6 advice.
{
...
// V2.5.20 structured labels must exist in every explicit language.
{
  ...
  log('All 23 explicit languages include V2.5.20 alliance and Sunday-prep labels');
}


// V2.5.20 source guards: no missing progress coercion, lifecycle UI and health flags are present.
{
...
// V2.5.20 Season UI: historical labels are dynamic and localized; lifecycle engine remains V2.5.10-compatible.
{
  ...
  for(const [code] of explicit){
    ...
    assert.notEqual(label,'season_last_profession_short',`${code} missing V2.5.20 historical profession label`);
...
// V2.5.20 private-beta privacy boundary: local/cloud state is preserved but never rendered signed out,
...
// V2.5.20 UI cleanup: no internal season lifecycle key, home tile shows inter-season state,
...
// V2.5.20 regression: EX explanations must match the real ranking and cover all five main heroes.
{
...
// V2.5.20 regression: an unknown EX remains visible as missing; it is never silently invented.
{
  const st=structuredClone(baseState);
...
// V2.5.20 regression: source evidence must be filtered to the active diagnostic domains.
{
  const st=structuredClone(baseState);
  st.player_context={objective:'vs',server_profile:'auto'};
  st.squads[0]={id:1,power:42.58,updated_at:now,needs_rescan:false,heroes:[
...
// V2.5.20 UI: expose all-hero EX comparison and dated/source-typed meta evidence without claiming live meta.
{
...
// V2.5.20 labels are present in all explicit languages, including the safer VS/server/alliance copy.
{
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const k of keys)assert.notEqual(tr(k),k,`${code} missing V2.5.20 ${k}`);assert.match(tr('tagline'),/V2\.5\.20/)}
  ...
  log('All 23 explicit languages include the V2.5.20 explainability and safer-context labels');
}


// V2.5.20 regression: Boutique IA consumes the exact current Diagnostic PRO EX ranking.
{
  const st=structuredClone(baseState);
  st.player_context={objective:'vs',server_profile:'auto'};
  st.vs={day:0,week:35,opponent:null,updated_at:now};
...
// V2.5.20 beta regression: historical real-money references are quarantined from current paid-offer rankings.
{
...
// V2.5.20 regression: a scanned paid offer remains guarded until price, contents AND cost/gain are all current/verified.
{
...
// V2.5.20 regression: gear-shop advice identifies a known target or explicitly says the target is unconfirmed.
{
...
// V2.5.20 UI/health/multilingual contract: beta players can distinguish historical paid references from current offers.
{
  ...
  assert.equal(pkg.version,'2.5.20');assert.equal(pkg.name,'warboost-v2-private-beta');assert.equal(pkg.scripts.verify,'node scripts/verify-v2.5.20.mjs');
  ...
  assert.match(css,/\.shopHistoricalPaidCard/);assert.match(css,/\.shopHistoryGuard/);assert.match(sw,/warboost-v2-5-20-private-beta-onboarding/);
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const key of keys)assert.notEqual(tr(key),key,`${code} missing V2.5.20 ${key}`);assert.match(tr('tagline'),/V2\.5\.20/)}
  assert.match(readme,/joueurs invités à la bêta privée/i);assert.match(readme,/private-beta/);assert.match(publisher,/Publisher Demo.*V2\.5\.18/is);assert.match(publisher,/V2\.5\.20.*bêta privée/is);
  log('All 23 explicit languages and beta release docs separate historical paid references from current paid offers');
}


// V2.5.20 HF1: unconfirmed beta accounts must recover in-place instead of showing raw English errors.
{
  ...
  assert.match(sw,/warboost-v2-5-20-private-beta-onboarding/,'Service worker cache must be bumped so beta players receive the V2.5.20 onboarding build');
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){
    ...
    for(const key of keys)assert.notEqual(tr(key),key,`${code} missing V2.5.20 HF1 ${key}`);
  }
...
// V2.5.20 private-beta onboarding: no publisher-demo label is rendered to players,
...
console.log('\nWarBoost V2.5.20 verification: PASS');
