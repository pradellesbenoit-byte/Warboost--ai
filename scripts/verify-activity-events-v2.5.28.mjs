...
// Security/access contract: declared R4/R5 unlocks advisory tools only; sensitive management stays verified.
{
  ...
  assert.match(app,/function hasDeclaredAllianceCommandRole\(\)\{return allianceCommandAccess\(\)\.allowed\}/);
  assert.match(app,/function isAllianceManager\(\)\{return allianceCommandAccess\(\)\.allowed\}/);
