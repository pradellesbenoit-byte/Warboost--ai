...
// V2.5.2 reliability must survive V2.5.5: Squad 1 is the selected main squad.
{
  ...
  assert.match(body.analysis.engine,/2\.5\.5/);
  ...
  assert.deepEqual(plan.days.map(x=>x.action_key),['top_priority','checkpoint_top','secondary_priority','measure_progress','shop_alignment','vs_alignment','weekly_review']);
...
// R5/R4 alliance AI: structured immediate groups + Plan B; R1 is blocked.
{
  ...
  assert.equal(status,200);
  assert.equal(body.ok,true);
  assert.match(body.engine,/2\.5\.5/);
...
// VS and Season structured endpoints still work.
{
  ...
  assert.equal(vs.status,200);assert.equal(vs.body.ok,true);assert.equal(vs.body.day,3);assert.match(vs.body.engine,/2\.5\.5/);
  assert.ok(Array.isArray(vs.body.priorities)&&vs.body.priorities.length>=2);
  const season=callAdvice('season',baseState,'fr-FR');
  assert.equal(season.status,200);assert.equal(season.body.ok,true);assert.equal(season.body.day,20);assert.equal(season.body.total_days,35);assert.match(season.body.engine,/2\.5\.5/);
  assert.ok(Array.isArray(season.body.priorities)&&season.body.priorities.length>=1);
  log('VS and Season advice remain operational on the V2.5.5 engine');
}
...
// Legacy/partial state normalization and merge must preserve known player/squad data.
{
  ...
  assert.equal(normalized.version,'2.5.5');
  ...
  log('Legacy/partial state normalization keeps known player and hero data through V2.5.5');
}
...
// 22 requested language families + separate US English variant = 23 explicit choices, plus Auto.
{
  ...
  const structuredKeys=['seven_day_plan','immediate_actions','plan_b','voice_settings','import_roster','ai_kind_scan','alliance_group_rally','vs_hold_rule','season_structured_priority','activity_active_confirmed','activity_reliability_note','roster_hint','unit_type_aircraft','unit_type_tank','unit_type_missile'];
  ...
  log('All 23 explicit language choices resolve V2.5.5 structured Player/Alliance/VS/Season/voice labels');
}
...
// Static application safeguards.
{
  ...
  assert.match(app,/APP_VERSION\s*=\s*["']2\.5\.5["']/);
  ...
  assert.equal(manifest.name.includes('V2.5.5'),true);
  assert.match(sw,/warboost-v2-5-4-cloud-alliance-access/);
...

{
  ...
  for(const code of LANGUAGES.map(x=>x[0]).filter(x=>x!=='auto')){const tr=translator(code);assert.notEqual(tr('plan7_checkpoint'),'plan7_checkpoint');assert.notEqual(tr('plan7_policy'),'plan7_policy')}
  log('7-day Player plan uses distinct actions, 23-language labels and a mobile-safe single policy note');
}

console.log('\nWarBoost V2.5.5 verification: PASS');
