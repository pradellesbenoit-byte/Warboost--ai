...
// V2.5.2 reliability must survive V2.5.8: Squad 1 is the selected main squad.
{
  ...
  assert.match(body.analysis.engine,/2\.5\.8/);
...
// R5/R4 alliance AI: structured immediate groups + Plan B; R1 is blocked.
{
  ...
  assert.equal(status,200);
  assert.equal(body.ok,true);
  assert.match(body.engine,/2\.5\.8/);
...
// V2.5.8 regression: stale/unknown members must NEVER be assigned to tactical groups.
{
...
// V2.5.8 regression: mixed roster assigns ONLY confirmed-active members.
{
...
// VS and Season structured endpoints still work.
{
  ...
  assert.equal(vs.status,200);assert.equal(vs.body.ok,true);assert.equal(vs.body.day,3);assert.match(vs.body.engine,/2\.5\.8/);
  assert.ok(Array.isArray(vs.body.priorities)&&vs.body.priorities.length>=2);
  const season=callAdvice('season',baseState,'fr-FR');
  assert.equal(season.status,200);assert.equal(season.body.ok,true);assert.equal(season.body.day,20);assert.equal(season.body.total_days,35);assert.match(season.body.engine,/2\.5\.8/);
  assert.ok(Array.isArray(season.body.priorities)&&season.body.priorities.length>=1);
  log('VS and Season advice remain operational on the V2.5.8 engine');
}
...
// Legacy/partial state normalization and merge must preserve known player/squad data.
{
  ...
  assert.equal(normalized.version,'2.5.8');
  ...
  log('Legacy/partial state normalization keeps known player and hero data through V2.5.8');
}
...
// 22 requested language families + separate US English variant = 23 explicit choices, plus Auto.
{
  ...
  log('All 23 explicit language choices resolve V2.5.8 structured Player/Alliance/VS/Season/voice labels');
}
...
// Static application safeguards.
{
  ...
  assert.match(app,/APP_VERSION\s*=\s*["']2\.5\.8["']/);
  ...
  assert.equal(manifest.name.includes('V2.5.8'),true);
  assert.match(sw,/warboost-v2-5-8-r5r4-tactical-reliability/);
  ...
  assert.equal(pkg.version,'2.5.8');
...
// New V2.5.8 labels must exist in every explicit language.
{
  const keys=['alliance_invite_connect','alliance_invite_ready','alliance_invite_failed','alliance_invite_not_found','alliance_joined','alliance_already_joined','alliance_join_failed','alliance_owner_switch_blocked','vs_prep_day','vs_prep_focus','vs_prep_hold_rule'];
  ...
  log('All 23 explicit languages include V2.5.8 alliance and Sunday-prep labels');
}

console.log('\nWarBoost V2.5.8 verification: PASS');
