...
// Static runtime/UI contract.
{
  ...
  assert.match(html,/WarBoost V2\.5\.28 HF[56]/);assert.match(app,/reconcileCurrentPlayerAllianceIdentity/);assert.match(app,/await joinPendingAlliance\(\);await syncAll\(\)/);assert.match(sync,/authoritativeTag/);assert.match(sync,/mergeCloudRosterWithIdentity/);assert.match(supabase,/server_id:serverId/);assert.match(supabase,/alliance_tag:profileAlliance/);assert.doesNotMatch(identity,/\.email\b|email\s*:/i);
  assert.match(sw,/warboost-v2-5-28-(?:hf5-lastwar-identity-link|hf6-player-ready-final)/);assert.match(health,/build:"(?:hf5-lastwar-identity-link|hf6-player-ready-final)"/);
...
// New labels exist in all explicit languages (fallback copy is acceptable, missing keys are not).
{
  ...
  for(const [code] of explicit){const tr=translator(code);for(const key of identityKeys){assert.notEqual(tr(key),key,`${code} missing ${key}`);if(!code.startsWith('en'))assert.notEqual(tr(key),en(key),`${code} still inherits English HF5 identity copy for ${key}`)}assert.match(tr('tagline'),/V2\.5\.28 HF[56]/)}
