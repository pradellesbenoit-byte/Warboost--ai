...

{
  ...
  assert.match(sw,/warboost-v2-5-26-safe-launch-auth-recovery/);
...

{
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){
    ...
    assert.match(t('tagline'),/V2\.5\.26/);
  }
  console.log('✓ Password recovery labels exist in all 23 explicit languages');
}

{
  const app=read('app.js'),migration=read('supabase/migration_v2_5_24_support.sql'),inviteMigration=read('supabase/migration_v2_5_26_beta_invites.sql'),support=read('api/support.js'),provider=read('lib/provider.js'),pro=read('api/pro.js'),health=read('api/health.js'),readme=read('README.md');
  ...
  assert.match(inviteMigration,/create table if not exists public\.wb1_beta_invites/i);
  ...
  assert.match(readme,/migration_v2_5_26_beta_invites\.sql/);
  console.log('✓ Data preservation, support and Safe Launch/payment locks remain intact');
}

console.log('\nWarBoost V2.5.26 auth recovery verification: PASS');
