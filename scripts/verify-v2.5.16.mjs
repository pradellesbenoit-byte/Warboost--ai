...
function callAdvice(scope,state,locale='fr-FR'){
  if(scope==='player'){
    ...
    analysis.engine='warboost-ai-core-v2.5.16';
  ...
  if(scope==='alliance'){
    ...
    return {status:200,body:{ok:true,engine:'warboost-alliance-ai-v2.5.16',...a}};
  }
...
// V2.5.2 reliability must survive V2.5.16: Squad 1 is the selected main squad.
{
  ...
  assert.match(body.analysis.engine,/2\.5\.16/);
...
// R5/R4 alliance AI: structured immediate groups + Plan B; R1 is blocked.
{
  ...
  assert.equal(status,200);
  assert.equal(body.ok,true);
  assert.match(body.engine,/2\.5\.16/);
...
// V2.5.16 regression: stale/unknown members must NEVER be assigned to tactical groups.
{
...
// V2.5.16 regression: mixed roster assigns ONLY confirmed-active members.
{
...
// VS and Season structured endpoints still work.
{
  ...
  assert.equal(vs.status,200);assert.equal(vs.body.ok,true);assert.equal(vs.body.day,3);assert.match(vs.body.engine,/2\.5\.16/);
  assert.ok(Array.isArray(vs.body.priorities)&&vs.body.priorities.length>=2);
  const season=callAdvice('season',baseState,'fr-FR');
  assert.equal(season.status,200);assert.equal(season.body.ok,true);assert.equal(season.body.day,20);assert.equal(season.body.total_days,35);assert.match(season.body.engine,/2\.5\.16/);
  assert.ok(Array.isArray(season.body.priorities)&&season.body.priorities.length>=1);
  log('VS and Season advice remain operational on the V2.5.16 engine');
}
...
// Legacy/partial state normalization and merge must preserve known player/squad data.
{
  ...
  assert.equal(normalized.version,'2.5.16');
  ...
  log('Legacy/partial state normalization keeps known player and hero data through V2.5.16');
}
...
// 22 requested language families + separate US English variant = 23 explicit choices, plus Auto.
{
  ...
  log('All 23 explicit language choices resolve V2.5.16 structured Player/Alliance/VS/Season/voice labels');
}

// V2.5.16 private beta: invitation allowlist, free PRO, consent and payment lock are enforced by design.
{
  ...
  assert.equal(fs.readdirSync(path.join(root,'api')).filter(x=>x.endsWith('.js')).length,12,'V2.5.16 must stay within the Vercel Hobby 12-function deployment limit');
...
// V2.5.16 browser auth: no third-party CDN dependency, compatible Supabase session storage,
...
// Static application safeguards.
{
  ...
  assert.match(app,/APP_VERSION\s*=\s*["']2\.5\.16["']/);
  ...
  assert.equal(manifest.name.includes('V2.5.16'),true);
  assert.match(sw,/warboost-v2-5-16-source-integrity-reliability/);
  ...
  assert.equal(pkg.version,'2.5.16');
...
// V2.5.16: relevance, data confidence and availability are independent dimensions.
{
...
// V2.5.16: recent user scan can raise evidence confidence but is still an observation, not official live availability.
{
...
// V2.5.16 Season lifecycle: unknown is not 0%, active keeps real 0%, ended/interseason disables S6 advice.
{
...
// V2.5.16 structured labels must exist in every explicit language.
{
  ...
  log('All 23 explicit languages include V2.5.16 alliance and Sunday-prep labels');
}


// V2.5.16 source guards: no missing progress coercion, lifecycle UI and health flags are present.
{
...
// V2.5.16 Season UI: historical labels are dynamic and localized; lifecycle engine remains V2.5.10-compatible.
{
  ...
  for(const [code] of explicit){
    ...
    assert.notEqual(label,'season_last_profession_short',`${code} missing V2.5.16 historical profession label`);
...
// V2.5.16 private-beta privacy boundary: local/cloud state is preserved but never rendered signed out,
...
// V2.5.16 UI cleanup: no internal season lifecycle key, home tile shows inter-season state,
...
// V2.5.16 regression: EX explanations must match the real ranking and cover all five main heroes.
{
  ...
  assert.equal(a.meta_intelligence.policy,'verified-links-visible; community-evidence-explanatory-only; account-data-drives-score');
...
// V2.5.16 regression: an unknown EX remains visible as missing; it is never silently invented.
{
  const st=structuredClone(baseState);
...
// V2.5.16 UI: expose all-hero EX comparison and dated/source-typed meta evidence without claiming live meta.
{
  ...
  assert.match(meta,/knowledge_date:'2026-08-30'/);assert.match(meta,/verified-links-visible; community-evidence-explanatory-only; account-data-drives-score/);assert.match(meta,/never claims to browse Last War live/i);assert.match(meta,/official-drone-chests/);assert.match(meta,/bonus:0/);
  assert.doesNotMatch(read('api/advice.js'),/offre actuellement le meilleur compromis|currently has the best balance/i);
  for(const flag of ['exclusive_comparison_all_main_heroes','exclusive_rank_explanations_consistent','exclusive_model_cap_disclosed','exact_ex_fragment_quantities_never_invented','dated_meta_sources_visible','community_meta_secondary_to_account_data','meta_source_exact_urls_visible','community_meta_never_direct_score_bonus','official_meta_source_claim_scoped'])assert.match(health,new RegExp(flag+':true'));
  log('UI and health contract expose explainable ranking and dated meta without live-source overclaiming');
}

// V2.5.16 labels are present in all explicit languages, including the safer VS/server/alliance copy.
{
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const k of keys)assert.notEqual(tr(k),k,`${code} missing V2.5.16 ${k}`);assert.match(tr('tagline'),/V2\.5\.16/)}
  ...
  log('All 23 explicit languages include the V2.5.16 explainability and safer-context labels');
}

console.log('\nWarBoost V2.5.16 verification: PASS');
