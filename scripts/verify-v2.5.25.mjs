...
function callAdvice(scope,state,locale='fr-FR'){
  if(scope==='player'){
    ...
    analysis.engine='warboost-ai-core-v2.5.25';
  ...
  if(scope==='alliance'){
    ...
    return {status:200,body:{ok:true,engine:'warboost-alliance-ai-v2.5.25',...a}};
  }
...
// V2.5.2 reliability must survive V2.5.25: Squad 1 is the selected main squad.
{
  ...
  assert.match(body.analysis.engine,/2\.5\.25/);
...
// R5/R4 alliance AI: structured immediate groups + Plan B; R1 is blocked.
{
  ...
  assert.equal(status,200);
  assert.equal(body.ok,true);
  assert.match(body.engine,/2\.5\.25/);
...
// V2.5.25 regression: stale/unknown members must NEVER be assigned to tactical groups.
{
...
// V2.5.25 regression: mixed roster assigns ONLY confirmed-active members.
{
...
// VS and Season structured endpoints still work.
{
  ...
  assert.equal(vs.status,200);assert.equal(vs.body.ok,true);assert.equal(vs.body.day,3);assert.match(vs.body.engine,/2\.5\.25/);
  assert.ok(Array.isArray(vs.body.priorities)&&vs.body.priorities.length>=2);
  const season=callAdvice('season',baseState,'fr-FR');
  assert.equal(season.status,200);assert.equal(season.body.ok,true);assert.equal(season.body.day,20);assert.equal(season.body.total_days,35);assert.match(season.body.engine,/2\.5\.25/);
  assert.ok(Array.isArray(season.body.priorities)&&season.body.priorities.length>=1);
  log('VS and Season advice remain operational on the V2.5.25 engine');
}
...
// Legacy/partial state normalization and merge must preserve known player/squad data.
{
  ...
  assert.equal(normalized.version,'2.5.25');
  ...
  log('Legacy/partial state normalization keeps known player and hero data through V2.5.25');
}
...
// 22 requested language families + separate US English variant = 23 explicit choices, plus Auto.
{
  ...
  log('All 23 explicit language choices resolve V2.5.25 structured Player/Alliance/VS/Season/voice labels');
}

// V2.5.25 Safe Launch beta: invitation allowlist, free PRO, consent and payment lock are enforced by design.
{
  ...
  assert.equal(access.allowed,false);assert.equal(access.access_status,'allowlist-setup-required');
  ...
  for(const flag of ['public_beta_invite_badge','beta_email_invitation_allowlist','beta_access_enforced_when_allowlist_configured','beta_pro_free_for_invited_testers','beta_payments_disabled','beta_consent_required_before_cloud_ai_writes','beta_consent_revocable_on_device','beta_consent_account_scoped','support_ticketing','support_private_attachments','support_admin_allowlist','support_password_never_collected','beta_feedback_no_full_user_agent','beta_existing_player_data_preserved','beta_status_reuses_pro_endpoint']) assert.match(health,new RegExp(flag+':true'));
  assert.equal(fs.existsSync(path.join(root,'supabase','migration_v2_5_23.sql')),false,'Safe Launch must not introduce an unnecessary database migration');
  assert.equal(fs.readdirSync(path.join(root,'api')).filter(x=>x.endsWith('.js')).length,12,'V2.5.25 must stay within the Vercel Hobby 12-function deployment limit');
  log('Safe Launch invite access, consent, free PRO, feedback privacy and payment lock safeguards are present');
}

// V2.5.25 browser auth: no third-party CDN dependency, compatible Supabase session storage,
...
// Static application safeguards.
{
  ...
  assert.match(app,/APP_VERSION\s*=\s*["']2\.5\.25["']/);
  ...
  assert.equal(manifest.name.includes('V2.5.25'),true);
  assert.match(sw,/warboost-v2-5-25-safe-launch/);
...
// V2.5.25: relevance, data confidence and availability are independent dimensions.
{
...
// V2.5.25: recent user scan can raise evidence confidence but is still an observation, not official live availability.
{
...
// V2.5.25 Season lifecycle: unknown is not 0%, active keeps real 0%, ended/interseason disables S6 advice.
{
...
// V2.5.25 structured labels must exist in every explicit language.
{
  ...
  log('All 23 explicit languages include V2.5.25 alliance and Sunday-prep labels');
}


// V2.5.25 source guards: no missing progress coercion, lifecycle UI and health flags are present.
{
...
// V2.5.25 Season UI: historical labels are dynamic and localized; lifecycle engine remains V2.5.10-compatible.
{
  ...
  for(const [code] of explicit){
    ...
    assert.notEqual(label,'season_last_profession_short',`${code} missing V2.5.25 historical profession label`);
...
// V2.5.25 private-beta privacy boundary: local/cloud state is preserved but never rendered signed out,
...
// V2.5.25 UI cleanup: no internal season lifecycle key, home tile shows inter-season state,
...
// V2.5.25 regression: EX explanations must match the real ranking and cover all five main heroes.
{
...
// V2.5.25 regression: an unknown EX remains visible as missing; it is never silently invented.
{
  const st=structuredClone(baseState);
...
// V2.5.25 regression: source evidence must be filtered to the active diagnostic domains.
{
  const st=structuredClone(baseState);
  st.player_context={objective:'vs',server_profile:'auto'};
  st.squads[0]={id:1,power:42.58,updated_at:now,needs_rescan:false,heroes:[
...
// V2.5.25 UI: expose all-hero EX comparison and dated/source-typed meta evidence without claiming live meta.
{
...
// V2.5.25 labels are present in all explicit languages, including the safer VS/server/alliance copy.
{
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const k of keys)assert.notEqual(tr(k),k,`${code} missing V2.5.25 ${k}`);assert.match(tr('tagline'),/V2\.5\.25/)}
  ...
  log('All 23 explicit languages include the V2.5.25 explainability and safer-context labels');
}


// V2.5.25 regression: Boutique IA consumes the exact current Diagnostic PRO EX ranking.
{
  const st=structuredClone(baseState);
  st.player_context={objective:'vs',server_profile:'auto'};
  st.vs={day:0,week:35,opponent:null,updated_at:now};
...
// V2.5.25 beta regression: historical real-money references are quarantined from current paid-offer rankings.
{
...
// V2.5.25 regression: a scanned paid offer remains guarded until price, contents AND cost/gain are all current/verified.
{
...
// V2.5.25 regression: gear-shop advice identifies a known target or explicitly says the target is unconfirmed.
{
...
// V2.5.25 UI/health/multilingual contract: beta players can distinguish historical paid references from current offers.
{
  ...
  assert.equal(pkg.version,'2.5.25');assert.equal(pkg.name,'warboost-v2-safe-launch-support-auth-recovery');assert.equal(pkg.scripts.verify,'node scripts/verify-v2.5.25.mjs && node scripts/verify-support-v2.5.24.mjs && node scripts/verify-auth-recovery-v2.5.25.mjs');
  ...
  assert.match(css,/\.shopHistoricalPaidCard/);assert.match(css,/\.shopHistoryGuard/);assert.match(sw,/warboost-v2-5-25-safe-launch/);
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const key of keys)assert.notEqual(tr(key),key,`${code} missing V2.5.25 ${key}`);assert.match(tr('tagline'),/V2\.5\.25/)}
...
// V2.5.25 HF1: unconfirmed beta accounts must recover in-place instead of showing raw English errors.
{
  ...
  assert.match(sw,/warboost-v2-5-25-safe-launch/,'Service worker cache must be bumped so beta players receive the V2.5.25 onboarding build');
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){
    ...
    for(const key of keys)assert.notEqual(tr(key),key,`${code} missing V2.5.25 HF1 ${key}`);
  }
...
// V2.5.25 private-beta onboarding: no publisher-demo label is rendered to players,
...
// V2.5.25 Safe Launch: external game access is hard-disabled even if legacy/official env vars exist.
{
...
// Independent branding and player-facing transparency.
{
  ...
  assert.match(html,/WarBoost V2\.5\.25/);
...
// V2.5.25 Support joueurs: private ticketing, optional screenshots, history, replies and admin console.
{
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const key of keys)assert.notEqual(tr(key),key,`${code} missing V2.5.25 support ${key}`)}
...
// V2.5.25 password recovery preview redirect must use the stable Vercel branch URL, not an ephemeral deployment URL.
{
...
console.log('\nWarBoost V2.5.25 Safe Launch verification: PASS');
