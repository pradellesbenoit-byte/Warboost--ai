...
for(const id of ['canyonPlanner','canyonStatus','canyonDateTime','canyonFaction','canyonAdjudicator','canyonAvailabilityList','canyonGenerateBtn','canyonValidateBtn','canyonObjectives','canyonSkills','canyonPlan'])assert.match(html,new RegExp(`id=[\"']${id}[\"']`));
assert.match(app,/function renderCanyonPlanner\(\)/);assert.match(app,/buildCanyonPlan\(members,canyon\.availability/);
