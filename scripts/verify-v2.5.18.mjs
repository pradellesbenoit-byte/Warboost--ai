...
function callAdvice(scope,state,locale='fr-FR'){
  if(scope==='player'){
    ...
    analysis.engine='warboost-ai-core-v2.5.18';
  ...
  if(scope==='alliance'){
    ...
    return {status:200,body:{ok:true,engine:'warboost-alliance-ai-v2.5.18',...a}};
  }
...
// V2.5.2 reliability must survive V2.5.18: Squad 1 is the selected main squad.
{
  ...
  assert.match(body.analysis.engine,/2\.5\.18/);
...
// R5/R4 alliance AI: structured immediate groups + Plan B; R1 is blocked.
{
  ...
  assert.equal(status,200);
  assert.equal(body.ok,true);
  assert.match(body.engine,/2\.5\.18/);
...
// V2.5.18 regression: stale/unknown members must NEVER be assigned to tactical groups.
{
...
// V2.5.18 regression: mixed roster assigns ONLY confirmed-active members.
{
...
// VS and Season structured endpoints still work.
{
  ...
  assert.equal(vs.status,200);assert.equal(vs.body.ok,true);assert.equal(vs.body.day,3);assert.match(vs.body.engine,/2\.5\.18/);
  assert.ok(Array.isArray(vs.body.priorities)&&vs.body.priorities.length>=2);
  const season=callAdvice('season',baseState,'fr-FR');
  assert.equal(season.status,200);assert.equal(season.body.ok,true);assert.equal(season.body.day,20);assert.equal(season.body.total_days,35);assert.match(season.body.engine,/2\.5\.18/);
  assert.ok(Array.isArray(season.body.priorities)&&season.body.priorities.length>=1);
  log('VS and Season advice remain operational on the V2.5.18 engine');
}
...
// Legacy/partial state normalization and merge must preserve known player/squad data.
{
  ...
  assert.equal(normalized.version,'2.5.18');
  ...
  log('Legacy/partial state normalization keeps known player and hero data through V2.5.18');
}
...
// 22 requested language families + separate US English variant = 23 explicit choices, plus Auto.
{
  ...
  log('All 23 explicit language choices resolve V2.5.18 structured Player/Alliance/VS/Season/voice labels');
}

// V2.5.18 private beta: invitation allowlist, free PRO, consent and payment lock are enforced by design.
{
  ...
  assert.equal(fs.readdirSync(path.join(root,'api')).filter(x=>x.endsWith('.js')).length,12,'V2.5.18 must stay within the Vercel Hobby 12-function deployment limit');
...
// V2.5.18 browser auth: no third-party CDN dependency, compatible Supabase session storage,
...
// Static application safeguards.
{
  ...
  assert.match(app,/APP_VERSION\s*=\s*["']2\.5\.18["']/);
  ...
  assert.equal(manifest.name.includes('V2.5.18'),true);
  assert.match(sw,/warboost-v2-5-18-shop-decision-integrity/);
...
// V2.5.18: relevance, data confidence and availability are independent dimensions.
{
  ...
  assert.ok(ref.recommendation_groups.real_money.length>=1);
  assert.ok(ref.recommendation_groups.real_money.every(x=>/Prix observé le 26\/08\/2026/.test(x.price_label)&&/prix actuel non vérifié/i.test(x.price_label)));
...
// V2.5.18: recent user scan can raise evidence confidence but is still an observation, not official live availability.
{
...
// V2.5.18 Season lifecycle: unknown is not 0%, active keeps real 0%, ended/interseason disables S6 advice.
{
...
// V2.5.18 structured labels must exist in every explicit language.
{
  ...
  log('All 23 explicit languages include V2.5.18 alliance and Sunday-prep labels');
}


// V2.5.18 source guards: no missing progress coercion, lifecycle UI and health flags are present.
{
...
// V2.5.18 Season UI: historical labels are dynamic and localized; lifecycle engine remains V2.5.10-compatible.
{
  ...
  for(const [code] of explicit){
    ...
    assert.notEqual(label,'season_last_profession_short',`${code} missing V2.5.18 historical profession label`);
...
// V2.5.18 private-beta privacy boundary: local/cloud state is preserved but never rendered signed out,
...
// V2.5.18 UI cleanup: no internal season lifecycle key, home tile shows inter-season state,
...
// V2.5.18 regression: EX explanations must match the real ranking and cover all five main heroes.
{
...
// V2.5.18 regression: an unknown EX remains visible as missing; it is never silently invented.
{
  const st=structuredClone(baseState);
...
// V2.5.18 regression: source evidence must be filtered to the active diagnostic domains.
{
  const st=structuredClone(baseState);
  st.player_context={objective:'vs',server_profile:'auto'};
  st.squads[0]={id:1,power:42.58,updated_at:now,needs_rescan:false,heroes:[
...
// V2.5.18 UI: expose all-hero EX comparison and dated/source-typed meta evidence without claiming live meta.
{
...
// V2.5.18 labels are present in all explicit languages, including the safer VS/server/alliance copy.
{
  ...
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const k of keys)assert.notEqual(tr(k),k,`${code} missing V2.5.18 ${k}`);assert.match(tr('tagline'),/V2\.5\.18/)}
  ...
  log('All 23 explicit languages include the V2.5.18 explainability and safer-context labels');
}


// V2.5.18 regression: Boutique IA consumes the exact current Diagnostic PRO EX ranking.
{
  const st=structuredClone(baseState);
  st.player_context={objective:'vs',server_profile:'auto'};
  st.vs={day:0,week:35,opponent:null,updated_at:now};
...
// V2.5.18 regression: reference payment channels are separated, and real-money references are dated observations only.
{
  ...
  assert.ok(g&&Array.isArray(g.game_currency)&&Array.isArray(g.diamonds)&&Array.isArray(g.real_money));
  assert.ok(g.game_currency.every(x=>x.purchase_type==='game_currency'));
  assert.ok(g.diamonds.every(x=>x.purchase_type==='diamonds'));
  assert.ok(g.real_money.length>=1&&g.real_money.every(x=>x.purchase_type==='real_money'));
  for(const x of g.real_money){
    ...
    assert.notEqual(x.verdict_key,'buy_now');
  }
  log('Shop payment channels are separated and dated cash references cannot become strong buy advice');
}

// V2.5.18 regression: a scanned paid offer remains guarded until price, contents AND cost/gain are all current/verified.
{
  ...
  const blockedState=makeState(false,false),blockedAnalysis=buildPlayerAnalysis(blockedState,'fr-FR'),blocked=buildShopAdvice(blockedState,'fr-FR',blockedAnalysis).recommendations.find(x=>x.item==='Pass Mensuel');
...
// V2.5.18 regression: gear-shop advice identifies a known target or explicitly says the target is unconfirmed.
{
...
// V2.5.18 UI/health/multilingual contract: grouped payment channels and paid guard are visible everywhere.
{
  const app=read('app.js'),css=read('styles.css'),health=read('api/health.js'),pkg=JSON.parse(read('package.json'));
  assert.equal(pkg.version,'2.5.18');assert.equal(pkg.scripts.verify,'node scripts/verify-v2.5.18.mjs');
  assert.match(app,/function shopGroupRows/);assert.match(app,/recommendation_groups/);assert.match(app,/data-shop-group/);assert.match(app,/shopPaidGuard/);assert.doesNotMatch(app,/recommendations\)\?shop\.recommendations:\[\]\)\.slice\(0,3\)/);
  for(const cls of ['shopGroupTitle','shopPaymentBadge','shopPaidGuard','shopPaidCard'])assert.match(css,new RegExp('\\.'+cls));
  for(const flag of ['shop_diagnostic_ex_single_source_of_truth','shop_payment_channels_separated','paid_offer_requires_current_price_contents_cost_gain','reference_cash_prices_dated_not_current','shop_gear_target_explicit_or_unconfirmed'])assert.match(health,new RegExp(flag+':true'));
  const keys=['shop_group_game','shop_group_diamonds','shop_group_paid','shop_group_unknown','shop_paid_guard'];
  for(const [code] of LANGUAGES.filter(([c])=>c!=='auto')){const tr=translator(code);for(const key of keys)assert.notEqual(tr(key),key,`${code} missing V2.5.18 ${key}`);assert.match(tr('tagline'),/V2\.5\.18/)}
  log('All 23 explicit languages and the UI/health contract expose separated shop payment channels and paid guards');
}

console.log('\nWarBoost V2.5.18 verification: PASS');
