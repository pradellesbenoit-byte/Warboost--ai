...
function sanitize(extracted,now,scanType){
  ...
  if(extracted?.player){const p={updated_at:now};for(const k of ["name","server_id","coordinates","role"]){const v=str(extracted.player[k],80);if(v)p[k]=v}for(const k of ["hq_level","power_m"]){const v=num(extracted.player[k]);if(v!=null)p[k]=v}if(Object.keys(p).length>1)out.player=p}
