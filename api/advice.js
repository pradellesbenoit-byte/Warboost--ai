...
function buildPlayerAnalysis(state,locale){
  ...
  const trusted=configured.filter(x=>x.s?.needs_rescan!==true),focusPool=trusted.length?trusted:configured;
  const powered=focusPool.filter(x=>num(x.s.power)!==null).sort((a,b)=>num(b.s.power)-num(a.s.power));
...
function referenceObservedPriceLabel(offer,store,locale){
  ...
  const prefix=localePack(locale)==="fr"?"réf. ":"ref. ";
