...
const EX_STATUS_TEXT={
  fr:{eligible:"Comparé",cap:"EX30 atteint · aucun palier 10/20/30 supérieur modélisé",missing:"EX à vérifier",other:"Pas de palier classable"},
  en:{eligible:"Compared",cap:"EX30 reached · no higher 10/20/30 breakpoint modeled",missing:"EX needs verification",other:"No rankable breakpoint"},
  es:{eligible:"Comparado",cap:"EX30 alcanzado · no hay otro hito 10/20/30 modelado",missing:"EX por verificar",other:"Sin hito clasificable"},
  de:{eligible:"Verglichen",cap:"EX30 erreicht · kein höherer 10/20/30-Meilenstein modelliert",missing:"EX prüfen",other:"Kein bewertbarer Meilenstein"},
  ja:{eligible:"比較済み",cap:"EX30到達・10/20/30モデル上これ以上の節目なし",missing:"EX要確認",other:"順位付けできる節目なし"},
  zh:{eligible:"已比较",cap:"已达EX30 · 10/20/30模型无更高节点",missing:"EX待核对",other:"无可排名节点"},
  ar:{eligible:"تمت المقارنة",cap:"تم بلوغ EX30 · لا توجد عتبة 10/20/30 أعلى في النموذج",missing:"يجب التحقق من EX",other:"لا توجد عتبة قابلة للترتيب"}
};
...
function buildExclusiveComparison(mainHeroes,scoredCandidates,lang){
  ...
  for(let i=0;i<rows.length;i++){const cur=rows[i],prev=rows[i-1];if(cur?.exclusive_rank&&prev?.exclusive_rank&&Math.round(Number(cur.marginal_value_score)||0)===Math.round(Number(prev.marginal_value_score)||0)){cur.tie_with_previous=true;cur.tie_with_hero=prev.hero;cur.tie_break_basis=["severity","roi","impact"]}else{cur.tie_with_previous=false;cur.tie_with_hero=null;cur.tie_break_basis=[]}}
...
function rankAwareExclusiveWhy(comparison,chosen,lang){
  ...
  if(row.exclusive_rank===1){if(next){const ns=Math.round(Number(next.marginal_value_score)||0),d=Math.max(0,score-ns);text=d===0?tie.leader(row.hero,next.hero,score):tx.leader(row.hero,score,next.hero,ns,d)}else text=tx.solo(row.hero,score)}
    ...
    const previous=rows.find(x=>x.exclusive_rank===row.exclusive_rank-1);
    const comparator=(previous&&Math.round(Number(previous.marginal_value_score)||0)===score)?previous:leader;
    if(comparator){const cs=Math.round(Number(comparator.marginal_value_score)||0),d=Math.max(0,cs-score);text=d===0?tie.follower(row.exclusive_rank,row.hero,comparator.hero,score):tx.follower(row.exclusive_rank,row.hero,score,comparator.hero,cs,d)}
  }
  const tail=exclusiveComparisonTail(comparison,lang);return [text,tail].filter(Boolean).join(" ");
}
const EX_RANK_ACTION={
  fr:{one:h=>`Priorité principale actuelle : concentre les fragments d’arme exclusive sur ${h}, puis recalcule au prochain palier.`,two:(h,l)=>`Deuxième priorité : garde ${h} derrière ${l}. N’y bascule que si la priorité n°1 est volontairement différée par le timing, les ressources ou un palier.`,three:(h,l)=>`Troisième priorité : ${h} reste une option, mais ne détourne pas les fragments des priorités n°1/n°2 sauf si elles sont volontairement différées.`},
  en:{one:h=>`Current main priority: focus exclusive-weapon shards on ${h}, then recalculate at the next breakpoint.`,two:(h,l)=>`Second priority: keep ${h} behind ${l}. Switch only if #1 is deliberately deferred by timing, resources or a breakpoint.`,three:(h,l)=>`Third priority: ${h} remains an option, but do not divert shards from #1/#2 unless those priorities are deliberately deferred.`},
  es:{one:h=>`Prioridad principal actual: concentra los fragmentos de arma exclusiva en ${h} y recalcula en el próximo hito.`,two:(h,l)=>`Segunda prioridad: mantén ${h} detrás de ${l}. Cambia solo si la n.º 1 se difiere por timing, recursos o hito.`,three:(h,l)=>`Tercera prioridad: ${h} sigue siendo opción, pero no desvíes fragmentos de la n.º 1/n.º 2 salvo que se difieran deliberadamente.`},
  de:{one:h=>`Aktuelle Hauptpriorität: Exklusivwaffen-Splitter auf ${h} konzentrieren und am nächsten Meilenstein neu berechnen.`,two:(h,l)=>`Zweite Priorität: ${h} bleibt hinter ${l}. Nur wechseln, wenn Nr. 1 wegen Timing, Ressourcen oder Meilenstein bewusst verschoben wird.`,three:(h,l)=>`Dritte Priorität: ${h} bleibt eine Option, aber Splitter nicht von Nr. 1/Nr. 2 abziehen, außer diese werden bewusst verschoben.`},
  ja:{one:h=>`現在の最優先：専用武器欠片を ${h} に集中し、次の節目で再計算します。`,two:(h,l)=>`第2優先：${h} は ${l} の後。1位をタイミング・資源・節目の都合で意図的に延期する場合のみ切り替えます。`,three:(h,l)=>`第3優先：${h} も候補ですが、1位/2位を意図的に延期しない限り欠片を分散しません。`},
  zh:{one:h=>`当前第一优先：专武碎片集中给 ${h}，到下一节点后重新计算。`,two:(h,l)=>`第二优先：${h} 排在 ${l} 之后。只有第1优先因时机、资源或节点被主动延后时才切换。`,three:(h,l)=>`第三优先：${h} 仍是候选，但除非第1/第2优先被主动延后，否则不要分散碎片。`},
  ar:{one:h=>`الأولوية الرئيسية الحالية: ركّز شظايا السلاح الحصري على ${h} ثم أعد الحساب عند العتبة التالية.`,two:(h,l)=>`الأولوية الثانية: أبقِ ${h} بعد ${l}. انتقل إليه فقط إذا تم تأجيل الأولوية 1 عمداً بسبب التوقيت أو الموارد أو العتبة.`,three:(h,l)=>`الأولوية الثالثة: يبقى ${h} خياراً، لكن لا تحول الشظايا من الأولوية 1/2 إلا إذا تم تأجيلهما عمداً.`}
};
function rankedExclusiveAction(rank,hero,leader,lang){const tx=EX_RANK_ACTION[localePack(lang)]||EX_RANK_ACTION.en;return rank===1?tx.one(hero):rank===2?tx.two(hero,leader):tx.three(hero,leader)}
...
function buildPlayerAnalysis(state,locale){
  ...
  const exclusiveLeader=unique.find(x=>x.kind==="exclusive")?.hero||unique.find(x=>x.kind==="exclusive")?.target||"#1";
  unique.forEach((x,i)=>{x.rank=i+1;if(x.kind==="exclusive"){x.comparison_note=rankAwareExclusiveWhy(exclusiveComparison,x,lang);x.action=rankedExclusiveAction(x.rank,canonicalHeroName(x.hero||x.target),canonicalHeroName(exclusiveLeader),lang)}});
...
const ALLIANCE_AI={
 fr:{empty:"Fais rejoindre les membres et synchronise leur progression avant de générer un plan fiable.",line:(a,r,i,u=0)=>`${a} actifs confirmés · ${r} à actualiser · ${i===0&&(r+u)>0?"inactivité non évaluée":`${i} inactifs probables`}.`,refresh:"Données trop anciennes pour des affectations tactiques fiables : actualise d’abord le roster.",core:n=>`Noyau actif confirmé : ${n}.`,action:"R5/R4 pilotent · R3 encadrent · R2 relaient/soutiennent · R1 forment la base. Contacte toujours un membre avant toute exclusion."},
 en:{empty:"Invite members and synchronize their progression before generating a reliable plan.",line:(a,r,i,u=0)=>`${a} confirmed active · ${r} need refresh · ${i===0&&(r+u)>0?"inactivity not evaluated":`${i} probable inactive`}.`,refresh:"Data is too old for reliable tactical assignments: refresh the roster first.",core:n=>`Confirmed active core: ${n}.`,action:"R5/R4 lead · R3 coordinate · R2 relay/support · R1 form the base. Always contact a member before removal."},
 es:{empty:"Invita miembros y sincroniza su progresión antes de generar un plan fiable.",line:(a,r,i,u=0)=>`${a} activos confirmados · ${r} por actualizar · ${i===0&&(r+u)>0?"inactividad no evaluada":`${i} inactivos probables`}.`,refresh:"Los datos son demasiado antiguos para asignaciones tácticas fiables: actualiza primero el roster.",core:n=>`Núcleo activo confirmado: ${n}.`,action:"R5/R4 dirigen · R3 coordinan · R2 apoyan/enlazan · R1 forma la base. Contacta siempre antes de excluir."},
 de:{empty:"Lade Mitglieder ein und synchronisiere ihren Fortschritt, bevor du einen verlässlichen Plan erstellst.",line:(a,r,i,u=0)=>`${a} bestätigt aktiv · ${r} zu aktualisieren · ${i===0&&(r+u)>0?"Inaktivität nicht bewertet":`${i} wahrscheinlich inaktiv`}.`,refresh:"Die Daten sind zu alt für verlässliche taktische Zuweisungen: zuerst den Roster aktualisieren.",core:n=>`Bestätigter aktiver Kern: ${n}.`,action:"R5/R4 führen · R3 koordinieren · R2 unterstützen/vermitteln · R1 bildet die Basis. Vor Ausschluss immer Kontakt aufnehmen."},
 ja:{empty:"信頼できる計画を作る前にメンバーを招待し進捗を同期してください。",line:(a,r,i,u=0)=>`アクティブ確認 ${a} · 更新必要 ${r} · ${i===0&&(r+u)>0?"非アクティブ判定未評価":`非アクティブの可能性 ${i}`}。`,refresh:"信頼できる戦術配置にはデータが古すぎます。先にロスターを更新してください。",core:n=>`確認済みアクティブ中核：${n}。`,action:"R5/R4が判断、R3が運用リーダー、R2が連携/支援、R1が基盤。除名前に必ず連絡してください。"},
 zh:{empty:"先邀请成员并同步进度，再生成可靠计划。",line:(a,r,i,u=0)=>`已确认活跃 ${a} · 需要更新 ${r} · ${i===0&&(r+u)>0?"未评估不活跃状态":`可能不活跃 ${i}`}。`,refresh:"数据过旧，无法进行可靠战术分配；请先更新名单。",core:n=>`已确认活跃核心：${n}。`,action:"R5/R4负责决策，R3作为行动干部，R2负责联络/支援，R1作为基础。移除成员前务必先联系。"},
 ar:{empty:"ادعُ الأعضاء وزامن تقدمهم قبل إنشاء خطة موثوقة.",line:(a,r,i,u=0)=>`${a} نشط مؤكد · ${r} يحتاج تحديث · ${i===0&&(r+u)>0?"لم يتم تقييم عدم النشاط":`${i} غير نشط على الأرجح`}.`,refresh:"البيانات قديمة جداً لتوزيعات تكتيكية موثوقة؛ حدّث القائمة أولاً.",core:n=>`النواة النشطة المؤكدة: ${n}.`,action:"R5/R4 يقودان القرار، R3 قادة عمليات، R2 دعم/ربط وR1 قاعدة التحالف. تواصل دائماً قبل الاستبعاد."}
};
function buildAllianceAdvice(state,locale){
  ...
  const planB=[];
  // `counts.refresh` already includes unknown members; do not double-count them in Plan B.
  if(needsRefresh)planB.push({rank:1,kind:"refresh",action_key:"alliance_plan_b_refresh",count:(counts.refresh||0)});
  ...
  const inactivityEvaluated=!((counts.inactive||0)===0&&((counts.refresh||0)>0));
  const advice=[pack.line(counts.active,counts.refresh,counts.inactive,counts.unknown),allianceRoleLine(summary.roleCounts,lang),needsRefresh?pack.refresh:"",core.length?pack.core(core.join(" / ")):"",!needsRefresh?pack.action:""].filter(Boolean).join("\n");
  return {advice,confidence:summary.confidence,activity:counts,inactivity_evaluated:inactivityEvaluated,roles:summary.roleCounts,core,reliability:needsRefresh?"refresh_required":"usable",immediate_actions:immediate,plan_b:planB,policy:"No member is removed by this advice; uncertain activity stays marked for refresh."};
}
...
const VS_DAILY_PLAN={
  fr:{
    1:{today:"Jour 1 · Radar / récolte : récupère les tâches Radar et les récoltes qui comptent réellement aujourd’hui.",keep:"Garde les accélérateurs de construction pour Jour 2, recherche/badges/composants Drone pour Jour 3 et fragments/EXP/médailles héros pour Jour 4.",avoid:"Évite de dépenser des ressources prévues pour Jour 2–4 si elles ne marquent pas aujourd’hui."},
    2:{today:"Jour 2 · Expansion de base : bâtiments et accélérateurs de construction. Si tes tâches l’affichent, récupère aussi bâtiments terminés, camions UR, missions secrètes légendaires et recrutement de survivants.",keep:"Garde accélérateurs de recherche, badges de bravoure et coffres/composants Drone pour Jour 3 ; garde fragments héros, EXP héros et médailles de compétence pour Jour 4.",avoid:"Évite fragments héros, EXP héros, médailles de compétence et ressources rares qui ne rapportent pas dans les tâches VS actives aujourd’hui."},
    3:{today:"Jour 3 · Science : priorité aux recherches, accélérateurs de recherche, badges de bravoure et composants Drone seulement s’ils figurent dans les tâches actives.",keep:"Garde fragments héros, EXP héros et médailles de compétence pour Jour 4 ; conserve les ressources combat/mobilisation pour leurs jours.",avoid:"Évite les dépenses héros avant Jour 4 si elles ne marquent pas aujourd’hui."},
    4:{today:"Jour 4 · Héros : utilise les ressources héros qui rapportent réellement aujourd’hui et respecte le TOP 3 personnalisé du Diagnostic PRO.",keep:"Garde les ressources de mobilisation/combat pour Jour 5–6 selon les tâches visibles.",avoid:"Évite de disperser fragments et ressources rares hors des priorités personnalisées du compte."},
    5:{today:"Jour 5 · Mobilisation : suis les tâches actives affichées et dépense uniquement les ressources qui y rapportent.",keep:"Prépare Jour 6 et garde les ressources combat qui y sont dédiées.",avoid:"Évite de vider les ressources combat avant leur fenêtre de points."},
    6:{today:"Jour 6 · Enemy Buster : privilégie les actions combat qui comptent réellement dans ton VS et protège tes troupes selon le contexte.",keep:"Après le VS, commence à préparer le prochain Jour 1 sans dépenser inutilement les ressources des prochains jours.",avoid:"Évite les combats à perte ou les dépenses sans points vérifiés."}
  },
  en:{
    1:{today:"Day 1 · Radar / gathering: claim Radar tasks and gathering that actually score today.",keep:"Keep construction speedups for Day 2, research/badges/Drone components for Day 3, and hero shards/EXP/skill medals for Day 4.",avoid:"Avoid spending Day 2–4 resources when they do not score today."},
    2:{today:"Day 2 · Base Expansion: buildings and construction speedups. When shown in your active tasks, also claim completed buildings, UR trucks, legendary secret missions and survivor recruitment.",keep:"Keep research speedups, Valor badges and Drone component chests for Day 3; keep hero shards, hero EXP and skill medals for Day 4.",avoid:"Avoid hero shards, hero EXP, skill medals and scarce resources that do not score in today’s active VS tasks."},
    3:{today:"Day 3 · Science: prioritize research, research speedups, Valor badges and Drone components only when they are in the active tasks.",keep:"Keep hero shards, hero EXP and skill medals for Day 4; save mobilization/combat resources for their scoring days.",avoid:"Avoid hero spending before Day 4 when it does not score today."},
    4:{today:"Day 4 · Heroes: use hero resources that actually score today and follow the personalized Diagnostic PRO TOP 3.",keep:"Keep mobilization/combat resources for Day 5–6 according to visible tasks.",avoid:"Avoid spreading shards and scarce resources outside the player-specific priorities."},
    5:{today:"Day 5 · Mobilization: follow the visible active tasks and spend only resources that score there.",keep:"Prepare Day 6 and keep combat resources dedicated to it.",avoid:"Avoid emptying combat resources before their scoring window."},
    6:{today:"Day 6 · Enemy Buster: prioritize combat actions that actually score in your VS and protect troops according to context.",keep:"After VS, start preparing the next Day 1 without wasting future-day resources.",avoid:"Avoid losing fights or spending resources without verified points."}
  }
};
function vsDailyPlan(locale,day,pack){const base=VS_DAILY_PLAN[localePack(locale)]||VS_DAILY_PLAN.en;return base?.[day]||{today:pack.days[day]||pack.hold,keep:pack.hold,avoid:localePack(locale)==="fr"?"Évite les ressources rares hors des tâches VS actives.":"Avoid scarce resources outside active VS tasks."}}
function vsFmtScore(v,locale){const n=Number(v);return Number.isFinite(n)?new Intl.NumberFormat(locale||"en",{maximumFractionDigits:0}).format(n):"—"}
function vsFmtCompact(v,locale){const n=Number(v);return Number.isFinite(n)?new Intl.NumberFormat(locale||"en",{notation:"compact",maximumFractionDigits:1}).format(n):"—"}
...
function buildVsAdvice(state,locale,opts={}){
  ...
  const decisionNow=opts?.now||new Date();
  const sit=vsSituation(v),trend=vsTrend(v),personal=personalVsPosition(v),decisionEngine=vsDecisionEngine(v,{now:decisionNow}),isFr=localePack(locale)==="fr",daily=vsDailyPlan(locale,day,pack),known=scoreKnown(v),liveKnown=known&&!decisionEngine.stale;
  let situationText=decisionEngine.stale?(isFr?"Données VS anciennes : le dernier scan n’appartient pas au jour serveur VS actuel.":"Stale VS data: the last scan is not from the current VS server day."):liveCopy.situation.unknown();
  if(liveKnown){const gap=vsFmtScore(Math.abs(sit.gap),locale),share=Math.round(sit.our_share??0);situationText=(liveCopy.situation[sit.status]||liveCopy.situation.unknown)(gap,share)}
  let decisionText=decisionEngine.stale?(isFr?"Scanne le VS du jour avant toute décision de dépense. L’ancien score reste historique uniquement.":"Scan today's VS before any spending decision. The old score is history only."):(liveCopy.action[decisionEngine.decision]||liveCopy.action.scan);
  const remain=num(v.time_remaining_seconds),activeTrend=(decisionEngine.ended||decisionEngine.stale)?null:trend;if(!decisionEngine.ended&&!decisionEngine.stale&&remain!==null&&remain>0&&remain<=7200)decisionText=`${decisionText} ${liveCopy.late}`;
  let trendText="";if(activeTrend){const mins=Math.max(1,Math.round(activeTrend.elapsed_seconds/60)),a=vsFmtCompact(activeTrend.our_gain,locale),b=vsFmtCompact(activeTrend.their_gain,locale);trendText=activeTrend.momentum==="theirs"?liveCopy.trendO(a,b,mins):liveCopy.trendU(a,b,mins)}else if(liveKnown&&!decisionEngine.ended)trendText=liveCopy.needSecond;
  ...
  const personalText=personal.score!==null?liveCopy.visibleContribution(vsFmtScore(personal.score,locale)):"",todayText=decisionEngine.stale?(isFr?"N’utilise aucune ressource sur la base de cet ancien scan ; scanne d’abord le VS actuel.":"Do not use resources based on this old scan; scan the current VS first."):(decisionEngine.ended?(isFr?"Fenêtre VS terminée : aucune dépense VS supplémentaire.":"VS window ended: no additional VS spending."):daily.today),keepText=decisionEngine.stale?(isFr?"Conserve les ressources jusqu’à un scan du jour confirmé.":"Keep resources until a confirmed current-day scan."):(decisionEngine.ended?(isFr?"Conserve les ressources pour la prochaine fenêtre utile.":"Keep resources for the next useful scoring window."):daily.keep),avoidText=decisionEngine.stale?(isFr?"Évite toute projection, tendance ou dépense basée sur le score ancien.":"Avoid any projection, trend or spending based on the old score."):(decisionEngine.ended?(isFr?"Évite tout nouveau scan ou dépense pour ce VS terminé.":"Avoid any new scan or spending for this ended VS window."):daily.avoid);
  const lines=[`${isFr?"Décision":"Decision"} : ${liveCopy.decision[decisionEngine.decision]||liveCopy.decision.scan}`,`${isFr?"Urgence":"Urgency"} : ${liveCopy.urgency[decisionEngine.urgency]||liveCopy.urgency.unknown}`,`${isFr?"Situation":"Situation"} : ${situationText}`,v.theme?`${isFr?"Thème":"Theme"} : ${v.theme}`:"",`${isFr?"Aujourd’hui":"Today"} : ${todayText}`,`${isFr?"Pourquoi":"Why"} : ${decisionText}`,trendText?`${isFr?"Tendance":"Trend"} : ${trendText}`:"",riskText?`${isFr?"Projection":"Projection"} : ${riskText}`:"",`${isFr?"À garder":"Keep"} : ${keepText}`,`${isFr?"À éviter":"Avoid"} : ${avoidText}`,personalText?`${isFr?"Contexte personnel":"Personal context"} : ${personalText}`:"",Array.isArray(v.leaderboard)&&v.leaderboard.length?liveCopy.rankingGuard:""].filter(Boolean);
  let confidence=45+(v.opponent?10:0)+(liveKnown?18:0)+(v.theme?6:0)+(activeTrend?8:0)+(state?.updated_at||state?.sync?.last_sync?7:0);confidence=Math.max(25,Math.min(decisionEngine.stale?49:94,confidence-(freshness?.confidence_penalty||0)));
  const priorities=[{rank:1,kind:"today",label:isFr?"Aujourd’hui":"Today",text:todayText},{rank:2,kind:"keep",label:isFr?"À garder":"Keep",text:keepText},{rank:3,kind:"avoid",label:isFr?"À éviter":"Avoid",text:avoidText}];
