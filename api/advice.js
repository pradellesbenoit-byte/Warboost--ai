...
function scoreVisibleOffer(o,needs,state){const rawStore=o?.store_type||o?.store||"",cat=itemCategory(o?.item_name,o?.category,rawStore),store=storeKind(rawStore),shop=state?.shop||{},a=adaptiveText(state?._locale),factors=[];let score=baseOfferScore(cat,needs);const mi=metaShopAdjustment(cat,needs,state);score+=mi.bonus;if(mi.bonus)factors.push(`Multi-source meta +${mi.bonus}`);
  ...
  return {score,cat,factors,budget:{currency,price,balance,reserve,diamond},opaque_container:opaqueGuard.opaque,opaque_score_cap:opaqueGuard.cap};
}
...
function referenceCatalogRecommendations(state,locale,needs){
  ...
  for(const r of refs){
    ...
    const da=diagnosticShopAdjustment(r.category,state?._shop_alignment,locale),opaqueGuard=opaqueContainerGuard(r.category,locale);out.push({item:r.item,store:`${r.store} · ${safe.notVerified}`,score,score_label:a.score(score),reason:[offerReason(r.category,needs,p),...da.reasons,opaqueGuard.reason,r.temporary?(localePack(locale)==="fr"?"Offre/rotation observée : rescanner avant achat.":"Observed rotating offer: rescan before buying."):""].filter(Boolean).join(" ").trim(),target,verdict:action.verify,verdict_key:"strategy",price_label:price,source:"reference",category:r.category,availability_label:safe.notVerified,diagnostic_alignment:{resource_family:da.family,priority_rank:da.rank,bonus:da.bonus},opaque_container:opaqueGuard.opaque,opaque_score_cap:opaqueGuard.cap,_sort:score});
  }
...
function buildShopAdvice(state,locale,analysis){
  ...
  return {summary,confidence,confidence_label:`${T[localePack(locale)]?.confidence?T[localePack(locale)].confidence(confidence):`Confidence ${confidence}%`}`,scan_based:scanBased,catalog_status:catalogStatus,catalog_complete:officialCatalog,catalog_label:catalogLabel,store_type:store,updated_at:shop?.updated_at||null,data_freshness:catalogFreshness,recommendations,knowledge_date:SHOP_REFERENCE_DATE,reference_catalog:referenceStats,observed_shop_count:new Set(liveObserved.map(o=>o._store_type)).size,observed_offer_count:liveObserved.length,meta_intelligence:metaContext(state),method:officialCatalog?"official-shop-catalog + diagnostic-pro-alignment + opaque-container-guard + adaptive-account-rules + diamond-reserve + VS/season-context":liveObserved.length?"multi-shop-visible-scan + dated-reference-matching + diagnostic-pro-alignment + opaque-container-guard + adaptive-account-rules + diamond-reserve + VS/season-context":"dated-user-screenshot-reference + diagnostic-pro-alignment + opaque-container-guard + adaptive-account-rules + availability-unverified",diagnostic_alignment:alignment,budget:{currency:currency||null,balance,reserve_diamonds:10000},needs:{exclusive_urgency:Math.round(needs.exclusiveUrgency*100),stars_urgency:Math.round(needs.starsUrgency*100),gear_urgency:Math.round(needs.gearUrgency*100),drone_urgency:Math.round(needs.droneUrgency*100)},limitations:officialCatalog?[]:["Reference catalogue is dated and does not prove current availability","Fresh scans are accumulated across multiple shops for live ranking","Unknown item types receive no purchase recommendation","Opaque chests cannot outrank known direct progression solely on rarity","Paid offers require fresh account/shop data"],sources:[officialCatalog?"Official Last War read-only catalogue":"User-provided Last War shop scans",`WarBoost screenshot reference catalogue ${SHOP_REFERENCE_DATE}`,"WarBoost saved account state"]};
}
